  ...
  public void testCmd() throws Exception {
    LinkedList<String> expectedFailureList =
        new LinkedList<String>(
            Arrays.asList(
                new String[] {
                  // exec-1.3 and exec-63 fail because the JVM process model doesn't allow for
                  // proper STDIN
                  // file descriptor inheritance, instead using a pipe model.  Too much of stdin can
                  // get
                  // eaten up because of this.  The solution is to create platform-specific
                  // subclasses
                  ...
                  // exec-11.5 and exec-17.1 fail because JVM doesn't allow background processes to
                  // survive the JVM
                  // if they read from STDIN or write to STDOUT/STDERR.  There's no direct access
                  // in the JVM to filedescriptors 0, 1, 2; we're only given InputStream() and
                  // OutputStream()
