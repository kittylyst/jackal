...
/**
 * A class for managing the System.in. Avoids blocking the current thread on the non-interruptible
 * System.in.read(), by isolating all System.in.read()'s in a separate thread. Allows other classes
 * to use System.in in a coordinated way, so that one class doesn't block access to System.in.
 *
 * <p>After the initial instance is created, Java objects can read from System.in directly to get
 * the benefits of this class, or can create another instance of this class that can be close()'d to
 * interrupt a blocked read.
 *
 * <p>This class forces standard input to remain unbuffered, so the JVM doesn't steal bytes from
 * stdin that might be useful to subsequent processes that run after this JVM exits.
 */
public class ManagedSystemInStream extends InputStream implements Runnable {
  ...
  /** The object that communicates requests between this thread and the reader thread */
  ...
  /**
   * Create a new ManagedSystemInStream. If this is the first ManagedSystemInStream instance,
   * System.in is set to this instance, so all direct access to System.in goes through this
   * instance. Other objects can either read System.in directly, or create an instance of this class
   * to read from.
   */
    ...
    synchronized (readRequest) {
      ...
      if (stdin == null) {
        ...
        try {
          ...
          // do nothing, but if this doesn't work exec and redirection of shell's input probably
  ...
  public int read(byte[] b, int offset, int length) throws IOException {
    ...
    while (true) {
      if (eofSeen || streamClosed) return -1;
      ...
      synchronized (readRequest) {
        ...
        if (requestMade && readRequest.validData) {
          ...
          readRequest.validData = false;
          if (readRequest.exception != null) throw readRequest.exception;
          if (readRequest.actualLength == -1) eofSeen = true;
  ...
  public void run() {
    ...
    while (true) {
      ...
      if (doRead) {
        ...
        try {
          readRequest.actualLength =
              stdin.read(readRequest.buf, readRequest.offset, readRequest.len);
