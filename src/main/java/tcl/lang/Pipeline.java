...
public class Pipeline implements Runnable {
   ...
   *
   * @param interp TCL interpreter for this Pipeline
   * @param objv Array of TclObjects that are in the for of TCL's 'exec' args
   * @param startIndex index of the objv object at which to start parsing
   * @throws TclException if a syntax error occurs
   */
        ...
        if (arg.length() > redirector.length()) {
          ...
          if (i > endIndex) {
            throw new TclException(
                interp, "can't specify \"" + redirector + "\" as last word in command");
          }
   ...
   *
   * @param command The command and its arguments. The List is not copied, so any changes will be
   *     reflected in the Pipeline
   * @param workingDir New command starts in this directory
  ...
  public int addCommand(List<String> command, File workingDir) throws TclException {
    ...
    if (workingDir != null) proc.setWorkingDir(workingDir);
  ...
  /**
   * Executes the pipeline. If the execInBackground property is false, exec() waits for processes to
   * complete before returning. If any input and output channels (pipeline*Channel) have not been
   * set either in the constructor with the TCL redirection operator, or with setPipeline*Channel(),
   * data will be read/written from System.in, System.out, System.err.
   *
   * @throws TclException
   */
  public void exec() throws TclException {
    ...
    if (stdinRedirect != null) {
       ...
       * consume those bytes.
       */
      if (stdinRedirect.getType() == Redirect.Type.INHERIT
          && execInBackground
          && !processes.get(0).canInheritFileDescriptors()) {
        processes.get(0).setStdinRedirect(null);
      } else processes.get(0).setStdinRedirect(stdinRedirect);
    }
    ...
    for (int i = 0; i < processes.size(); i++) {
      ...
      try {
        ...
        throw new TclPosixException(
            interp, e, true, "couldn't execute \"" + process.command().get(0) + "\"");
      }
    }
  }
  ...
  /**
   * @param execInBackground Set the execInBackground property, which determines if the Pipeline
  ...
  /**
   * @return Exit values of all processes in the pipeline; only valid if processes have exited
   */
  ...
  /**
   * @throws IOException if one has been caught during a background waitForExitAndCleanup
   */
