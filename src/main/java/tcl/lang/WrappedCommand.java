 ...
 * generic/tclInt.h. It is "wrapped" around a TclJava Command interface reference. We need to wrap
 * Command references so that we can keep track of sticky issues like what namespace the command is
 * defined in without requiring that every implementation of a Command interface provide method to
...
public final class WrappedCommand {
  ...
  /** Set to true while delete traces are being executed to prevent recursive traces */
  private boolean deleteTraceInProgress = false;

  /** Set to true while rename traces are being executed to prevent recursive traces */
  ...
  public void untraceCommand(int type, String callbackCmd) throws TclException {
    ...
    if (commandTraces.size() == 0) commandTraces = null;
  }
   ...
   *
   * @param interp interpreter for error messages
   * @return List, in the form of [trace command info]
   * @throws TclException
   */
  public TclObject traceCommandInfo(Interp interp) throws TclException {
    if (commandTraces == null || commandTraces.size() == 0) return TclString.newInstance("");
    ...
    for (int i = 0; i < commandTraces.size(); i++) {
      ...
      if (isRename) TclList.append(interp, ops, TclString.newInstance("rename"));
      if (isDelete) TclList.append(interp, ops, TclString.newInstance("delete"));
   ...
   *
   * @param type ExecutionTrace.ENTER, ExecutionTrace.LEAVE, ExecutionTrace.ENTERSTEP or
   *     ExecutionTrace.LEAVESTEP
   * @param callbackCmd callback command to remove
   * @throws TclException
   */
  public void untraceExecution(int type, String callbackCmd) throws TclException {
    if (executionTraces == null) return;
    ...
    if (executionTraces != null
        && (type == ExecutionTrace.ENTERSTEP || type == ExecutionTrace.LEAVESTEP)) {
      hasStepTrace = false;
      for (ExecutionTrace trace : executionTraces) {
        if (trace.getType() == ExecutionTrace.ENTERSTEP
            || trace.getType() == ExecutionTrace.LEAVESTEP) {
          hasStepTrace = true;
          break;
        }
      }
   ...
   * @param type ExecutionTrace.ENTER, ExecutionTrace.ENTERSTEP, ExecutionTrace.LEAVE or
   *     ExecutionTrace.LEAVESTEP
   ...
   * @throws TclException
   */
  void callExecutionTraces(
      Interp interp, TclObject[] objv, int type, int completionCode, TclObject result)
      throws TclException {
  ...
  public TclObject traceExecutionInfo(Interp interp) throws TclException {
    if (executionTraces == null || executionTraces.size() == 0) return TclString.newInstance("");
    TclObject rv = TclList.newInstance();
    ...
    for (int i = 0; i < executionTraces.size(); i++) {
      ...
      for (int k = 0; k < 3; k++) traceTypes[k] = false;
  ...
  /**
   * @return true if the caller must call WrappedCommand.invoke(Interp, TclObject[]) because there
   *     are execution traces in effect. Note that it is always OK to call invoke() instead of
   *     cmd.cmdProc(), but doing so adds an extra Java stack frame that may limit how deep Tcl code
   *     stack frames can go.
   */
  public final boolean mustCallInvoke(Interp interp) {
    return ((executionTraces != null && !executionTraceInProgress)
        || interp.hasActiveExecutionStepTraces());
  }

  /**
   * Call cmd.cmdProc after calling execution traces on this command, and call execution traces
   * after this command exits. Application code should use this call, rather than directly calling
   * Command.cmdProc(Interp, TclObject[]) which does not fire execution traces.
   *
   * @param interp The interpreter containing the context for thie command, and where results are
   *     returned
   * @param objv argument list for command; objv[0] is the command name itself
    ...
    if (!mustCallInvoke(interp)) {
      ...
      try {
        ...
        callExecutionTraces(
            interp,
            objv,
            ExecutionTrace.LEAVE,
            savedException == null ? TCL.OK : savedException.getCompletionCode(),
            savedResult);
