...
class ParseExpr {
  ...
  static void ParseLorExpr(Interp interp, ParseInfo info) throws TclException {
    ...
    while (info.lexeme == OR) {
      ...
      PrependSubExprTokens(operator, 2, srcStart, (info.prevEnd - srcStart), firstIndex, info);
    }
  }
  ...
  static void ParseBitOrExpr(Interp interp, ParseInfo info) throws TclException {
    ...
    while (info.lexeme == BIT_OR) {
      ...
      PrependSubExprTokens(operator, 1, srcStart, (info.prevEnd - srcStart), firstIndex, info);
    }
  }
  ...
  static void ParseEqualityExpr(Interp interp, ParseInfo info) throws TclException {
    ...
    lexeme = info.lexeme;
    while ((lexeme == EQUAL) || (lexeme == NEQ) || (lexeme == STREQ) || (lexeme == STRNEQ)) {
      operator = info.start;
  ...
  static void ParseRelationalExpr(Interp interp, ParseInfo info) throws TclException {
    ...
    lexeme = info.lexeme;
    while ((lexeme == LESS) || (lexeme == GREATER) || (lexeme == LEQ) || (lexeme == GEQ)) {
      operator = info.start;
      ...
      PrependSubExprTokens(
          operator, operatorSize, srcStart, (info.prevEnd - srcStart), firstIndex, info);
      lexeme = info.lexeme;
    }
  }
  ...
  static void ParsePrimaryExpr(Interp interp, ParseInfo info) throws TclException {
    ...
    // System.out.println("parse primary "+info.lexeme+" "+info.start+" "+info.size+"
    // "+parseObj.numTokens);
    ...
    switch (lexeme) {
      ...
      case QUOTE:
        ...
        parseObj =
            Parser.ParseQuotedString(
                interp,
                info.originalExpr,
                (info.next - 1),
                (info.lastChar - stringStart),
                parseObj,
                true);
      ...
      case FUNC_NAME:
        ...
        if (info.lexeme != OPEN_PAREN) {
          ...
          TclObject obj =
              TclString.newInstance(
                  new String(savedInfo.originalExpr, savedInfo.start, savedInfo.size));
        ...
        while (info.lexeme != CLOSE_PAREN) {
          ...
          if (info.lexeme == COMMA) {
            ...
            LogSyntaxError(info, "missing close parenthesis at end of function call");
          }
        }
      ...
      case UNKNOWN:
        LogSyntaxError(info, "single equality character not legal in expressions");
      case UNKNOWN_CHAR:
  ...
  static void GetLexeme(Interp interp, ParseInfo info) throws TclException
    ...
    if ((c != '+') && (c != '-')) {
      ...
      } else if ((length = ParseMaxDoubleLength(info.originalExpr, src, info.lastChar)) > 0) {
  ...
  static void PrependSubExprTokens(
      int op, int opBytes, int src, int srcBytes, int firstIndex, ParseInfo info)
        /*
  ...
  static void LogSyntaxError(
      ParseInfo info, // Holds the parse state for the
