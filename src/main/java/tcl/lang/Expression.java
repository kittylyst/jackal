...
public class Expression {
  ...
  public static int precTable[] = {
    ...
    11, // PLUS, MINUS
    10,
    10, // LEFT_SHIFT, RIGHT_SHIFT
    9,
    9,
    9,
    9, // LESS, GREATER, LEQ, GEQ
    8,
    8, // EQUAL, NEQ
    7, // BIT_AND
    6, // BIT_XOR
  ...
  /**
   * Type of the last token to be parsed from the expression. Corresponds to the characters just
   * before expr.
   */
  int m_token;
  ...
  /**
   * Cache of ExprValue objects. These are cached on a per-interp basis to speed up most
   * expressions.
   */
  ...
  static void IllegalType(Interp interp, int badType, int operator) throws TclException {
    throw new TclException(
        interp,
        "can't use "
            + ((badType == ExprValue.DOUBLE) ? "floating-point value" : "non-numeric string")
            + " as operand of \""
  ...
  public static void IntegerTooLarge(Interp interp) throws TclException {
    interp.setErrorCode(
        TclString.newInstance("ARITH IOVERFLOW {integer value too large to represent}"));
  ...
  static void DomainError(Interp interp) throws TclException {
    interp.setErrorCode(
        TclString.newInstance("ARITH DOMAIN {domain error: argument not in valid range}"));
  ...
  static void EmptyStringOperandError(Interp interp, int operator) throws TclException {
    throw new TclException(
        interp,
        "can't use " + "empty string" + " as operand of \"" + operatorStrings[operator] + "\"");
  }

  /**
   * Given a TclObject, such as the result of a command or variable evaluation, fill in a ExprValue
   * with the parsed result. If the TclObject already has an internal rep that is a numeric type,
   * then no need to parse from the string rep. If the string rep is parsed into a numeric type,
   * then update the internal rep of the object to the parsed value.
   */
  public static void ExprParseObject(
      final Interp interp, final TclObject obj, final ExprValue value) throws TclException {
   ...
   *
   * <p>Given a TclObject that contains a String to be parsed (from a command or variable subst),
   * fill in an ExprValue based on the string's numeric value. The value may be a floating-point, an
   * integer, or a string. If the string value is converted to a numeric value, then update the
   * internal rep of the TclObject.
   *
   * @param interp the context in which to evaluate the expression.
   * @param obj the TclObject containing the string to parse.
   * @param value the ExprValue object to save the parsed value in.
   */
  public static void ExprParseString(
      final Interp interp, final TclObject obj, final ExprValue value) {
    char c;
    long ival;
    double dval;
   ...
   *
   * @param interp the context in which to evaluate the expression.
   * @param prec treat any un-parenthesized operator with precedence <= this as the end of the
   *     expression.
   * @exception TclException for malformed expressions.
   * @return the value of the expression.
   */
  private ExprValue ExprGetValue(Interp interp, int prec) throws TclException {
    ...
    if (m_token == OPEN_PAREN) {
      ...
      if (m_token >= UNARY_MINUS) {
        ...
        if (m_token >= MULT) SyntaxError(interp, "unexpected operator " + operatorStrings[m_token]);
        else if (m_token == UNKNOWN) SyntaxError(interp, "character not legal in expressions");
        else SyntaxError(interp, "premature end of expression");
      }
    }
    if (value == null) {
      SyntaxError(interp);
  ...
  public static void evalUnaryOperator(
      final Interp interp, final int operator, final ExprValue value) throws TclException {
    switch (operator) {
      case UNARY_MINUS:
        if (value.isIntType()) {
  ...
  public static void evalBinaryOperator(
      final Interp interp,
      final int operator,
      final ExprValue value, // value on left hand
      // side
    ...
    switch (operator) {
      ...
      case MOD:
        ...
        if (dividend == TCL.INT_MIN && divisor == -1) {
          ...
          if (remainder < 0 && !(neg_divisor && (dividend == TCL.INT_MIN))) {
            remainder += divisor;
          }
        }
   ...
   * GetLexeme -> ExprLex
   *
   * <p>Lexical analyzer for expression parser: parses a single value, operator, or other syntactic
   * element from an expression string.
   *
   * <p>Size effects: the "m_token" member variable is set to the value of the current token.
   *
   * @param interp the context in which to evaluate the expression.
   * @exception TclException for malformed expressions.
   * @return the value of the expression.
  ...
  /**
   * Parses a math function from an expression string, carry out the function, and return the value
   * computed.
   *
  ...
  ExprValue mathFunction(Interp interp) throws TclException {
    ...
    ExprLex(interp);
    if (m_token != OPEN_PAREN) {
      if (mathFunc == null) SyntaxError(interp, "variable references require preceding $");
      else SyntaxError(interp, "expected parenthesis enclosing function arguments");
    }
    ...
    if (numArgs == 0) {
      ...
      for (int i = 0; ; i++) {
        ...
        if ((value == null) && (m_token == CLOSE_PAREN)) {
          if (i == numArgs) break;
          else throw new TclException(interp, "too few arguments for math function");
        }
  ...
  /**
   * This procedure will lookup and invoke a math function given the name of the function and an
   * array of ExprValue arguments. Each ExprValue is released before the function exits. This method
   * is intended to be used by other modules that may need to invoke a math function at runtime. It
   * is assumed that the caller has checked the number of arguments, the type of the arguments will
   * be adjusted before invocation if needed.
   *
   * <p>The values argument can be null when there are no args to pass.
   *
   * <p>The releaseValues argument should be true when the ExprValue objecys in the array should be
   * released.
   */
  public void evalMathFunction(
      Interp interp, String funcName, ExprValue[] values, boolean releaseValues, ExprValue result)
      throws TclException {
  ...
  /**
   * This procedure implements a math function invocation. See the comments for the function above,
   * note that this method is used when the math function pointer has already been resolved.
   *
   * <p>The values argument can be null when there are no args to pass.
   *
   * <p>The releaseValues argument should be true when the ExprValue objecys in the array should be
  ...
  /**
   * This procedure will register a math function by adding it to the table of available math
   * functions. This methods is used when regression testing the expr command.
   */
  ...
  /**
   * This procedure decides whether the leading characters of a string look like an integer or
   * something else (such as a floating-point number or string). If the whole flag is true then the
   * entire string must look like an integer.
   *
...
class RandFunction extends NoArgMathFunction {
  ...
  /**
   * Srand calls the main algorithm for rand after it sets the seed. To facilitate this call, the
   * method is static and can be used w/o creating a new object. But we also need to maintain the
   * inheritance hierarchy, thus the dynamic apply() calls the static statApply().
   */
