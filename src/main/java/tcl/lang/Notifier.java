 ...
 * event system. It is used by higher-level event sources such as file, JavaBean and timer events.
 * The Notifier manages an event queue that holds TclEvent objects.
 *
 * <p>The Jacl notifier is designed to run in a multi-threaded environment. Each notifier instance
 * is associated with a primary thread. Any thread can queue (or dequeue) events using the
 * queueEvent (or deleteEvents) call. However, only the primary thread may process events in the
 * queue using the doOneEvent() call. Attepmts to call doOneEvent from a non-primary thread will
 * cause a TclRuntimeError.
 *
 * <p>This class does not have a public constructor and thus cannot be instantiated. The only way to
 * for a Tcl extension to get an Notifier is to call Interp.getNotifier() (or
 * Notifier.getNotifierForThread() ), which returns the Notifier for that interpreter (thread).
 */
public class Notifier implements EventDeleter {
  ...
  /**
   * Increment the reference count of the notifier. The notifier will be kept in the notifierTable
   * (and alive) as long as its reference count is greater than zero.
   */
  ...
  /**
   * Decrement the reference count of the notifier. The notifier will be freed when its refCount
   * goes from one to zero. Side effects: The notifier may be removed from the notifierTable when
  ...
  /**
   * Insert an event into the event queue at one of three positions: the head, the tail, or before a
   * floating marker. Events inserted before the marker will be processed in first-in-first-out
   * order, but before any events inserted at the tail of the queue. Events inserted at the head of
   * the queue will be processed in last-in-first-out order.
   *
   * <p>Results: None.
   *
   * <p>Side effects: If this method is invoked by a non-primary thread, the primaryThread of this
   * Notifier will be notified about the new event.
   *
  ...
  /**
   * Calls an EventDeleter for each event in the queue and deletes those for which
   * deleter.deleteEvent() returns 1. Events for which the deleter returns 0 are left in the queue.
   * This method includes code to handle the special case of the Notifier wanting to delete a single
   * event after is has been serviced. This method is concurrent safe.
   *
   * <p>Results: None.
   *
   * <p>Side effects: Potentially removes one or more events from the event queue.
   *
  ...
  /**
   * This method is required to implement the EventDeleter interface It is not actually used though,
   * see deleteEvents method for special casing of the deletion of a specific event.
   */
  @Deprecated
  ...
  /**
   * Process a single event of some sort. If there's no work to do, wait for an event to occur, then
   * process it. May delay execution of process while waiting for an event, unless TCL.DONT_WAIT is
   * set in the flags argument.
   *
   * <p>Side effects: May delay execution of process while waiting for an event, unless
   * TCL.DONT_WAIT is set in the flags argument. Event sources are invoked to check for and queue
   * events. Event handlers may produce arbitrary side effects.
   *
   ...
   *     event sources.
   * @return 1 if the procedure actually found an event to process. If no processing occurred, then
   *     0 is returned (this can happen if the TCL.DONT_WAIT flag is set or if there are no event
  ...
  public int doOneEvent(int flags) {
    ...
    if (primaryThread == null) {
      ...
      throw new TclRuntimeError(
          "Notifier.doOneEvent() with " + "no Interp() objects in the current thread");
    }
