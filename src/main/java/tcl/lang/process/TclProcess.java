 ...
 * should create a subclass of TclProcess, which is instantiated with with TclProcess.newInstance().
 * This class is somewhat of a combination of java.lang.ProcessBuilder and java.lang.Process, with
 * inspiration from the Java 1.7 API. It is provided to allow for better native process control than
 * Java's APIs.
 *
 * @author Dan Bodoh
 */
public abstract sealed class TclProcess permits JavaProcess {
  ...
  /**
   * Create a new platform-specific instance of a TclProcess subclass. This method detects the
   * current platform and returns an instance of the appropriate TclProcess subclass.
   *
   ...
   *
   * @param command Command words to be executed
   */
  ...
  /**
   * @return a unmodifiable Map containing the environment that the subprocess should inherit.
   *     System.getenv() is not sufficient because JTcl cannot update it on env() array changes.
   */
   ...
   * @throws IllegalThreadStateException if the process has not yet completed
   */
   ...
   * @throws IllegalThreadStateException if the process has not yet been started
   * @throws InterruptedException if the process is interrupted
   * @throws IOException
   */
  public int waitFor() throws IllegalThreadStateException, InterruptedException, IOException {
    if (!isStarted()) throw new IllegalThreadStateException("Process not yet started");
    int rv = implWaitFor();
    if (stdoutRedirect != null
        && stdoutRedirect.type == Redirect.Type.TCL_CHANNEL
        && stdoutRedirect.closeChannel) {
      TclIO.unregisterChannel(interp, stdoutRedirect.channel);
    }
    if (stderrRedirect != null
        && stderrRedirect.type == Redirect.Type.TCL_CHANNEL
        && stderrRedirect.closeChannel) {
  ...
  /**
   * @return The process identifier of this process, or -1 if it cannot be determined
   * @throws IllegalThreadStateException if the process has not yet started
   */
  ...
  /**
   * @param workingDir Directory the process starts in
   */
  ...
  /**
   * @return true if the this TclProcess subclass can actually inherit the open file descriptors, or
   *     false if it emulates inheritance
   */
  ...
  /**
   * @param e Exception to throw later in the main thread
   */
  ...
  /**
   * @throws Exception if any exception was passed to saveException()
   */
  ...
  /**
   * @param stdinRedirect the stdinRedirect to set
   */
    ...
    if (stdinRedirect.type == Redirect.Type.TCL_CHANNEL) {
      if (stdinRedirect.channel.isWriteOnly())
        throw new TclException(
            interp,
            "channel \"" + stdinRedirect.channel.getChanName() + "\" wasn't opened for reading");
    }
    if (stdinRedirect.type == Redirect.Type.FILE) {
      if (!stdinRedirect.file.exists()) {
        throw new TclPosixException(
            ...
            true,
            "couldn't read file \"" + stdinRedirect.specifiedFilePath + "\"");
      }
    }

    /* Convert stdin channel to inherit */
    if (stdinRedirect.type == Redirect.Type.TCL_CHANNEL
        && "stdin".equals(stdinRedirect.channel.getChanName())) {
  ...
  /**
   * @param stdoutRedirect the stdoutRedirect to set
   */
  public void setStdoutRedirect(Redirect stdoutRedirect) throws TclException {
    ...
    if (stdoutRedirect.type == Redirect.Type.TCL_CHANNEL) {
      if (stdoutRedirect.channel.isReadOnly())
        throw new TclException(
            interp,
            "channel \"" + stdoutRedirect.channel.getChanName() + "\" wasn't opened for writing");
      try {
    ...
    if (stdoutRedirect.type == Redirect.Type.FILE) {
      ...
      if (!testFile.exists()) {
        throw new TclPosixException(
            ...
            TclPosixException.ENOENT,
            true,
            "couldn't write file \"" + stdoutRedirect.specifiedFilePath + "\"");
      }
    }
    if (stdoutRedirect.type == Redirect.Type.TCL_CHANNEL
        && "stdout".equals(stdoutRedirect.channel.getChanName())) {
  ...
  /**
   * @param stderrRedirect the stderrRedirect to set
   */
  public void setStderrRedirect(Redirect stderrRedirect) throws TclException {
    ...
    if (stderrRedirect.type == Redirect.Type.TCL_CHANNEL) {
      if (stderrRedirect.channel.isReadOnly())
        throw new TclException(
            interp,
            "channel \"" + stderrRedirect.channel.getChanName() + "\" wasn't opened for writing");
      try {
    ...
    if (stderrRedirect.type == Redirect.Type.FILE) {
      ...
      if (!testFile.exists()) {
        throw new TclPosixException(
            ...
            TclPosixException.ENOENT,
            true,
            "couldn't write file \"" + stderrRedirect.specifiedFilePath + "\"");
      }
    }

    if (stderrRedirect.type == Redirect.Type.TCL_CHANNEL
        && "stderr".equals(stderrRedirect.channel.getChanName())) {
