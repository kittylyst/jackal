...
public class TclInteger implements InternalRep {
   ...
   *
   * <p>Creates a new instance of a TclObject with a TclInteger internal representation.
   *
   * @param i initial value of the integer object.
   ...
   *
   * <p>Called to convert the other object's internal rep to this type.
   *
   * @param interp current interpreter.
   * @param tobj the TclObject to convert to use the representation provided by this class.
   */
   ...
   *
   * <p>Returns the integer value of the object as a Java long.
   *
   * @param interp current interpreter.
   * @param tobj the object to operate on.
   * @return the integer value of the object.
   */
  public static long getLong(final Interp interp, final TclObject tobj) throws TclException {
    if (!tobj.isIntType()) {
   ...
   * Tcl_GetIntFromObj -> TclInteger.getValue
   *
   * <p>Returns the integer value of the object as a Java int.
   *
   * @param interp current interpreter.
   * @param tobj the object to operate on.
   ...
   */
  public static int getInt(final Interp interp, final TclObject tobj) throws TclException {
   ...
   * @param tobj The TclObject to be tested
   * @return true if getInt() will return an int; false if object cannot be converted to a
   *     TclInteger or if it is out of range for an int
   */
  ...
  /**
   * Increments the integer value of the object by the given amount. One could implement this same
   * operation by calling get() and then set(), this method provides an optimized implementation.
   * This method is not public since it will only be invoked by the incr command.
   *
   * @param interp current interpreter.
   * @param tobj the object to operate on.
   * @param incrAmount amount to increment
   */
  public static void incr(final Interp interp, final TclObject tobj, final long incrAmount)
      throws TclException {
    if (!tobj.isIntType()) {
      setIntegerFromAny(interp, tobj);
    }
  ...
  /**
   * This special helper method is used only by the Expression module. This method will change the
   * internal rep to a TclInteger with the passed in int value. This method does not invalidate the
   * string rep since the object's value is not being changed.
   *
   * @param tobj the object to operate on.
    ...
    if (validate) {
      ...
      if (tobj.isIntType()) {
        throw new TclRuntimeError(
            "exprSetInternalRep() called with object" + " that is already of type TclInteger");
      }
      ...
      if (i != i2) {
        throw new TclRuntimeError(
            "exprSetInternalRep() called with int value "
                + i
                + " that does not match parsed int value "
                + i2
                + ", parsed from str \""
                + tobj.toString()
                + "\"");
