...
public class Var {
   ...
   * give the "type" of the variable. UNDEFINED is independent of the variable's type. Note that
   * using an int field here instead of allocating 9 boolean members makes the resulting object take
   * up less memory. If there were only 8 boolean fields then the size of the Var object would be
   * the same.
   *
   ...
   * local table, but it is not a local or imported local. This flag is only set in compiled code
   * when scoped global var refs like $::myvar are found. These variables are not considered part of
   * a variable frame and can't be found at runtime.
   */
   ...
   * NewVar -> Var
   *
   * <p>Construct a variable and initialize its fields.
   */
  public Var() {
    setValue(null);
    deleteArrayMap();
  ...
  /**
   * Used by ArrayCmd to create a unique searchId string. If the sidVec List is empty then simply
   * return 1. Else return 1 plus the SearchId.index value of the last Object in the vector.
   *
  ...
  /**
   * Find the SearchId that in the sidVec List that is equal the unique String s and returns the
   * iterator associated with that SearchId.
   *
   ...
   *
   * <p>This procedure is used by virtually all of the variable code to locate a variable given its
   * name(s).
   *
   * @param part1 if part2 isn't NULL, this is the name of an array. Otherwise, this is a full
   *     variable name that could include a parenthesized array element or a scalar.
   ...
   * @param createPart2 OR'ed combination of CRT_PART1 and CRT_PART2. Tells which entries to create
   *     if they don't already exist.
   * @return a two element array. a[0] is the variable indicated by part1 and part2, or null if the
   *     variable couldn't be found and throwException is false.
   *     <p>If the variable is found, a[1] is the array that contains the variable (or null if the
   *     variable is a scalar). If the variable can't be found and either createPart1 or createPart2
   *     are true, a new as-yet-undefined (VAR_UNDEFINED) variable instance is created, entered into
   *     a hash table, and returned. Note: it's possible that var.value of the returned variable may
   *     be null (variable undefined), even if createPart1 or createPart2 are true (these only cause
   *     the hash table entry or array to be created). For example, the variable might be a global
   *     that has been unset but is still referenced by a procedure, or a variable that has been
   *     unset but it only being kept in existence by a trace.
   * @exception TclException if the variable cannot be found and throwException is true.
   */
  public static Var[] lookupVar(
      Interp interp,
      String part1,
      String part2,
      int flags,
            ...
            if (clocal == null) {
              ...
              if (clocal.isVarNonLocal()) {
                throw new TclRuntimeError(
                    "can't lookup scoped variable \"" + part1 + "\" in local table");
              }
              var = clocal;
            }
            break;
   ...
   *
   * <p>Query the value of a variable, given a two-part name consisting of array name and element
   * within array.
   *
   ...
   */
  public static TclObject getVar(
      Interp interp, // interpreter to look for the
      // var in
   ...
   *
   * <p>Query the value of a variable, given refs to the variables Var objects.
   *
   * @param interp the interp that holds the variable
   * @param part1 1st part of the variable name.
   * @param part2 2nd part of the variable name.
  ...
  static TclObject getVarPtr(
      Interp interp, // interpreter to look for the var
      // in
      Var var,
      Var array,
      String part1, // Name of an array (if part2 is
      // non-null)
      // or the name of a variable.
      String part2, // If non-null, gives the name of an element
      // in the array part1.
   ...
   *
   * <p>Given a two-part variable name, which may refer either to a scalar variable or an element of
   * an array, change the value of the variable to a new Tcl object value. See the setVarPtr()
   * method for the arguments to be passed to this method.
   */
  public static TclObject setVar(
      Interp interp, String part1, String part2, TclObject newValue, int flags)
      throws TclException {
   ...
   *
   * <p>This method implements setting of a variable value that has already been resolved into Var
   * refrences. Pass the resolved var refrences and a two-part variable name, which may refer either
   * to a scalar or an element of an array. This method will change the value of the variable to a
   * new TclObject value. If the named scalar or array or element doesn't exist then this method
   * will create one.
   *
   * @param interp the interp that holds the variable
   ...
   * @param flags misc flags that control the actions of this method
   *     <p>Returns a pointer to the TclObject holding the new value of the variable. If the write
   *     operation was disallowed because an array was expected but not found (or vice versa), then
   *     null is returned; if the TCL.LEAVE_ERR_MSG flag is set, then an exception will be raised.
   *     Note that the returned object may not be the same one referenced by newValue because
   *     variable traces may modify the variable's value. The value of the given variable is set. If
   *     either the array or the entry didn't exist then a new variable is created.
   *     <p>The reference count is decremented for any old value of the variable and incremented for
   *     its new value. If the new value for the variable is not the same one referenced by newValue
   *     (perhaps as a result of a variable trace), then newValue's ref count is left unchanged by
   *     Tcl_SetVar2Ex. newValue's ref count is also left unchanged if we are appending it as a
   *     string value: that is, if "flags" includes TCL.APPEND_VALUE but not TCL.LIST_ELEMENT.
   *     <p>The reference count for the returned object is _not_ incremented: if you want to keep a
  ...
  public static TclObject initVarCompiledLocalScalar(
      final Interp interp, // interp
      // to
      // search
      // for
      // the
  ...
  public static TclObject setVarCompiledLocalScalarInvalid(
  ...
  public static TclObject getVarCompiledLocalScalarInvalid(
      Interp interp, // interp
      // to
      // search
      // for
  ...
  public static TclObject initVarCompiledLocalArray(
      final Interp interp, // interp
      // to
      // search
      // for
  ...
  public static TclObject setVarCompiledLocalArrayInvalid(
      Interp interp, // interp
      // to
      // search
      // for
  ...
  public static TclObject getVarCompiledLocalArrayInvalid(
      Interp interp, // interp
      // to
      // search
      // for
  ...
  public static TclObject getVarCompiledLocalArray(
      final Interp interp,
    ...
    Var[] result =
        Var.lookupArrayElement(interp, varname, key, flags, "read", false, false, resolved);
    if (result == null) {
      return null;
    }
  ...
  public static TclObject setVarCompiledLocalArray(
      final Interp interp,
      final String varname, // name of array variable
      final String key, // array key, can't be null
      final TclObject newValue,
      final Var resolved) // resolved array
      // variable
      throws TclException {
   ...
   *
   * <p>Given a two-part variable name, which may refer either to a scalar variable or an element of
   * an array, increment the Tcl object value of the variable by a specified amount.
   *
   * @param part1 1st part of the variable name.
   * @param part2 2nd part of the variable name.
   * @param incrAmount Amount to be added to variable.
   * @param flags misc flags that control the actions of this method
   *     <p>Results: Returns a reference to the TclObject holding the new value of the variable. If
   *     the specified variable doesn't exist, or there is a clash in array usage, or an error
   *     occurs while executing variable traces, then a TclException will be raised.
   *     <p>Side effects: The value of the given variable is incremented by the specified amount. If
   *     either the array or the entry didn't exist then a new variable is created. The ref count
   *     for the returned object is _not_ incremented to reflect the returned reference; if you want
   *     to keep a reference to the object you must increment its ref count yourself.
   *     <p>---------------------------------------------------------------- ------
   */
  public static TclObject incrVar(
      Interp interp, // Command interpreter in
      // which variable is
      // to be found.
   ...
   *
   * <p>Unset a variable, given a two-part name consisting of array name and element within array.
   *
   * @param part1 1st part of the variable name.
   * @param part2 2nd part of the variable name.
   * @param flags misc flags that control the actions of this method.
   *     <p>If part1 and part2 indicate a local or global variable in interp, it is deleted. If
   *     part1 is an array name and part2 is null, then the whole array is deleted.
   */
   ...
   *
   * <p>Trace a variable, given a two-part name consisting of array name and element within array.
   *
   * @param part1 1st part of the variable name.
   * @param part2 2nd part of the variable name. null means part1 is a scalar or whole array.
   * @param flags misc flags that control the actions of this method. OR-ed collection of bits,
   *     including any of TCL.TRACE_READS, TCL.TRACE_WRITES, TCL.TRACE_UNSETS, TCL.GLOBAL_ONLY, //
   *     and TCL.NAMESPACE_ONLY.
   * @param proc the trace to comand to add.
   */
  public static void traceVar(Interp interp, String part1, String part2, int flags, VarTrace proc)
      throws TclException {
    Var[] result;
    Var var;
   ...
   *
   * <p>Untrace a variable, given a two-part name consisting of array name and element within array.
   * This will Remove a previously-created trace for a variable.
   *
   ...
   * MakeUpvar -> makeUpvar
   *
   * <p>Create a reference of a variable in otherFrame in the current CallFrame, given a two-part
   * name consisting of array name and element within array.
   *
   * @param interp Interp containing the variables
   * @param frame CallFrame containing "other" variable. null means use global context.
   ...
   * @param myFlags only the TCL.GLOBAL_ONLY bit matters, indicating the scope of myName.
  ...
  public static void makeUpvar(
      ...
      String otherP2,
      int otherFlags, // 0, TCL.GLOBAL_ONLY or
      // TCL.NAMESPACE_ONLY:
  ...
  public static String getVariableFullName(
   ...
   *
   * <p>This procedure is invoked to find and invoke relevant trace procedures associated with a
   * particular operation on a variable. This procedure invokes traces both on the variable and on
   * its containing array (where relevant).
   *
   * @param interp Interpreter containing variable.
   * @param array array variable that contains the variable, or null if the variable isn't an
   *     element of an array.
   ...
   * @param flags Flags to pass to trace procedures: indicates what's happening to variable, plus
   *     other stuff like TCL.GLOBAL_ONLY, TCL.NAMESPACE_ONLY, and TCL.INTERP_DESTROYED.
   * @return null if no trace procedures were invoked, or if all the invoked trace procedures
   *     returned successfully. The return value is non-null if a trace procedure returned an error
   *     (in this case no more trace procedures were invoked after the error was returned). In this
   *     case the return value is a pointer to a string describing the error.
   */
  public static String callTraces(
      Interp interp, Var array, Var var, String part1, String part2, int flags) {
    TclObject oldResult;
    int i;
   ...
   *
   * <p>This procedure is called to free up all of the searches associated with an array variable.
   *
   * @param arrayVar the array variable to delete searches from.
   */
  protected static void deleteSearches(Var arrayVar) // Variable whose
        // searches are to be
        // deleted.
      {
   ...
   *
   * <p>This procedure is called to recycle all the storage space associated with a table of
   * variables. For this procedure to work correctly, it must not be possible for any of the
   * variables in the table to be accessed from Tcl commands (e.g. from trace procedures).
   *
  ...
  /**
   * // FIXME: Make more like TclDeleteCompiledLocalVars() TclDeleteVars -> deleteVars
   *
   * <p>This procedure is called to recycle all the storage space associated with an array of
   * variables. For this procedure to work correctly, it must not be possible for any of the
   * variables in the array to be accessed from Tcl commands (e.g. from trace procedures).
   *
   * @param interp Interpreter containing array.
   ...
   * deleteVar
   *
   * <p>This procedure is called to recycle all the storage space associated with a single Var
   * instance.
   *
   * @param interp Interpreter containing array.
   ...
   *
   * <p>This procedure is called to free up everything in an array variable. It's the caller's
   * responsibility to make sure that the array is no longer accessible before this procedure is
   * called.
   *
   * @param interp Interpreter containing array.
   ...
   */
  protected static void deleteArray(
      Interp interp, // Interpreter containing
      // array.
   ...
   *
   * <p>This procedure is called when it looks like it may be OK to free up the variable's record
   * and hash table entry, and those of its containing parent. It's called, for example, when a
   * trace on a variable deletes the variable.
   *
   * @param var variable that may be a candidate for being expunged.
   * @param array Array that contains the variable, or NULL if this variable isn't an array element.
   */
  ...
  public static void AppendLocals(
