...
public class LappendCmd implements Command {
   ...
   *
   * <p>This procedure is invoked to process the "lappend" Tcl command. See the user documentation
   ...
   * @throws TclException
   */
  public static TclObject lappendVar(
      Interp interp,
      String varName, // name of
      // variable
    ...
    try {
      ...
      if (Var.isArrayVarname(varName)) {
        ...
        Var[] result =
            Var.lookupVar(interp, varName.substring(0, part1End), null, 0, "read", false, false);
        if (result != null && result[0].isVarUndefined()) {
          Var.callTraces(
              interp,
              null,
              result[0],
              varName.substring(0, part1End),
              varName.substring(part1End + 1, varName.length() - 1),
              TCL.TRACE_READS);
        }
        createVar = false;
      }
