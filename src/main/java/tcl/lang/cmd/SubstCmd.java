...
public class SubstCmd implements Command {
  ...
  /**
   * This method is invoked to process the "subst" Tcl command. See the user documentation for
  ...
  public void cmdProc(Interp interp, TclObject argv[]) throws TclException {
    ...
    if (objc == 0) {
      throw new TclNumArgsException(
          interp, 1, argv, "?-nobackslashes? ?-nocommands? ?-novariables? string");
    }
  ...
  /**
   * Catch any break, continue or return exception and stuff the appropriate substitution string
   * into result
   *
   * @param e The TclException that occurred
   * @param originalString The complete original string passed to subst
   * @param offset The offset of the '[' or '$' being substituted
   * @param replacementSubstring The value of the replacement
   * @param result The StringBuffer containing substitutions that will be returned from subst
   * @return Index of next character in 'originalString' to process
   * @throws TclException If the exception was an error, rather than a break, continue or return
   */
  private static int doCmdOrVarSub(
      TclException e,
