...
public class SocketCmd implements Command {
  ...
  /**
   * This procedure is invoked to process the "socket" Tcl command. See the user documentation for
  ...
  public void cmdProc(Interp interp, TclObject argv[]) throws TclException {
    ...
    if (server) {
      ...
      try {
        SocketChannel sock =
            new SocketChannel(interp, TclIO.RDWR, myaddr, myport, async, host, port);
  ...
  /**
   * A unique error msg is printed for socket. Call this methods instead of the standard
   * TclException.wrongNumArgs().
   *
   * @param interp the current interpreter.
   ...
   *
   * <p>Maps from a string, which could be a service name, to a port. Unfortunately, Java does not
   * provide any way to access the getservbyname functionality so we are limited to port numbers.
   */
  private static int getPort(Interp interp, TclObject tobj) throws TclException {
    long num = Util.getInt(interp, tobj.toString());
    if (num > 0xFFFFL) throw new TclException(interp, "couldn't open socket: port number too high");
    return (int) num;
  }
}
