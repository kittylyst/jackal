...
public final class AfterCmd implements Command {
  ...
  public void cmdProc(
    ...
    try {
      ...
      throw new TclException(
          interp, "bad argument \"" + argv[1] + "\": must be cancel, idle, info, or a number");
    }
    ...
    switch (index) {
      ...
      case OPT_INFO:
        ...
        TclList.append(
            interp,
            list,
            ((info instanceof TimerInfo) ? ((TimerInfo) info).command : ((IdleInfo) info).command));
        TclList.append(
            interp, list, TclString.newInstance((info instanceof TimerInfo) ? "timer" : "idle"));
  ...
  /**
   * This procedure parses an "after" id such as "after#4" and returns an AfterInfo object.
   *
   * @param interp current interpreter
   * @param string text identifier for event, such as "after#6"
   * @return an AfterInfo object. if one is found that corresponds to "string", or null if no
  ...
  /**
   * This inner class manages the list of handlers created by the "after" command. We keep the
  ...
  class AfterAssocData implements AssocData {
    ...
    /**
     * This method is called when the interpreter is destroyed or when Interp.deleteAssocData is
     * called on a registered AssocData instance.
     *
     * <p>Results: None.
     *
     * <p>Side effects: All unfired handler are cancelled; their command objects are released.
     *
