 ...
 * cmdProc implements the built-in Tcl command "trace" which is used to manipulate variable traces.
...
public final class TraceCmd implements Command {
  ...
  public void cmdProc(Interp interp, TclObject[] objv) throws TclException {
    ...
    switch (opt) {
      ...
      case OPT_INFO:
        ...
        if (opt == OPT_INFO) {
          if (objv.length != 4) throw new TclNumArgsException(interp, 3, objv, "name");
        } else {
          if (objv.length != 6)
    ...
    if ((opt == OPT_ADD || opt == OPT_REMOVE)) {
      ...
      if (ops.length == 0) {
        throw new TclException(
            interp,
            "bad operation list \"\": must be one or more of enter, leave, enterstep, or leavestep");
      }
      boolean enter = false;
      boolean leave = false;
  ...
  private void traceCommand(Interp interp, int opt, TclObject[] objv) throws TclException {
    ...
    if ((opt == OPT_ADD || opt == OPT_REMOVE)) {
      ...
      if (ops.length == 0) {
        throw new TclException(
            interp, "bad operation list \"\": must be one or more of delete or rename");
      }
   ...
   *
   * @param interp current interpreter
   * @param opt option index (OPT_ADD, OPT_REMOVE, etc.)
   * @param objv argument list
   * @param start the index of the argument list to start parsing sub-options
   * @throws TclException
   */
  private void traceVariable(Interp interp, int opt, TclObject[] objv, int start)
      throws TclException {
    int len;
    ...
    switch (opt) {
      ...
      case OPT_REMOVE:
        ...
        if (start == 3) {
          ...
          if (flags == 0) {
            throw new TclException(
                interp, "bad operations \"" + objv[3] + "\": should be one or more of rwua");
          }
        }
          ...
          if (traces != null) {
            ...
            for (int i = 0; i < len; i++) {
              ...
              if (rec.trace instanceof VarTraceProc) {
                ...
                if (proc.flags == flags
                    && proc.command.toString().equals(objv[start + 2].toString())) {
