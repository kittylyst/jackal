  ...
  public void cmdProc(Interp interp, TclObject argv[]) throws TclException {
    ...
    try {
      ...
      if (readAll) {
        ...
        chan.read(interp, result, TclIO.READ_N_BYTES, toRead);
        interp.setResult(result);
      }

    } catch (IOException e) {
      throw new TclRuntimeError(
          "ReadCmd.cmdProc() Error: IOException when reading " + chan.getChanName());
    }
  }

  /**
   * A unique error msg is printed for read, therefore dont call this instead of the standard
   * TclNumArgsException().
   *
   * @param interp the current interpreter.
