...
public final class LsetCmd implements Command {
   ...
   * flat (TclLsetFlat) --
   *
   * <p>Core of the 'lset' command when objc>=5. Objv[2], ... , objv[objc-2] contain scalar indices.
   *
   * <p>Side effects: Surgery is performed on the list value to produce the result.
   *
   * <p>On entry, the reference count of the variable value does not reflect any references held on
   * the stack. The first action of this function is to determine whether the object is shared, and
   * to duplicate it if it is. The reference count of the duplicate is incremented. At this point,
   * the reference count will be 1 for either case, so that the object will appear to be unshared.
   *
   * <p>If an error occurs, and the object has been duplicated, the reference count on the duplicate
   * is decremented so that it is now 0: this dismisses any memory that was allocated by this
   * procedure.
   *
   * <p>If no error occurs, the reference count of the original object is incremented if the object
   * has not been duplicated, and nothing is done to a reference count of the duplicate. Now the
   * reference count of an unduplicated object is 2 (the returned object, plus the one stored in the
   * variable). The reference count of a duplicate object is 1, reflecting that the returned object
   * is the only active reference. The caller is expected to store the returned value back in the
   * variable and decrement its reference count.
   *
   * <p>list (Tcl_LsetList) and related methods maintain a linked list of TclObject's whose string
   * representations must be spoilt by threading via 'ptr2' On entry to list (Tcl_LsetList), the
   * values of 'ptr2' are immaterial; on exit, the 'ptr2' is set to null.
   *
   * @param interp Tcl interpreter
   * @param list The list being modified
   * @param indexCount Number of index args
   * @param indexArray Index args
   * @param value Value arg to 'lset'
   * @return Returns the new value of the list variable, or NULL if an error occurs.
   * @see Tcl 8.4.19
   * @author rszulgo
   * @since 2009-07-07
   */
  private static final TclObject flat(
      Interp interp, TclObject list, int indexCount, TclObject[] indexArray, TclObject value)
      throws TclException {
    ...
    ArrayList chainList =
        new ArrayList(); // check track of sub-lists, in order to invalidate string reps
   ...
   * list TclLsetList --
   *
   * <p>Core of the 'lset' command when objc == 4. Objv[2] may be either a scalar index or a list of
   * indices.
   *
   * <p>Side effects: Surgery is performed on the list value to produce the result.
   *
   ...
   * variable and decrement its reference count.
   *
   * <p>flat method and related methods maintain a linked list of TclObject's whose string
   * representations must be spoilt by threading via 'ptr2'. On entry to list method, the values of
   * 'ptr2' are immaterial; on exit, the 'ptr2' is set to null.
   *
   * @param interp Tcl interpreter
   * @param list the list being modified
   * @param indexArg Index or index-list arg to 'lset'
   * @param value Value arg to 'lset'
   * @return Returns the new value of the list variable, or NULL if an error occurs.
   * @see Tcl 8.4.19
   * @author rszulgo
   * @since 2009-07-07
   */
  private static final TclObject list(
      Interp interp, TclObject list, TclObject indexArg, TclObject value) throws TclException {
