...
public class FormatCmd implements Command {
  ...
  /**
   * This procedure is invoked to process the "format" Tcl command. See the user documentation for
   * details on what it does.
   *
   * <p>The first argument to the cmdProc is the formatString. The cmdProc simply copies all the
   * chars into the sbuf until a '%' is found. At this point the cmdProc parces the formatString and
   * determines the format parameters. The parcing of the formatString can be broken into six
   * possible phases:
   *
   * <p>Phase 0 - Simply Print: If the next char is % Phase 1 - XPG3 Position Specifier: If the
   * format [1-n]$ is used Phase 2 - A Set of Flags: One or more of the following + - [space] 0 #
   * Phase 3 - A Minimun Field Width Either [integer] or * Phase 4 - A Precision If the format
   * .[integer] or .* Phase 5 - A Length Modifier If h is present Phase 6 - A Conversion Character
   * If one of the following is used d u i o x X c s f E g G
   *
   * <p>Any phase can skip ahead one or more phases, but are not allowed to move back to previous
   * phases. Once the parameters are determined the cmdProc calls one of three private methods that
   * returns a fully formatted string. This loop occurs for ever '%' in the formatString.
   */
        ...
        case 'i':
          {
            ...
            if (index == 'o') {
              sbuf.append(
                  cvtLngToStr(
                      lngValue, width, precision, fmtFlags, 8, "01234567".toCharArray(), "0"));
            } else if (index == 'x') {
              sbuf.append(
                  cvtLngToStr(
                      lngValue,
              ...
              sbuf.append(
                  cvtLngToStr(
                      lngValue, width, precision, fmtFlags, 10, "0123456789".toCharArray(), ""));
            }
            break;
          }
        case 'c':
        ...
        case 'e':
          {
            dblValue = TclDouble.get(interp, argv[argIndex]);
            sbuf.append(
                cvtDblToStr(dblValue, width, precision, fmtFlags, 10, "e".toCharArray(), "", EXP));
            break;
          }
        case 'E':
          {
        ...
        case 'g':
          {
            ...
            sbuf.append(
                cvtDblToStr(
                    dblValue, width, precision, fmtFlags, 10, "e".toCharArray(), "", GENERIC));
            break;
          }
        case 'G':
          {
  ...
  /**
   * This procedure is invoked in "phase 6" od the Format cmdProc. It converts the lngValue to a
   * string with a specified format determined by the other input variables.
   *
   * @param lngValue - Is the value of the argument input
   * @param width - The minimum width of the string.
   * @param precision - The minimum width if the integer. If the string len is less than precision,
   *     leading 0 are appended.
   * @param flags - Specifies various formatting to the string representation (-, +, space, 0, #)
   * @param base - The base of the integer (8, 10, 16)
   * @param charSet - The char set to use for the conversion to ascii OR The char used for sci
   *     notation.
   * @param altPrefix - If not empty, str to append on the beginnig of the resulting string (eg 0 or
   *     0x or 0X ).
   * @return String representation of the long.
   */
  private String cvtLngToStr(
      long lngValue,
      int width,
      int precision,
      int flags,
    ...
    if ((flags & ALT_OUTPUT) != 0) {
      if ((altPrefix.length() > 0) && (sbuf.charAt(0) != altPrefix.charAt(0))) {
  ...
  public static String toString(double dblValue, int precision, int base) {
    return cvtDblToStr(dblValue, 0, precision, 0, base, "e".toCharArray(), null, GENERIC);
  }
   ...
   * @param altPrefix - If not empty, str to append on the beginnig of the resulting string (eg 0 or
   *     0x or 0X ).
   * @param xtype - Either FLOAT, EXP, or GENERIC depending on the format specifier.
   * @return String representation of the long.
   */
  private static String cvtDblToStr(
      double dblValue,
      int width,
      int precision,
      int flags,
    ...
    if (xtype == FLOAT) {
      ...
      if (flag_rtz && flag_dp) {
        ...
        int len, index = 0;
        for (len = sbuf.length() - 1; (len >= 0) && (sbuf.charAt(len) == '0'); len--, index++) {}
  ...
  /**
   * This procedure is invoked in "phase 6" od the Format cmdProc. It converts the strValue to a
   * string with a specified format determined by the other input variables.
   *
  ...
  /**
   * Search through the array while the current char is a digit. When end of array occurs or the
   * char is not a digit, stop the loop, convert the sub-array into a long. At this point update a
   * StrtoulResult object that contains the new long value and the current pointer to the array.
  ...
  /**
   * Called whenever the fmtIndex in the cmdProc is changed. It verifies the the array index is
   * still within the bounds of the array. If no throw error.
   *
  ...
  /**
   * Called whenever Sequential format specifiers are interlaced with XPG format specifiers in one
   * call to cmdProc.
   *
   * @param interp - The TclInterp which called the cmdProc method .
   */
  ...
  /**
   * Called whenever the argIndex access outside the argv array. If the type is an XPG then the
   * error message is different.
   *
   * @param interp - The TclInterp which called the cmdProc method .
   * @param gotXpg - Boolean the determines if the current format is of a XPG type or Sequential
   */
