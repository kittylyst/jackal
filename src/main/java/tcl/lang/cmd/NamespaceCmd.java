...
public final class NamespaceCmd implements InternalRep, Command {
  ...
  /**
   * Invoked to implement the "namespace children" command that returns a list containing the
   * fully-qualified names of the child namespaces of a given namespace. Handles the following
   * syntax:
   *
   * <p>namespace children ?name? ?pattern?
   *
   * <p>Results: Nothing.
   *
    ...
    if (objv.length == 2) {
      ...
      if (ns == null) {
        throw new TclException(
            interp,
            "unknown namespace \"" + objv[2].toString() + "\" in namespace children command");
      }
    } else {
   ...
   *
   * <p>Invoked to implement the "namespace code" command to capture the namespace context of a
   * command. Handles the following syntax:
   *
   * <p>namespace code arg
   *
   * <p>Here "arg" can be a list. "namespace code arg" produces a result equivalent to that produced
   * by the command
   *
   * <p>list namespace inscope [namespace current] $arg
   *
   * <p>However, if "arg" is itself a scoped value starting with "namespace inscope", then the
   * result is just "arg".
   *
   * <p>Results: Nothing.
   *
   * <p>Side effects: If anything goes wrong, this procedure returns an error message as the result
  ...
  /**
   * Invoked to implement the "namespace current" command which returns the fully-qualified name of
   * the current namespace. Handles the following syntax:
   *
   * <p>namespace current
   *
  ...
  /**
   * Invoked to implement the "namespace delete" command to delete namespace(s). Handles the
   * following syntax:
   *
   * <p>namespace delete ?name name...?
   *
   * <p>Each name identifies a namespace. It may include a sequence of namespace qualifiers
   * separated by "::"s. If a namespace is found, it is deleted: all variables and procedures
   * contained in that namespace are deleted. If that namespace is being used on the call stack, it
   * is kept alive (but logically deleted) until it is removed from the call stack: that is, it can
   * no longer be referenced by name but any currently executing procedure that refers to it is
   * allowed to do so until the procedure returns. If the namespace can't be found, this procedure
   * returns an error. If no namespaces are specified, this command does nothing.
   *
   * <p>Results: Returns if successful, raises TclException if something goes wrong.
   *
   * <p>Side effects: Deletes the specified namespaces. If anything goes wrong, this procedure
   * returns an error message in the interpreter's result object.
   */
  private static void deleteCmd(Interp interp, TclObject[] objv) throws TclException {
    ...
    for (i = 2; i < objv.length; i++) {
      ...
      if (namespace == null) {
        throw new TclException(
            interp, "unknown namespace \"" + objv[i].toString() + "\" in namespace delete command");
      }
    }
  ...
  /**
   * Invoked to implement the "namespace eval" command. Executes commands in a namespace. If the
   * namespace does not already exist, it is created. Handles the following syntax:
   *
   * <p>namespace eval name arg ?arg...?
   *
   * <p>If more than one arg argument is specified, the command that is executed is the result of
   * concatenating the arguments together with a space between each argument.
   *
   * <p>Results: Returns if successful, raises TclException if something goes wrong.
   *
   * <p>Side effects: Returns the result of the command in the interpreter's result object. If
   * anything goes wrong, this procedure returns an error message as the result.
   */
   ...
   *
   * <p>Invoked to implement the "namespace exists" command that returns true if the given namespace
   * currently exists, and false otherwise. Handles the following syntax:
   *
   * <p>namespace exists name
   *
   * <p>Results: Returns if successful, raises TclException if something goes wrong.
   *
   * <p>Side effects: Returns a result in the interpreter's result object. If anything goes wrong,
  ...
  /**
   * Invoked to implement the "namespace export" command that specifies which commands are exported
   * from a namespace. The exported commands are those that can be imported into another namespace
   * using "namespace import". Both commands defined in a namespace and commands the namespace has
   * imported can be exported by a namespace. This command has the following syntax:
   *
   * <p>namespace export ?-clear? ?pattern pattern...?
   *
   * <p>Each pattern may contain "string match"-style pattern matching special characters, but the
   * pattern may not include any namespace qualifiers: that is, the pattern must specify commands in
   * the current (exporting) namespace. The specified patterns are appended onto the namespace's
   * list of export patterns.
   *
   * <p>To reset the namespace's export pattern list, specify the "-clear" flag.
   *
   * <p>If there are no export patterns and the "-clear" flag isn't given, this command returns the
  ...
  /**
   * Invoked to implement the "namespace forget" command to remove imported commands from a
   * namespace. Handles the following syntax:
   *
   * <p>namespace forget ?pattern pattern...?
   *
   * <p>Each pattern is a name like "foo::*" or "a::b::x*". That is, the pattern may include the
   * special pattern matching characters recognized by the "string match" command, but only in the
   * command name at the end of the qualified name; the special pattern characters may not appear in
   * a namespace name. All of the commands that match that pattern are checked to see if they have
   * an imported command in the current namespace that refers to the matched command. If there is an
   * alias, it is removed.
   *
   * <p>Results: Returns if successful, raises TclException if something goes wrong.
   *
   * <p>Side effects: Imported commands are removed from the current namespace. If anything goes
   * wrong, this procedure returns an error message in the interpreter's result object.
   */
  ...
  private static void inscopeCmd(Interp interp, TclObject[] objv) throws TclException {
    ...
    if (namespace == null) {
      throw new TclException(
          interp, "unknown namespace \"" + objv[2].toString() + "\" in inscope namespace command");
    }
  ...
  /**
   * Invoked to implement the "namespace origin" command to return the fully-qualified name of the
   * "real" command to which the specified "imported command" refers. Handles the following syntax:
   *
   * <p>namespace origin name
   *
   * <p>Results: An imported command is created in an namespace when that namespace imports a
   * command from another namespace. If a command is imported into a sequence of namespaces a,
   * b,...,n where each successive namespace just imports the command from the previous namespace,
   * this command returns the fully-qualified name of the original command in the first namespace,
   * a. If "name" does not refer to an alias, its fully-qualified name is returned. The returned
   * name is stored in the interpreter's result object. This procedure returns TCL_OK if successful,
   * and TCL_ERROR if anything goes wrong.
   *
   * <p>Side effects: If anything goes wrong, this procedure returns an error message in the
   * interpreter's result object.
   */
  ...
  private static void parentCmd(Interp interp, TclObject[] objv) throws TclException {
    ...
    if (objv.length == 2) {
      ...
      if (ns == null) {
        throw new TclException(
            interp, "unknown namespace \"" + objv[2].toString() + "\" in namespace parent command");
      }
    } else {
      throw new TclNumArgsException(interp, 2, objv, "?name?");
    }
  ...
  public void dispose() {
    ...
    if (debug) {
      System.out.println(
          "dispose() called for namespace object " + (otherValue == null ? null : otherValue.ns));
    }
  ...
  public InternalRep duplicate() {
    ...
    if (debug) {
      System.out.println(
          "duplicate() called for namespace object " + (otherValue == null ? null : otherValue.ns));
    }
  ...
  static void setNsNameFromAny(
      Interp interp, // Reference to the namespace in
      // which to
  ...
  public String toString() {
    ...
    if (debug) {
      System.out.println(
          "toString() called for namespace object " + (otherValue == null ? null : otherValue.ns));
    }
