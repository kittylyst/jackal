 ...
 * built-in "interp create" command in Tcl.
 *
 * <p>It is also used by the "interp" command to record and find information about slave
...
public class InterpSlaveCmd implements CommandWithDispose, AssocData {
  ...
  /**
   * Command to manipulate an interpreter, e.g. to send commands to it to be evaluated. One such
   * command exists for each slave interpreter.
   *
   * <p>Results: A standard Tcl results
   */
  public void cmdProc(
   ...
   * <p>disposeCmd --
   *
   * <p>Invoked when an object command for a slave interpreter is deleted; cleans up all state
   * associated with the slave interpreter and destroys the slave interpreter.
   *
   * <p>Results: None.
   *
   * <p>Side effects: Cleans up all state associated with the slave interpreter and destroys the
   * slave interpreter.
   *
   * <p>----------------------------------------------------------------------
   */
  public void disposeCmd() {
    ...
    if (slaveInterp != null) {
      if (debug) {
        System.out.println("slaveInterp with path \"" + path + "\" disposed of " + slaveInterp);
      }
      slaveInterp.dispose();
    } else {
      if (debug) {
  ...
  /**
   * Invoked when an interpreter is being deleted. It releases all storage used by the
   * master/slave/safe interpreter facilities.
   *
   * <p>Results: None.
   *
   * <p>Side effects: Cleans up storage. Sets the interpInfoPtr field of the interp to NULL.
   */
  ...
  /**
   * Helper function to do the actual work of creating a slave interp and new object command. Also
   * optionally makes the new slave interpreter "safe".
   *
   * <p>Results: Returns the new Tcl_Interp * if successful or NULL if not. If failed, the result of
   * the invoking interpreter contains an error message.
   *
   * <p>Side effects: Creates a new slave interpreter and a new object command.
   *
  ...
  static Interp create(
    ...
    if (debug) {
      System.out.println(
          "slaveTable entry for \"" + pathString + "\" created (" + slaveInterp + ")");
    }
   ...
   *
   * <p>Side effects: After this call scripts in the slave will be able to invoke the newly exposed
   * command.
   *
   * @param interp interpreter for error return
   * @param slaveInterp slave interpreter in which command will be evaluated
   ...
   * <p>Results: A standard Tcl result.
   *
   * <p>Side effects: After this call scripts in the slave will no longer be able to invoke the
   * named command.
   *
   * @param interp interpreter for error return
   * @param slaveInterp slave interpreter in which command will be evaluated
   ...
   * ----------------------------------------------------------------------
   *
   * <p>SlaveInvokeHidden -> invokeHidden
   *
   * <p>Helper function to invoke a hidden command in a slave interpreter.
   *
   * <p>Results: A standard Tcl result.
   *
   * <p>Side effects: Whatever the hidden command does.
   *
   * <p>----------------------------------------------------------------------
   */
  static void invokeHidden(
      Interp interp, // Interp for error return.
      Interp slaveInterp, // The slave interpreter in which command
      // will be invoked.
   ...
   * ----------------------------------------------------------------------
   *
   * <p>SlaveMarkTrusted -> markTrusted
   *
   * <p>Helper function to mark a slave interpreter as trusted (unsafe).
   *
   * <p>Results: A standard Tcl result.
   *
   * <p>Side effects: After this call the hard-wired security checks in the core no longer prevent
   * the slave from performing certain operations.
   *
   * <p>----------------------------------------------------------------------
   */
  static void markTrusted(
      Interp interp, // Interp for error return.
   ...
   * <p>Results: A standard Tcl result.
   *
   * <p>Side effects: When (objc == 1), slaveInterp will be set to a new recursion limit of objv[0].
   *
   ...
   * <p>makeSafe --
   *
   * <p>Makes its argument interpreter contain only functionality that is defined to be part of Safe
   * Tcl. Unsafe commands are hidden, the env array is unset, and the standard channels are removed.
   *
   * <p>Results: None.
   *
   * <p>Side effects: Hides commands in its argument interpreter, and removes settings and channels.
   *
   * <p>----------------------------------------------------------------------
   */
   ...
   * ----------------------------------------------------------------------
   *
   * <p>Tcl_GetSlave, GetInterp -> getSlave
   *
   * <p>Finds a slave interpreter by its path name.
   *
   * <p>Results: Returns a Interp for the named interpreter or raises a TclException if the
  ...
  static Interp getSlave(
    ...
    if (searchInterp == null) {
      throw new TclException(interp, "could not find interpreter \"" + slavePath + "\"");
    }
    return searchInterp;
  }
} // end InterpSlaveCmd
