  ...
  public void cmdProc(Interp interp, TclObject[] objv) throws TclException {
    ...
    if (objv.length == 1) {
      throw new TclException(
          interp, "wrong # args: should be \"variable ?name value...? name ?value?\"");
    }
    for (i = 1; i < objv.length; i = i + 2) {
      ...
      if (varName.endsWith(")") && varName.contains("(")) {
        throw new TclException(
            interp, "can't define \"" + varName + "\": name refers to an element in an array");
      }
      Var[] result =
          Var.lookupVar(
              interp,
      ...
      if ((interp.varFrame != null) && interp.varFrame.isProcCallFrame) {
        ...
        Var.makeUpvar(interp, null, varName, null, TCL.NAMESPACE_ONLY, varTail, 0, -1);
      }
    }
  }
}
