...
public final class FconfigureCmd implements Command {
  ...
  private static final String socketValidCommands[] = {
    "-blocking",
    "-buffering",
    "-buffersize",
    "-encoding",
  ...
  /**
   * This procedure is invoked to process the "fconfigure" Tcl command. See the user documentation
  ...
  public void cmdProc(Interp interp, TclObject argv[]) throws TclException {
    ...
    if ((argv.length < 2) || (((argv.length % 2) == 1) && (argv.length != 3))) {
      throw new TclNumArgsException(
          interp, 1, argv, "channelId ?optionName? ?value? ?optionName value?...");
    }
    ...
    String validCmds[] =
        (chan instanceof AbstractSocketChannel) ? socketValidCommands : commonValidCommands;
    ...
    if (argv.length == 2) {
      ...
      TclList.append(
          interp, list, TclString.newInstance(TclIO.getBufferingString(chan.getBuffering())));
      ...
      if (chan.isReadOnly()) {
        ...
        TclList.append(
            interp,
            list,
            (eofChar == 0) ? TclString.newInstance("") : TclString.newInstance(eofChar));
        ...
        TclList.append(
            interp,
            eofchar_pair,
            (inEofChar == 0) ? TclString.newInstance("") : TclString.newInstance(inEofChar));

        TclList.append(
            interp,
            eofchar_pair,
            (outEofChar == 0) ? TclString.newInstance("") : TclString.newInstance(outEofChar));
        ...
        default:
          {
            throw new TclRuntimeError(
                "Fconfigure.cmdProc() error: " + "incorrect index returned from TclIndex.get()");
          }
      }
    }
      ...
      switch (index) {
        ...
        case OPT_BUFFERING:
          { // -buffering
            ...
            if (id == -1) {
              throw new TclException(
                  interp, "bad value for -buffering: must be " + "one of full, line, or none");
            }
        ...
        case OPT_TRANSLATION:
          { // -translation
            ...
            if (length < 1 || length > 2) {
              throw new TclException(
                  interp, "bad value for -translation: " + "must be a one or two element list");
            }
