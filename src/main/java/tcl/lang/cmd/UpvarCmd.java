...
public final class UpvarCmd implements Command {
   ...
   *
   * <p>This procedure is invoked to process the "upvar" Tcl command. See the user documentation for
  ...
  public void cmdProc(Interp interp, TclObject[] objv) throws TclException {
    ...
    if (objv.length < 3) {
      throw new TclNumArgsException(
          interp, 1, objv, "?level? otherVar localVar ?otherVar localVar ...?");
    }
    ...
    for (; objc > 0; objc -= 2, objv_index += 2) {
      ...
      if ((p != -1) && (otherVarName.charAt(otherLength - 1) == ')')) {
        ...
        Var.makeUpvar(interp, frame, otherVarName, null, 0, myVarName, 0, -1);
      }
    }
    interp.resetResult();
    return;
