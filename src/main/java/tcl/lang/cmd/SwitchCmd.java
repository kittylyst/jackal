...
public final class SwitchCmd implements Command {
    ...
    for (i = 1; i < objv.length; i++) {
      ...
      if (opt == LAST) {
        ...
        throw new TclException(
            interp, "SwitchCmd.cmdProc: bad option " + opt + " index to validCmds");
      } else {
        if (foundmode) {
          throw new TclException(
              interp,
              "bad option \"" + objv[i] + "\": " + validCmds[mode] + " option already found");
        }
        foundmode = true;
        mode = opt;
      }
  ...
  public static int getBodyOffset(
      Interp interp,
      TclObject[] switchObjv, // array
      // that
      // contains
      // pattern/body
      // strings
    ...
    if (switchObjv[slen - 1] != null && switchObjv[slen - 1].toString().equals("-")) {
      interp.resetResult();
      throw new TclException(
          interp, "no body specified for pattern \"" + switchObjv[slen - 2].toString() + "\"");
    }
    ...
    for (int i = pbStart; i < slen; i += 2) {
      ...
      for (body = i + 1; ; body += 2) {
        ...
        if (switchObjv[body] == null || !switchObjv[body].toString().equals("-")) {
          break;
        }
      }
