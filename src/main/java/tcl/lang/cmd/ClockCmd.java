...
public class ClockCmd implements Command {
   ...
   * <p>cmdProc --
   *
   * <p>This procedure is invoked as part of the Command interface to process the "clock" Tcl
      ...
      case CMD_FORMAT:
        {
          if ((objv.length < 3) || (objv.length > 7)) {
            throw new TclNumArgsException(
                interp, 2, objv, "clockval ?-format string? ?-gmt boolean?");
          }
      ...
      case CMD_SCAN:
        {
          if ((objv.length < 3) || (objv.length > 7)) {
            throw new TclNumArgsException(
                interp, 2, objv, "dateString ?-base clockValue? ?-gmt boolean?");
          }
          ...
          if (date == null) {
            throw new TclException(
                interp, "unable to convert date-time string \"" + dateString + "\"");
          }
  ...
  /**
   * Formats a time value based on seconds into a human readable string. Formatted string is
   * returned in the interpreter's result
   *
   * @param interp interpreter into which to put result
   * @param clockVal Time in seconds.
   * @param useGMT true if use GMT, if false use environment's defined timezone
  ...
  private void FormatClock(Interp interp, int clockVal, boolean useGMT, String format)
    ...
    if (format.equals("%Q")) { // Enterprise Stardate.
      ...
      int trekDay =
          (calendar.get(Calendar.DAY_OF_YEAR) * 1000)
              / (calendar.isLeapYear(calendar.get(Calendar.YEAR)) ? 366 : 365);
      int trekHour =
          (calendar.get(Calendar.HOUR_OF_DAY) * 60 + calendar.get(Calendar.MINUTE)) / 144;

      interp.setResult(
          "Stardate " + (trekYear < 10 ? "0" : "") + (trekYear * 1000 + trekDay) + '.' + trekHour);
      return;
    }
        ...
        switch (format.charAt(++ix)) {
          ...
          case 's': // seconds since epoch.
            ...
            if (useGMT) {
              ...
              millis -=
                  localCalendar.get(Calendar.ZONE_OFFSET) + localCalendar.get(Calendar.DST_OFFSET);
            }
   ...
   *
   * @param calendar calendar with given date
   * @return year
   */
  ...
  /**
   * Returns the week_of_year of the given date. The weekday considered as start of the week is
   * given as argument. Specify iso as true to get the week_of_year accourding to ISO.
   *
   * <p>Side effects: The interpreter will contain the formatted string as result.
   *
   * @param calendar calendar containing the date
   * @param firstDayOfWeek this day starts a week
  ...
  /**
   * The date of the given calendar will be incremented, so that it will match the weekday in the
   * diff object. If dayOrdinal is bigger than 1, additional weeks will be added.
   *
   * @param calendar contains date
   * @param diff time difference to evaluate
   */
  ...
  /**
   * The date of the given calendar will be incremented, so that it will match the ordinal month in
   * the diff object.
   *
   * <p>Results: None.
   *
   * <p>Side effects: Modifies the given calendar.
   */
  private void SetOrdMonth(
      Calendar calendar, // Calendar containing Date.
      ClockRelTimespan diff) // time difference to evaluate
      {
   ...
   *
   * <p>Results: The scanned date (or null, if an error occured).
   *
   * <p>Side effects: None.
   */
  private Date GetDate(
    ...
    calendar.set(
        now.get(Calendar.YEAR), now.get(Calendar.MONTH), now.get(Calendar.DAY_OF_MONTH), 0, 0, 0);
    if (useGMT) {
      calendar.setTimeZone(TimeZone.getTimeZone("GMT"));
    }
  ...
  /**
   * Parse a time string and sets the Calendar. A time string is valid, if it confirms to the
   * following yacc rule: time : tUNUMBER tMERIDIAN | tUNUMBER ':' tUNUMBER o_merid | tUNUMBER ':'
   * tUNUMBER '-' tUNUMBER | tUNUMBER ':' tUNUMBER ':' tUNUMBER o_merid | tUNUMBER ':' tUNUMBER ':'
   * tUNUMBER '-' tUNUMBER ;
   *
   * <p>Results: True, if a time was read (parsePos was incremented and calendar was set according
   * to the read time); false otherwise.
   *
   * <p>Side effects: None.
   */
  private boolean ParseTime(
      ClockToken[] dt, // Input as scanned array of
      // tokens
   ...
   * <p>ParseZone --
   *
   * <p>Parse a timezone string and sets the Calendar. A timezone string is valid, if it confirms to
   * the following yacc rule: zone : tZONE tDST | tZONE | tDAYZONE ;
   *
   * <p>Results: True, if a timezone was read (parsePos was incremented and calendar was set
   * according to the read timezone); false otherwise.
   *
   * <p>Side effects: None.
   *
   * <p>------------------------------------------------------------------------ -----
   ...
   * <p>ParseDay --
   *
   * <p>Parse a day string and sets the Calendar. A day string is valid, if it confirms to the
   * following yacc rule: day : tDAY | tDAY ',' | tUNUMBER tDAY | '+' tUNUMBER tDAY | '-' tUNUMBER
   * tDAY | tNEXT tDAY ;
   *
   * <p>Results: True, if a day was read (parsePos was incremented and the time difference was set
   * according to the read day); false otherwise.
   *
   * <p>Side effects: None.
   *
   * <p>------------------------------------------------------------------------ -----
   ...
   * <p>ParseDate --
   *
   * <p>Parse a date string and sets the Calendar. A date string is valid, if it confirms to the
   * following yacc rule: date : tUNUMBER '/' tUNUMBER | tUNUMBER '/' tUNUMBER '/' tUNUMBER |
   * tISOBASE | tUNUMBER '-' tMONTH '-' tUNUMBER | tUNUMBER '-' tUNUMBER '-' tUNUMBER | tMONTH
   * tUNUMBER | tMONTH tUNUMBER ',' tUNUMBER | tUNUMBER tMONTH | tEPOCH | tUNUMBER tMONTH tUNUMBER ;
   *
   * <p>Results: True, if a date was read (parsePos was incremented and calendar was set according
   * to the read day); false otherwise.
   *
   * <p>Side effects: None.
   *
   * <p>------------------------------------------------------------------------ -----
   ...
   * <p>ParseNumber --
   *
   * <p>Parse a number and sets the Calendar. If argument mayBeYear is true, this number is
   * conidered as year, otherwise it is date and time in the form HHMM.
   *
   * <p>Results: True, if a number was read (parsePos was incremented and calendar was set according
   * to the read day); false otherwise.
   *
   * <p>Side effects: None.
   *
   ...
   * <p>ParseRelSpec --
   *
   * <p>Parse a relative time specification and sets the time difference. A relative time
   * specification is valid, if it confirms to the following yacc rule: relspec : relunits tAGO |
   * relunits ;
   *
   * <p>Results: True, if a relative time specification was read (parsePos was incremented and the
   * time difference was set according to the read relative time specification); false otherwise.
   *
   * <p>Side effects: None.
   *
   * <p>------------------------------------------------------------------------ -----
   */
  private boolean ParseRelSpec(
      ClockToken[] dt, // Input as scanned array of
      // tokens
      ParsePosition parsePos, // Current position in input
      ClockRelTimespan diff) // time difference to evaluate
   ...
   * <p>ParseRelUnits --
   *
   * <p>Parse a relative time unit and sets the time difference. A relative time unit is valid, if
   * it confirms to the following yacc rule: relspec : '+' tUNUMBER unit | '-' tUNUMBER unit |
   * tUNUMBER unit | tNEXT unit | tNEXT tUNUMBER unit | unit ;
   *
   * <p>Results: True, if a relative time specification was read (parsePos was incremented and the
   * time difference was set according to the read relative time specification); false otherwise.
   *
   ...
   * <p>ParseOrdMonth --
   *
   * <p>Parse a relative month and sets the date difference. A relative month is valid, if it
   * confirms to the following yacc rule: ordMonth: tNEXT tMONTH | tNEXT tUNUMBER tMONTH ;
   *
   * <p>Results: True, if a relative month was read (parsePos was incremented and the time
   * difference was set according to the read relative time unit); false otherwise.
   *
   * <p>Side effects: None.
   *
   * <p>------------------------------------------------------------------------ -----
   */
  private boolean ParseOrdMonth(
      ClockToken[] dt, // Input as scanned array of
      // tokens
      ParsePosition parsePos, // Current position in input
      ClockRelTimespan diff) // time difference to evaluate
   ...
   * <p>ParseIso --
   *
   * <p>Parse an ISO 8601 point in time and sets the Calendar. An ISO 8601 point in time is valid,
   * if it confirms to the following yacc rule: iso : tISOBASE tZONE tISOBASE | tISOBASE tZONE
   * tUNUMBER ':' tUNUMBER ':' tUNUMBER | tISOBASE tISOBASE ;
   *
   * <p>Results: True, if an ISO 8601 point in time was read (parsePos was incremented and calendar
   * was set according to the read point); false otherwise.
   *
   * <p>Side effects: None.
   *
   * <p>------------------------------------------------------------------------ -----
   */
  private boolean ParseIso(
      ClockToken[] dt, // Input as scanned array of
      // tokens
      ParsePosition parsePos, // Current position in input
      Calendar calendar) // calendar object to set
   ...
   * <p>ParseTrek --
   *
   * <p>Parse a Stardate and sets the Calendar. A Stardate is valid, if it confirms to the following
   * yacc rule: iso : tSTARDATE tUNUMBER '.' tUNUMBER ;
   *
   * <p>Results: True, if a Stardate was read (parsePos was incremented and calendar was set
   * according to the read point); false otherwise.
   *
   * <p>Side effects: None.
   *
   * <p>------------------------------------------------------------------------ -----
   */
  private boolean ParseTrek(
      ClockToken[] dt, // Input as scanned array of
      // tokens
      ParsePosition parsePos, // Current position in input
    ...
    if (pos + 3 < dt.length
      ...
      int trekDay =
          1 + ((dt[pos + 1].getInt() % 1000) * (calendar.isLeapYear(trekYear) ? 366 : 365)) / 1000;
   ...
   * ------------------------------------------------------------------------ -----
   *
   * <p>ParseMeridianAndSetHour --
   *
   * <p>Parse a meridian and sets the hour field of the calendar. A meridian is valid, if it
   * confirms to the following yacc rule: o_merid : // NULL | tMERIDIAN ;
   *
   * <p>Results: None; parsePos was incremented and the claendar was set according to the read
   * meridian.
   *
   * <p>Side effects: None.
   *
   * <p>------------------------------------------------------------------------ -----
   */
  private void ParseMeridianAndSetHour(
      ClockToken[] dt, // Input as scanned
      // array of tokens
      ParsePosition parsePos, // Current position in input
      Calendar calendar, // calendar object to set
   ...
   * <p>GetTokens --
   *
   * <p>Lexical analysis of the input string.
   *
   * <p>Results: An array of ClockToken, representing the input string.
   *
   * <p>Side effects: None.
   *
   * <p>------------------------------------------------------------------------ -----
   */
  private ClockToken[] GetTokens(
   ...
   * <p>GetNextToken --
   *
   * <p>Lexical analysis of the next token of input string.
   *
   * <p>Results: A ClockToken representing the next token of the input string, (parsePos was
   * incremented accordingly), if one was found. null otherwise (e.g. at end of input).
   *
   * <p>Side effects: None.
   *
   * <p>------------------------------------------------------------------------ -----
   */
  private ClockToken GetNextToken(
      String in, // String to parse
      ParsePosition parsePos) // Current position in input
      {
    int pos = parsePos.getIndex();
   ...
   * <p>LookupWord --
   *
   * <p>Construct a ClockToken for the given word.
   *
   * <p>Results: A ClockToken representing the given word.
   *
   * <p>Side effects: None.
   *
   * <p>------------------------------------------------------------------------ -----
   ...
   * ------------------------------------------------------------------------ -----
   *
   * <p>GetTimeZoneFromRawOffset --
   *
   * <p>Look for a timezone with the given offset (in hours) from gmt.
   *
   * <p>Results: A ClockToken representing the specified timezone.
   *
   * <p>Side effects: None.
   *
   * <p>------------------------------------------------------------------------ -----
 ...
 * <p>CLASS ClockToken --
 *
 * <p>An object of this class represents a lexical unit of the human readable date string. It can be
 * one of the following variants:
 *
 * <p>- unsigned number, = occurence can be asked by isUNumber(), = value can be retrieved by means
 * of getInt(); - iso base date, = occurence can be asked by isIsoBase(), = value can be retrieved
 * by means of getInt(); - a single character (delimiters like ':' or '/'), = occurence can be asked
 * by is(), e.g. is('/'); - a word (like "January" or "DST") = occurence can be asked by is(), e.g.
 * is(ClockToken.AGO); = value can be retrieved by means of getInt() or getZone().
 *
 * <p>-----------------------------------------------------------------------------
 */
class ClockToken {
 ...
 * -----------------------------------------------------------------------------
 *
 * <p>CLASS ClockRelTimespan --
 *
 * <p>An object of this class can be used to track the time difference during the analysis of a
 * relative time specification.
 *
 * <p>It has four read only properties seconds, months, weekday and dayOrdinal, which are set to 0
 * during initialization and which can be modified by means of the addSeconds(), addMonths(),
 * setWeekday() and negate() methods.
 *
 * <p>-----------------------------------------------------------------------------
 */
