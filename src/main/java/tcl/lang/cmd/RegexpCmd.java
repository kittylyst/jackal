...
public final class RegexpCmd implements Command {
  ...
  private static final String validOptsWithXFlags[] = {
    "-all",
    "-about",
    "-indices",
    "-inline",
  ...
  /**
   * Sets this instance of RegexpCmd to interpret and pass on the arguments of -xflags. By default,
   * allowXFlags is false.
   *
  ...
  /**
   * Returns a list for -about when a compile error occur. first element of the list is a
   * subexpression count (0). The second element is a should be a list of property names that
   * describe various attributes of the regular expression. Here we use just "COMPILE_ERROR";
   * reg.test is adjusted accordingly.
   *
  ...
  public void cmdProc(
    ...
    for (i = 1; i < objv.length; i++) {
      ...
      int index =
          TclIndex.get(interp, obj, allowXFlags ? validOptsWithXFlags : validOpts, "switch", 0);
    ...
    if ((objv.length - i) < (2 - (about ? 1 : 0))) {
      throw new TclNumArgsException(
          interp, 1, objv, "?switches? exp string ?matchVar?" + " ?subMatchVar subMatchVar ...?");
    }
    ...
    try {
      if (xflags == null) reg = new Regex(exp, string, offset, flags);
      else reg = new Regex(exp, string, offset, flags, xflags);
    } catch (PatternSyntaxException ex) {
      interp.setErrorCode(
          TclString.newInstance(
              "REGEXP COMPILE_ERROR {" + Regex.getPatternSyntaxMessage(ex) + "}"));
