...
public class EncodingCmd implements Command {
  ...
  /**
   * The java equivalent name (either java.nio or java.io/java.lang) of the value returned for
   * 'encoding system'
   */
  ...
  static EncodingMap[] encodings = {
    new EncodingMap(
        "identity",
        "UTF8",
        1), // this is not really true; convertfrom/convertto handle it differently
    ...
    new EncodingMap(
        "iso2022", "ISO-2022-JP-2", -1), // C Tcl's mapping files looks like ISO2022-JP-2
    new EncodingMap(
        "iso2022-jp", "ISO-2022-JP-2", -1), // C Tcl's mapping file looks like ISO2022-JP-2
  ...
  /**
   * This table maps Tcl's built-in 'symbol' encoding to Unicode characters; it is derived from
   * symbol.enc in the Tcl source It covers the symbols from 0x40 to 0x7A
   */
  ...
  /**
   * This table maps Tcl's built-in 'symbol' encoding to Unicode characters; it is derived from
   * symbol.enc in the Tcl source It covers the symbols from 0xA0 to 0xFF
   */
  ...
  /**
   * This procedure is invoked to process the "encoding" Tcl command. See the user documentation for
  ...
  public void cmdProc(Interp interp, TclObject[] objv) throws TclException {
    ...
    switch (index) {
      ...
      case OPT_CONVERTFROM:
        {
          ...
          try {
            if (index == OPT_CONVERTFROM) {
              ...
              if (tclEncoding.equals("identity")) {
                if (data.isByteArrayType()) interp.setResult(data);
                else {
                  ...
                  ByteArrayOutputStream bos =
                      new ByteArrayOutputStream(data.toString().length() * 3);
                ...
                byte[] bbuf =
                    encodeSymbol(data.toString().toCharArray(), 0, data.toString().length());
            ...
            throw new TclException(
                interp,
                "Encoding.cmdProc() error: "
                    + "unsupported java encoding \""
                    + javaEncoding
                    + "\"");
          } catch (IOException e) {
      ...
      case OPT_SYSTEM:
        {
          if (objv.length > 3) throw new TclNumArgsException(interp, 2, objv, "?encoding?");
      ...
      default:
        {
          throw new TclRuntimeError(
              "Encoding.cmdProc() error: " + "incorrect index returned from TclIndex.get()");
        }
    }
  }
   ...
   *
   * @param name Java name of encoding as returned from getJavaName(
   ...
   *
   * @param name Tcl name for encoding
   * @return java name for encoding, which may be either the new-style java.nio name, or the old
   *     style java.io/java.lang name
   */
  ...
  /**
   * Decode bytes encoded in Tcl's symbol encoding into a character array. We probably should have
   * done this by registering a Java CharSet, but that's a lot of code for this not-often-used
   * encoding.
   *
  ...
  /**
   * Encode chars into Tcl's symbol encoding into a byte array. We probably should have done this by
   * registering a Java CharSet, but that's a lot of code for this not-often-used encoding.
   *
