...
public final class FileCmd implements Command {
  ...
  public void cmdProc(Interp interp, TclObject argv[]) throws TclException {
    ...
    switch (opt) {
      case OPT_ATIME:
        ...
        if (!FileUtil.getNewFileObj(interp, argv[2].toString()).exists()
            || argv[2].toString().length() == 0) {
          throw new TclPosixException(
              interp, TclPosixException.ENOENT, true, "could not read \"" + argv[2] + "\"");
        }
        if (argv.length == 4) {
          try {
        ...
        throw new TclException(
            interp, "sorry, \"file atime\" is not available due to JVM restrictions.");
      ...
      case OPT_ATTRIBUTES:
        ...
        if (argv.length >= 3) {
          ...
          if (fn.length() == 0 || !fileObj.exists()) {
            throw new TclPosixException(
                interp, TclPosixException.ENOENT, true, "could not read \"" + fn + "\"");
          }
        }
        switch (argv.length) {
          case 3:
          ...
          case 5:
            if (argv[3].toString().equals("-JTCL_DOES_NOT_SUPPORT_ATTRIBUTES")) {
              if (argv.length == 4) interp.setResult("FILE_ATTRIBUTES_NOT_SUPPORTED");
              return;
            }
            /* Else, fall through */
          default:
            throw new TclException(
                interp, "Sorry, \"file attributes\" is not available to due JVM restrictions");
        }
      ...
      case OPT_DIRNAME:
        ...
        TclObject splitArrayObj[] =
            TclList.getElements(interp, FileUtil.splitAndTranslate(interp, path));
      ...
      case OPT_EXECUTABLE:
        ...
        if (argv[2].toString().length() == 0) {
          ...
          interp.setResult(
              FileUtil.isExecutable(FileUtil.getNewFileObj(interp, argv[2].toString())));
        }
        return;
      ...
      case OPT_LINK:
        ...
        if ("-symbolic".equals(argv[2].toString()) || "-hard".equals(argv[2].toString())) {
          ...
          throw new TclException(
              interp, "bad switch \"" + argv[2] + "\": must be -symbolic or -hard");
        } else {
        ...
        if (targetName != null) {
          if (link.exists()) {
            throw new TclException(
                interp, "could not create new link \"" + linkName + "\": that path already exists");
          }
          ...
          if (!link.exists() || linkName.length() == 0) {
            throw new TclPosixException(
                interp, TclPosixException.ENOENT, true, "could not read link \"" + linkName + "\"");
          }
          File targetOfLink = FileUtil.getLinkTarget(link);
          if (targetOfLink == null) {
            throw new TclException(
                interp, "could not read link \"" + linkName + "\": invalid argument");
          }
      ...
      case OPT_READLINK:
        ...
        if (!fileObj.exists() || argv[2].toString().length() == 0) {
          throw new TclPosixException(
              interp, TclPosixException.ENOENT, true, "could not readlink \"" + argv[2] + "\"");
        }
        File target = FileUtil.getLinkTarget(fileObj);
        if (target == null) {
          throw new TclException(
              interp, "could not readlink \"" + argv[2] + "\": invalid argument");
        }
      ...
      case OPT_SIZE:
        ...
        if (!fileObj.exists()) {
          throw new TclPosixException(
              ...
              true,
              "could not read \"" + argv[2].toString() + "\"");
        }
      ...
      default:
        throw new TclRuntimeError(
            "file command with opt " + argv[1].toString() + " is not implemented");
    }
  }

  /**
   * If "File" is owned by the uid associated with the program, return true. Otherwise, return
   * false.
   *
   * @param interp
   * @param fileObj
  ...
  private static int getMtime(Interp interp, String fileName, File fileObj) throws TclException {
    ...
    if (!fileObj.exists()) {
      throw new TclPosixException(
          interp, TclPosixException.ENOENT, true, "could not read \"" + fileName + "\"");
    }
  ...
  /**
   * Finds the type of file in fileObj. WARNING: Only checks for file and directory status. If
   * neither file or direcotry, return link. Java only supports file and directory checking.
   *
   * @param interp
   * @param fileName
   * @param fileObj
  ...
  /**
   * This is a utility procedure that breaks out the fields of a "stat" structure and stores them in
   * textual form into the elements of an associative array. WARNING: skipping dev, gid, ino, mode,
   * and nlink attributes. WARNING: ctime and atime are the same as mtime. Java does not support the
   * above attributes.
   *
   * <p>Results: Returns a standard Tcl return value. If an error occurs then a message is left in
   * interp->result.
   *
   * <p>Side effects: Elements of the associative array given by "varName" are modified.
   *
   * @param interp current interpreter
   * @param fileName file to stat
   * @param varName variable name into which to put stat results
   * @param lstat if true, perform lstat instead of stat
   * @throws TclException
   */
  private static void getAndStoreStatData(
      Interp interp, String fileName, String varName, boolean lstat) throws TclException {
    File fileObj = FileUtil.getNewFileObj(interp, fileName);
    if (lstat) {
      File target = FileUtil.getLinkTarget(fileObj);
      if (target != null) fileObj = target;
    }
    if (!fileObj.exists()) {
      throw new TclPosixException(
          interp, TclPosixException.ENOENT, true, "could not read \"" + fileName + "\"");
  ...
  /**
   * Return the substring of "path" which represents the file's extension. It is necessary to
   * perform system specific operations because different systems have different separators.
   *
   * @param path
   * @return
   */
  ...
  /**
   * Return the substring of "path" which represents the file's tail. It is necessary to perform
   * system specific operations because different systems have different separators.
   *
   * @param interp
   * @param path Path for which to find the tail.
   * @return
   * @throws TclException Thrown if tilde subst, which may be called by splitAndTranslate, fails.
   */
  ...
  /**
   * This procedure implements the "mkdir" subcommand of the "file" command. Filename arguments need
   * to be translated to native format before being passed to platform-specific code that implements
   * mkdir functionality. WARNING: ignoring links because Java does not support them.
   *
   * @param interp current interpreter
   * @param argv arguments to the 'file command
   * @throws TclException
   */
  private static void fileMakeDirs(Interp interp, TclObject[] argv) throws TclException {
    ...
    for (int currentDir = 2; currentDir < argv.length; currentDir++) {
      ...
      if (dirName.length() == 0) {
        throw new TclPosixException(
            interp, TclPosixException.ENOENT, true, "can't create directory \"\"");
      }
      ...
      if (dirObj.exists()) {
        ...
        throw new TclPosixException(
            interp, TclPosixException.EEXIST, true, "can't create directory \"" + dirName + "\"");
      }
      try {
      ...
      if (!madeDir) {
        throw new TclPosixException(
            interp,
            TclPosixException.EACCES,
            true,
            "can't create directory \"" + dirName + "\":  best guess at reason");
      }
    }
  }
  ...
  private static void fileDelete(Interp interp, TclObject[] argv) throws TclException {
    ...
    for (boolean last = false; (firstSource < argv.length) && (!last); firstSource++) {
      ...
      switch (opt) {
        ...
        default:
          throw new TclRuntimeError(
              "FileCmd.cmdProc: bad option " + opt + " index to validOptions");
      }
    }
   ...
   * @param force if force, delete directories recursively
   * @throws TclException
   */
  private static void deleteOneFile(Interp interp, File fileObj, boolean force)
      throws TclException {
    ...
    if (fileObj.isDirectory() && (fileObj.list().length > 0)) {
      if (force) {
        ...
        throw new TclPosixException(
            interp,
            TclPosixException.ENOTEMPTY,
            "error deleting \"" + fileObj.getName() + "\": directory not empty");
      }
    }
    try {
    ...
    if (!isDeleted) {
      throw new TclPosixException(
          ...
          TclPosixException.EACCES,
          true,
          "error deleting \"" + fileName + "\":  best guess at reason");
    }
  }

  /**
   * This procedure implements the "copy" and "rename" subcommands of the "file" command. Filename
   * arguments need to be translated to native format before being passed to platform-specific code
   * that implements copy functionality.
   *
   * @param interp current interpreter
   * @param argv arguments to 'file' command
   * @param copyFlag if true, do a copy, else do a rename
   * @throws TclException
   */
  private static void fileCopyRename(Interp interp, TclObject[] argv, boolean copyFlag)
      throws TclException {
    int firstSource = 2;
    boolean force = false;
    ...
    if (firstSource >= (argv.length - 1)) {
      throw new TclNumArgsException(
          interp, firstSource, argv, "?options? source ?source ...? target");
    }
    ...
    if (targetObj.isDirectory()) {
      ...
      if (firstSource + 1 != target) {
        ...
        throw new TclPosixException(
            interp,
            TclPosixException.ENOTDIR,
            "error " + action + ": target \"" + argv[target].toString() + "\" is not a directory");
      }
  ...
  /**
   * After performing error checking, performs the copy and rename commands. WARNING: ignoring links
   * because Java does not support them.
   *
   * @param interp
   * @param sourceName source file.
   * @param targetName target file.
   * @param copyFlag whether to copy or rename.
   * @param force whether to overwrite.
   * @throws TclException
   */
  private static void copyRenameOneFile(
      Interp interp, String sourceName, String targetName, boolean copyFlag, boolean force)
      throws TclException {
    ...
    if ((!sourceFileObj.exists()) || (sourceName.length() == 0)) {
      throw new TclPosixException(
          interp, TclPosixException.ENOENT, true, "error " + action + " \"" + sourceName + "\"");
    }
    ...
    if (sourceFileObj.isDirectory() && !targetFileObj.isDirectory() && targetFileObj.exists()) {
      throw new TclPosixException(
          interp,
          TclPosixException.EISDIR,
          "can't overwrite file \"" + targetName + "\" with directory \"" + sourceName + "\"");
    }
    if (targetFileObj.isDirectory() && !sourceFileObj.isDirectory()) {
      throw new TclPosixException(
          interp,
          TclPosixException.EISDIR,
          "can't overwrite directory \"" + targetName + "\" with file \"" + sourceName + "\"");
    }
