...
public final class ScanCmd implements Command {
  ...
  /**
   * This procedure is invoked to process the "scan" Tcl command. See the user documentation for
    ...
    while (formatIndex < format.length()) {
      ...
      if (Character.isWhitespace(ch)) {
        while (stringIndex < string.length()
            && Character.isWhitespace(string.charAt(stringIndex))) {
          ++stringIndex;
        }
    ...
    if (numVars > 0) {
      ...
      for (int i = 0; i < totalVars; i++) {
        ...
        try {
          ...
          if (varErrors == null) varErrors = new StringBuffer();
      ...
      if (varErrors != null) throw new TclException(interp, varErrors.toString());
    } else {
      /*
  ...
  /**
   * Parse the format string and verify that it is properly formed and that there are exactly enough
   * variables on the command line.
   *
   * @param interp The current interpreter
   * @param format The format string
   * @param numVars The number of variables passed to the scan command
   * @return Number of variables that will be required.
   * @throws TclException if any invalid scan format is encountered.
   */
    ...
    while (formatIndex < format.length()) {
      ...
      if (!((flags & SCAN_SUPPRESS) != 0)) {
        if (objIndex >= nassign.length) {
           ...
           */
          int nspace;
          if (xpgSize > 0) nspace = xpgSize;
          else nspace = nassign.length + 16;
  ...
  /**
   * Called whenever the number of varName args do not match the number of found and valid
   * formatSpecifiers (matched and unmatched).
   *
  ...
  private static final void errorCannotMix(Interp interp, char fieldSpecifier) throws TclException {
    throw new TclException(
        interp, "cannot mix \"%\" and \"%n" + fieldSpecifier + "\" conversion specifiers");
  }
  ...
  /**
   * Called whenever the a width field is used in a char ('c') format specifier
   *
   * @param interp - The TclInterp which called the cmdProc method .
   */
  ...
  private static final void errorMultipleAssignments(Interp interp) throws TclException {
    throw new TclException(
        interp, "variable is assigned by multiple \"%n$\" conversion specifiers");
  }
  ...
  private static class CharSet {
    /** True if CharSet starts with '^' indicating it has chars to be excluded */
    ...
    /**
     * Index of the first character after the end of the charset's ']' in the format string passed
     * to the constructor
     */
    int endOfFormat;
     ...
     *
     * @param format Format string which may contain other format information
     * @param formatIndex index of first character in format after opening '[' of Charset
     */
    ...
    /**
     * @return the next character to continue processing from in the format string
     */
    int getEndOfFormat() {
      return endOfFormat;
    }
