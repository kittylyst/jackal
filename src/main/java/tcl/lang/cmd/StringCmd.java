...
public final class StringCmd implements Command {
  ...
  private static final String isOptions[] = {
    "alnum",
    "alpha",
    "ascii",
    "control",
  ...
  /**
   * Java's Character class has a many boolean test functions to check the kind of a character (like
   * isLowerCase() or isISOControl()). Unfortunately some are missing (like isPunct() or isPrint()),
   * so here we define bitsets to compare the result of Character.getType().
   */
  private static final int ALPHA_BITS =
  ...
  private static final int PUNCT_BITS =
      ((1 << Character.CONNECTOR_PUNCTUATION)
          | (1 << Character.DASH_PUNCTUATION)
          | (1 << Character.START_PUNCTUATION)
          | (1 << Character.END_PUNCTUATION)
          | (1 << Character.OTHER_PUNCTUATION));
  private static final int PRINT_BITS =
      (ALPHA_BITS
  ...
  private static final int WORD_BITS =
      (ALPHA_BITS | (1 << Character.DECIMAL_DIGIT_NUMBER) | (1 << Character.CONNECTOR_PUNCTUATION));

  /**
   * ----------------------------------------------------------------------
   *
   * <p>Tcl_StringObjCmd -> StringCmd.cmdProc
   *
   * <p>This procedure is invoked to process the "string" Tcl command. See the user documentation
      ...
      case STR_COMPARE:
        {
          if (objv.length < 4 || objv.length > 7) {
            throw new TclNumArgsException(
                interp, 2, objv, "?-nocase? ?-length int? string1 string2");
          }
          ...
          for (int i = 2; i < objv.length - 2; i++) {
            ...
            if ((length2 > 1) && "-nocase".startsWith(string2)) {
              ...
              throw new TclException(
                  interp, "bad option \"" + string2 + "\": must be -nocase or -length");
            }
          }
          ...
          if (objv.length != 4) {
            for (int i = 3; i < objv.length - 1; i++) {
              ...
              if ((length2 > 1) && "-strict".startsWith(string2)) {
                ...
                throw new TclException(
                    interp, "bad option \"" + string2 + "\": must be -strict or -failindex");
              }
            }
          }
                ...
                for (failat = 0; failat < length1; failat++) {
                  ...
                  switch (index) {
                    ...
                    case STR_IS_DIGIT:
                      ...
                      break;
                    case STR_IS_GRAPH:
                      result = ((1 << Character.getType(c)) & PRINT_BITS) != 0 && c != ' ';
                      break;
                    case STR_IS_PRINT:
      ...
      case STR_REPLACE:
        {
          ...
          if ((last < first) || (first > length1) || (last < 0)) {
            ...
            TclObject tstr = TclString.newInstance(string1.substring(0, first));
  ...
  public static final int Utf8Count(String s) {
    int p = 0;
