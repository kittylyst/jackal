...
public final class PackageCmd implements Command {
  ...
  public static void pkgProvide(
      Interp interp, // Interpreter in which package
      // is now
      // available.
   ...
   * ----------------------------------------------------------------------
   */
  static String pkgPresent(
      Interp interp, // Interpreter in which package is
      // now
      // available.
    ...
    for (i = 0; i < max; i++) {
      ...
      try {
        ...
        throw new TclRuntimeError(
            "NumberFormatException for package versions \"" + v1 + "\" or \"" + v2 + "\"");
      }
