...
public final class LsearchCmd implements Command {
  ...
  /**
   * This procedure is invoked to process the "lsearch" Tcl command. See the user documentation for
   ...
   *
   * <p>This function compares two strings as if they were being used in an index or card catalog.
   * The case of alphabetic characters is ignored, except to break ties. Thus "B" comes before "b"
   * but after "a". Also, integers embedded in the strings compare in numerical order. In other
   * words, "x10y" comes after "x9y", not before it as it would when using strcmp().
   *
   * <p>Results: A negative result means that the first element comes before the second, and a
   * positive result means that the second element should come first. A result of zero means the two
   * elements are equal and it doesn't matter which comes first.
   *
   * <p>Side effects: None.
   *
   * @param left
  ...
  private static int DictionaryCompare(String left, String right) {
    ...
    while (true) {
      if ((rInd < rightArr.length)
        ...
        int zeros = 0;
        while ((rightArr[rInd] == '0')
            && (rInd + 1 < rightArr.length)
            && (Character.isDigit(rightArr[rInd + 1]))) {
          rInd++;
          zeros--;
        }
        while ((leftArr[lInd] == '0')
            && (lInd + 1 < leftArr.length)
