...
public final class BinaryCmd implements Command {
                  ...
                  if (count == BINARY_NOCOUNT) {
                    ...
                    if (count == BINARY_ALL) {
                      ...
                      throw new TclException(
                          interp, "number of elements in list" + " does not match count");
                    }
                  }
                  offset += count * size;
                  break;
              ...
              case 'x':
                {
                  if (count == BINARY_ALL) {
                    throw new TclException(
                        interp, "cannot use \"*\"" + " in format string with \"x\"");
                  }
                  if (count == BINARY_NOCOUNT) {
                    count = 1;
                  }
      ...
      case CMD_SCAN:
        {
          if (argv.length < 4) {
            throw new TclNumArgsException(
                interp, 2, argv, "value formatString ?varName varName ...?");
          }
  ...
  /**
   * This function parses the format strings used in the binary format and scan commands.
   *
   * <p>Results: Moves the parsePos to the start of the next command. Returns the current command
   * character or FORMAT_END if the string did not have a format specifier.
   *
   * @param format Format string.
   * @param parsePos Current position in input.
   * @return
   */
   ...
   * This function parses the format strings used in the binary format and scan commands.
   *
   * <p>Results: Moves the formatPtr to the start of the next command. Returns the current command
   * count. The count is set to BINARY_ALL if the count character was '*' or BINARY_NOCOUNT if no
   * count was specified.
   *
   * @param format Format string.
   * @param parsePos Current position in input.
  ...
  /**
   * This method is called by the binary cmdProc to format a number into a location pointed at by
   * cursor.
   *
   * @param interp
   * @param type Type of number to format.
   * @param src Number to format.
   * @param resultBytes
   * @param cursor
   * @return
   * @throws TclException
  ...
  /**
   * This routine is called by Tcl_BinaryObjCmd to scan a number out of a buffer.
   *
   * <p>Results: Returns a newly created object containing the scanned number. This object has a ref
   * count of zero.
   *
   * @param src Buffer to scan number.
   * @param pos Position in buffer.
   * @param type Format character from "binary scan"
   * @return
   */
  ...
  /**
   * Called whenever a format was found which restricts the valid range of characters in the
   * specified string, but the string contains at least one char not in this range.
   *
   * @param interp
   * @param expected
   * @param str
  ...
  private static void expectedButGot(Interp interp, String expected, String str)
      throws TclException {
    throw new TclException(
        interp, "expected " + expected + " string but got \"" + str + "\" instead");
  }
}
