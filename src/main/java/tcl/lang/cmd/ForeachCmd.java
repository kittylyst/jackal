...
public final class ForeachCmd implements Command {
   ...
   *
   * <p>This procedure is invoked to process the "foreach" Tcl command. See the user documentation
        ...
        for (j = 0; j < nSize; j++) {
          try {
            if (base + j >= TclList.getLength(interp, value[x])) {
              ...
              interp.setVar(
                  TclList.index(interp, name[x], j), TclList.index(interp, value[x], base + j), 0);
            }
          } catch (TclException e) {
            throw new TclException(
                interp,
                "couldn't set loop variable: \"" + TclList.index(interp, name[x], j) + "\"");
          }
        }
      }
