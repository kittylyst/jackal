...
public final class InterpCmd implements Command {
   ...
   * ----------------------------------------------------------------------
   *
   * <p>Tcl_InterpObjCmd -> cmdProc
   *
   * <p>This procedure is invoked as part of the Command interface to process the "interp" Tcl
      ...
      case OPT_ALIAS:
        {
          ...
          throw new TclNumArgsException(
              interp, 2, objv, "slavePath slaveCmd ?masterPath masterCmd? ?args ..?");
        }
      case OPT_ALIASES:
        {
      ...
      case OPT_SHARE:
        {
          ...
          TclIO.giveChannel(
              getInterp(interp, objv[2]), getInterp(interp, objv[4]), objv[3].toString(), false);
          break;
        }
      case OPT_TARGET:
        {
          ...
          if (targetInterp == null) {
            throw new TclException(
                interp,
                "alias \"" + aliasName + "\" in path \"" + objv[2].toString() + "\" not found");
          }
      ...
      case OPT_TRANSFER:
        {
          ...
          TclIO.giveChannel(
              getInterp(interp, objv[2]), getInterp(interp, objv[4]), objv[3].toString(), true);
          break;
        }
    }
  }
  ...
  private static Interp getInterp(
      Interp interp, // Default interp if no
   ...
   * ----------------------------------------------------------------------
   *
   * <p>Tcl_GetInterpPath -> getInterpPath
   *
   * <p>Sets the result of the asking interpreter to a proper Tcl list containing the names of
   * interpreters between the asking and target interpreters. The target interpreter must be either
   * the same as the asking interpreter or one of its slaves (including recursively).
   *
   * <p>Results: true if the target interpreter is the same as, or a descendant of, the asking
   * interpreter; false otherwise. This way one can distinguish between the case where the asking
   * and target interps are the same (true is returned) and when the target is not a descendant of
   * the asking interpreter (in which case false is returned).
   *
   * <p>Side effects: None.
   *
   * <p>----------------------------------------------------------------------
   */
  private static boolean getInterpPath(
      Interp askingInterp, // Interpreter to
      // start search
      // from.
   ...
   * <p>getInterp --
   *
   * <p>Helper function to find a slave interpreter given a pathname.
   *
   * <p>Results: Returns the slave interpreter known by that name in the calling interpreter, or
   * NULL if no interpreter known by that name exists.
   *
   * <p>Side effects: Assigns to the pointer variable passed in, if not NULL.
   *
   * <p>----------------------------------------------------------------------
   */
  static Interp getInterp(
