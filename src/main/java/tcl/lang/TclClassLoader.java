 ...
 * TclClassLoader.java --
 *
 * <p>Implements the Class Loader for dynamically loading Tcl packages. When attempting to resolve
 * and load a new Package the loader looks in four places to find the class. In order they are:
 *
 * <p>1) The unique cache, "classes", inside the TclClassLoader. 2) Using the system class loader
 * (via the context class loader). 3) Any paths passed into the constructor via the pathList
 * variable. 4) Any path in the interps env(TCL_CLASSPATH) variable.
 *
 * <p>The class will be found if it is any of the above paths or if it is in a jar file located in
 * one of the paths.
 *
 * <p>TclClassLoader.java --
 *
 * <p>A class that helps filter directory listings when for jar/zip files during the class
 * resolution stage.
 *
 * <p>Copyright (c) 1997 by Sun Microsystems, Inc.
 *
...
public class TclClassLoader extends ClassLoader {
   ...
   * per-interp basis, so this will cache instances of class data for each access in the interp.
   ...
   * instance basis because classes may be resolved at later times. classpath is passed into the
   * constructor.
   */
  ...
  /**
   * Each instance can have a list of additional paths to search. This needs to be stored on a per
   * instance basis because classes may be resolved at later times. loadpath is extracted from the
   * env(TCL_CLASSPATH) interp variable.
   */
  private String[] loadpath = null;

  /**
   * Each instance can have a list of additional paths to search. This needs to be stored on a per
   * instance basis because classes may be resolved at later times. cached_tclclasspath is last
   * used.
   */
  private String cached_tclclasspath = null;

  /**
   * Used only for error reporting when something went wrong with a class that was loaded out of a
   * jar and we want to know which jar. Will be null unless the last searched class was found in a
   * jar.
   */
  ...
  /**
   * Pointer to interp, non-null when the value of env(TCL_CLASSPATH) should be used and checked for
   * updates.
   */
  ...
  /**
   * TclClassLoader stores the values to classpath and env(TCL_CLASSPATH) on a per object basis.
   * This is necessary because classes may not be loaded immediately, but classpath and loadpath may
   * change over time, or from object to to object.
   *
   * <p>The list of paths in pathList and env(TCL_CLASSPATH) can be relative to the current interp
   * dir. The full path names are resolved, before they are stored.
   *
   * <p>Results:
   *
   * <p>Side effects:
  ...
  /**
   * Release resources, to ensure that interp has no references (especially in a container
   * environment.)
   */
  public void dispose() {
    parent = null;
  ...
  /**
   * Check the env(TCL_CLASSPATH) variable to see if it has changed since the last invocation. In
   * the init case, the loadpath array is null.
   */
  ...
  /**
   * Resolves the specified name to a Class. This method differs from the regular loadClass(String)
   * because it always passes a true resolveIt argument to loadClass(String, boolean).
   *
   * <p>Results: the resolved Class, or null if it was not found.
   *
   * <p>Side effects: ClassNotFoundException if the class loader cannot find a definition for the
   * class.
   *
  ...
  /**
   * Resolves the specified name to a Class. The method loadClass() is called by the JavaLoadCmd and
   * via Interp.loadClass().
   *
   * <p>Results: the resolved Class, or null if it was not found.
   *
   * <p>Side effects: ClassNotFoundException if the class loader cannot find a definition for the
  ...
  /**
   * Resolves the specified resource name to a URL via a search of env(TCL_CLASSPATH). This method
   * is invoked by getResource() when a resource has not been found by the system loader or the
   * parent loader.
   *
   * <p>Results: the resolved URL, or null if it was not found.
   *
   * <p>Side effects: None.
   *
  ...
  protected URL findResource(String resName) throws PackageNameException {
    ...
    if (!resName.startsWith("tcl/lang/library/")) {
      if ((resName.startsWith("java/")) || (resName.startsWith("tcl/lang/"))) {
        throw new PackageNameException(
            "Can't load resource \"" + resName + "\" with java or tcl prefix via TCL_CLASSPATH",
            resName);
      }
    }
  ...
  /**
   * Attempt to resolve a resource using the parent class loader and then the tcl class loader. This
   * method seems to be needed because the JDK is not behaving the way it should as it is not
   * loading resources from the parent.
   *
   * <p>Results: the resolved URL, or null if it was not found.
   *
   * <p>Side effects: None.
  ...
  /**
   * Given an array of bytes that define a class, create the Class. If the className is null, we are
   * creating a lambda class. Otherwise cache the className and definition in the loaders cache.
   *
  ...
  /**
   * At this point, the class wasn't found in the cache or by the parent loader. Search through
   * 'classpath' list and the Tcl environment TCL_CLASSPATH to see if the class file can be found
   * and resolved. If ".jar" or ".zip" files are found, search them for the class as well.
   *
   * <p>Results: an array of bytes that is the content of the className file. null is returned if
   * the class could not be found or resolved (e.g. permissions error).
   *
   * <p>Side effects: None.
   *
   * @param paths the name of the class trying to be resolved
   * @param className
   * @return
   */
  ...
  /**
   * Given a directory and a class to be found, get a list of ".jar" or ".zip" files in the current
   * directory. Call extractClassFromJar to search the Jar file and extract the class if a match is
   * found.
   *
   * <p>Results: An array of bytes that is the content of the className file. null is returned if
   * the class could not be resolved or found.
   *
   * <p>Side effects: None.
   *
   * @param curDir An absoulte path for a directory to search
   * @param className The name of the class to extract from the jar file.
   * @return
   * @throws IOException
   */
  ...
  /**
   * Look inside the jar file, jarName, for a ZipEntry that matches the className. If a match is
   * found extract the bytes from the input stream.
   *
   * <p>Results: An array of bytes that is the content of the className file. null is returned if
   * the class could not be resolved or found.
   *
   * <p>Side effects: None.
   *
   * @param jarName An absoulte path for a jar file to search.
   * @param className The name of the class to extract from the jar file
   * @return
   * @throws IOException
   */
  ...
  /**
   * For some reason, using ZipInputStreams, the ZipEntry returned by getNextEntry() doesn't contain
   * a valid uncompressed size, so there is no way to determine how much to read. Using the ZipFile
   * object will return useful values for the size, but the inputStream returned doesn't work. The
   * solution was to use both methods to ultimtely extract the class, which results in an order n^2
   * algorithm. Hopefully this will change...
   *
   * <p>Results: The size of the uncompressed class file.
   *
   * <p>Side effects: None.
   *
   * @param jarName
  ...
  /**
   * Given a String, construct a File object. If it is not an absoulte path, then prepend the
   * interps current working directory, to the dirName.
   *
   * <p>Results: The absolute path of dirName
   *
   * <p>Side effects: None.
   *
   * @param interp the current Interp
   * @param dirName name of directory to be qualified
   * @return
   */
  ...
  /**
   * Given a resource name like "/testext/cmd.tcl" loop over the classpath elements looking for a
   * match to this resource name. If ".jar" or ".zip" files are found, search them for the resource
   * as well.
   *
   * <p>Results: A URL object.
   *
   * <p>Side effects: None.
   *
   * @param paths
   * @param resName the name of the resource trying to be resolved
   * @return
   */
  ...
  /**
   * Given a directory and a resource to be found, get a list of ".jar" or ".zip" files in the
   * current directory. Call extractURLFromJar to search the Jar file and extract the resource if a
   * match is found.
   *
   * <p>Results: A URL or null.
   *
   * <p>Side effects: None.
   *
   * @param curDir An absoulte path for a directory to search
   * @param resName he name of the resource to extract from the jar file
   * @return
   * @throws IOException
   */
  ...
  /**
   * Look inside the jar file, jarName, for a ZipEntry that matches the resName. If a match is found
   * then generate a URL object.
   *
   * <p>Results: A URL object, or null.
   *
   * <p>Side effects: None.
   *
   * @param jarName An absoulte path for a jar file to search.
   * @param resName The name of the resource to extract from the jar file.
   * @return
   * @throws IOException
   */
