...
public final class TclLambda implements InternalRep {
  ...
  public static void apply(Interp interp, TclObject lambdaExpr, TclObject[] args)
    ...
    Namespace ns =
        Namespace.findNamespace(
            interp,
            lambda.namespaceName,
            /* contextNs: */ null,
            TCL.GLOBAL_ONLY | TCL.LEAVE_ERR_MSG);
    if (ns == null) {
  ...
  private static TclLambda parseLambda(Interp interp, TclObject lambdaExpr) throws TclException {
    ...
    if (elems.length < 2 || elems.length > 3) {
      throw new TclException(
          interp, "can't interpret \"" + lambdaExpr.toString() + "\" as a lambda expression");
    }
  ...
  /**
   * The string representation of a lambda is generated (if necessary) when the lambda is first
   * created, and should then never need to be regenerated.
   *
