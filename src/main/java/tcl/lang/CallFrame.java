...
public final class CallFrame {
  ...
  /**
   * Chain this frame into the call frame stack and binds the parameters values to the formal
   ...
   * @return true if a variable exists and is defined inside this CallFrame, false otherwise
   */
  ...
  /**
   * @return a List of the names of the (defined) local variables in this CallFrame (excluding
   *     upvar's)
   */
   ...
   *
   * <p>Given a description of a procedure frame, such as the first argument to an "uplevel" or
   * "upvar" command, locate the call frame for the appropriate level of procedure.
   *
   * <p>The return value is 1 if string was either a number or a number preceded by "#" and it
   * specified a valid frame. 0 is returned if string isn't one of the two things above (in this
   * case, the lookup acts as if string were "1"). The frameArr[0] reference will be filled by the
   * reference of the desired frame (unless an error occurs, in which case it isn't modified).
   *
   * @param string a string that specifies the level.
   * @exception TclException if s is a valid level specifier but refers to a bad level that doesn't
   *     exist.
   */
  public static int getFrame(Interp interp, String string, CallFrame[] frameArr)
      throws TclException {
  ...
  /**
   * This method is called when this CallFrame is no longer needed. Removes the reference of this
   * object from the interpreter so that this object can be garbage collected.
   *
   * <p>For this procedure to work correctly, it must not be possible for any of the variable in the
