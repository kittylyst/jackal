...
public class TclByteArray implements InternalRep {
   ...
   * ByteArray object. Keeps track of how much memory has been used. This can be different from how
   * much has been allocated for the byte array to enable growing and shrinking of the ByteArray
   * object with fewer allocations.
   */
  private int used;
   ...
   *
   * @return the string representation of the TclByteArray, calculated by copying each byte into the
   *     lower byte of each character (Tcl's 'identity' encoding)
   */
  @Override
  ...
  /**
   * This method changes the length of the byte array for this object. Once the caller has set the
   * length of the array, it is acceptable to directly modify the bytes in the array up until
   * Tcl_GetStringFromObj() has been called on this object.
   *
   * <p>Results: The new byte array of the specified length.
   *
   * <p>Side effects: Allocates enough memory for an array of bytes of the requested size. When
   * growing the array, the old array is copied to the new array; new bytes are undefined. When
   * shrinking, the old array is truncated to the specified length.
   *
   * @param interp the current interpreter
   * @param tobj object in which to change the byte array length. Object is converted to a
   *     TclByteArray
  ...
  /**
   * Queries the length of the byte array. If tobj is not a byte array object, an attempt will be
   * made to convert it to a byte array.
   *
   * @param interp current interpreter.
  ...
  /**
   * Returns the bytes of a ByteArray object. If tobj is not a ByteArray object, an attempt will be
   * made to convert it to a ByteArray.
   *
   * <p>
   *
   * @param interp the current interpreter.
  ...
  /**
   * Interpret the bytes in the byte array according to the specified tcl encoding and return the
   * corresponding string.
   *
   * @param interp the current interpreter
   * @param tobj the object, which is converted to a TclByteArray if necessary
   * @param tclEncoding Tcl encoding in which to decode to String. 'identity', 'binary' and null all
   *     imply that the string will be made of chars whose lower byte is the corresponding byte from
   *     the array.
