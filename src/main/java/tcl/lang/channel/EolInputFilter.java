 ...
 * Channel's translation property into '\n'
 *
 * @author Dan Bodoh
 */
class EolInputFilter extends FilterReader {
  ...
  /** readline() return value if blocking is true and a complete line was not available */
   ...
   *
   * @param sb An empty stringbuffer that will contain the the line of data, without EOL characters.
   * @param block if true, block for an entire line of input, possibly blocking for input. If false,
   *     return an empty string if input would block, and don't consume any input.
  ...
  int readLine(StringBuffer sb, boolean block) throws IOException {
     ...
     */
    if (!block) unicodeDecoder.mark(1024);
    ...
    while (true) {
      ...
      if (cnt == 0) {
        if (block) continue; // channel could have switched to non-blocking
      ...
      if (mightBeEol2CharSequence(c1)) {
        ...
        if (block) {
          ...
          if (unicodeDecoder.available() > 0) {
            c2 = unicodeDecoder.peek(false);
          } else break; // can't get a character, so we don't know
        }
    ...
    if (eofSeen && sb.length() == 0) return EOF;
  ...
  /**
   * @param c1 first character in sequence
   * @param c2 second character in sequence
   * @return true if c1 by itself constitutes a EOL character, and c2 does not participate
   */
   ...
   * @param c1 first character in sequence
   * @param c2 second character in sequence
   * @return true if c1 and c2 together make up an eol sequence. Return false if only c1, or the
