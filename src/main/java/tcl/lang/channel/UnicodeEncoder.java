...
/**
 * Encodes Unicode characters into the requested encoding. Supports Tcl's 'binary' encoding. Similar
 * to a OutputStreamWriter, but prevents any internal buffering.
 *
 * @author Dan Bodoh
 */
class UnicodeEncoder extends Writer {
   ...
   *
   * @param out the underlying OutputStream that will be encoded
  ...
  /** Set the encoding to the requestedEncoding, if it is different than the current encoding */
  private void setEncoding() {
    if (encoding == null && requestedEncoding == null) return;
    if (encoding != null && encoding.equals(requestedEncoding)) return;

    encoding = requestedEncoding;
    if (encoding == null || "symbol".equals(encoding)) cse = null;
    else {
    ...
    if (cse != null) {
      ...
      if (cb.remaining() > 0) {
        ...
        throw new RuntimeException(
            "Unicode Encoder did not consume all of the input, this is unexpected: pos="
                + bb.position()
                + " limit="
                + bb.limit()
                + " result="
