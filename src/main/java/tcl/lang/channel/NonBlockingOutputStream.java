...
class NonBlockingOutputStream extends FilterOutputStream implements Runnable {
   ...
   * @param blocking if true, perform blocking writes. If false, perform non-blocking writes
   * @param sync Channel implementation specific Sync object, can be null
   */
  ...
  public void flush() throws IOException {
    ...
    if (blocking) waitForEmptyQueue();
    throwExceptionIfCaught();
  }

  /**
   * This write() will assume the caller will overwrite the buffer, so it will make a copy if
   * non-blocking writes are in effect. Use writeAssumingExclusiveBufferUse(byte[], int, int) to
   * avoid the byte array copy.
   *
   * @param b buffer containing data to write
   * @param off offset of first byte in b to write
  ...
  /**
   * Similar to write(byte [], int, int), although it assumes that the caller will not write to the
   * b after this method returns. Use this method for optimized writes.
   *
   * @param b buffer containing data to write
   * @param off offset of first byte in b to write
   * @param len number of bytes to write
  ...
  /**
   * Runs as the background writer thread; writes data placed in the queue in both blocking and
   * non-blocking mode.
   *
  ...
  public void run() {
    while (true) {
      ...
      synchronized (notifier) {
        ...
        if (isQueueEmpty()) notifier.notifyAll();
      }
    }
  }
  ...
  private class Transaction {
     ...
     *
     * @param type Either Write, Flush or Close. If Write, the data to write is undefined.
     */
     ...
     * @param off index of first byte in buffer to write
     * @param len number of bytes to write
     */
