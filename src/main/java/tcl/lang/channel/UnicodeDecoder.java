...
/**
 * This class implements Unicode decoding for Tcl input Channels. It is similar to an
 * InputStreamReader, but can report on its internal buffering, and handles Tcl's "binary" encoding.
 *
 * @author Dan Bodoh
 */
class UnicodeDecoder extends Reader {
   ...
   *
   * @param in the underlying InputStream (as an InputBuffer) that will be decoded
  ...
  private void setEncoding() {
    ...
    if (encoding == null && requestedEncoding == null) return;
    if (encoding != null && encoding.equals(requestedEncoding)) return;
  ...
  void cancelEof() {
    this.eofSeen = false;
    if (csd != null) csd.reset();
  }

  /**
   * Take a look at the next byte in the stream, bypassing the unicode decoder. Used to look for the
   * \n or a \r\n sequence.
   *
   * @param consume if true, consume the next byte in the input stream
   * @return next byte (not the Unicode character!) from underlying input stream, possibly consuming
   *     it. Returns -1 on eof
   * @throws IOException
   */
  ...
  /**
   * @return number of bytes (not chars!) that can be consumed without blocking.
   */
   ...
   *
   * @param cb character buffer to receive characters
   * @param origPos position in character buffer at start of decoding operation
        ...
        if (bb.remaining() > 0 && !eofSeen) {
          ...
          int cnt =
              in.read(
                  byteArray,
                  bb.position(),
                  cb.remaining() > bb.remaining() ? bb.remaining() : cb.remaining());
