...
/**
 * Provides a resizeable output buffer for Tcl Channels. Unlike InputBuffer, it is not responsible
 * for non-blocking writes. It also does a flush() after every write to the underlying stream, to
 * flush out any underlying buffers.
 *
 * @author Dan Bodoh
 */
class OutputBuffer extends FilterOutputStream {
   ...
   * @param bufferingMode one of TclIO.BUFF_FULL, TclIO.BUFF_LINE, TclIO.BUFF_NONE
   */
  ...
  /**
   * @param bufferSize new buffer size, which may not be honored immediately
   */
  ...
  /**
   * Set buffer to requestedSize, if it is empty. Always re-allocates buffer, even if size stays the
   * same, because buffer gets passed down to nonBlockingOutputStream which may not use it
   * immediately.
   */
