 ...
 * schedules a duplicate of itself for the next fileevent script execution. It is closely tied with
 * FileEventScript.
 *
 * @author Dan Bodoh
 */
public class FileEvent extends TclEvent {
  ...
  /** set to true if stdin is being used for command input and filevents should not be fired */
   ...
   *
   * @param interp interpreter in which to create the file event
   * @param channel Channel on which to create the file event
   * @param type either READABLE or WRITEABLE
   */
   ...
   * @param interp interpreter in which to create the file event
   * @param channel Channel on which to create the file event
   * @param type either READABLE or WRITABLE
   */
  ...
  /** Permanently remove the FileEventScript for this FileEvent from the interpreter */
  void dispose() {
  ...
  public int processEvent(int flags) {
     ...
     */
    if (channel instanceof StdChannel
        && "stdin".equals(channel.getChanName())
  ...
  /**
   * @return true if stdin is currently being used for command input and file events should not be
   *     fired on stdin
   */
