...
/**
 * Implements a resizeable buffer as an InputStream for Tcl Channels. It is also responsible for
 * non-blocking reads.
 *
 * @author Dan Bodoh
 */
class InputBuffer extends FilterInputStream {
   ...
   *
   * @param in underlying InputStream for the buffer
   * @param size initial size, in bytes. Can be 0.
   * @param buffering Buffering mode: TclIO.BUFF_NONE, TclIO.BUFF_LINE or TclIO.BUFF_FULL
   * @param blockingMode Set to true for blocking input, false for non-blocking input
   * @param channel Channel which contains this InputBuffer
   */
  ...
  /**
   * @param blockingMode Set to true for blocking operation, or false for non-blocking
   */
  ...
  private void resizeBuffer() {
    synchronized (getRefillerNotifier()) {
      ...
      if (remaining() > 0 || (buffer != null && buffer.length == size)) return;
  ...
  /**
   * Request that the buffer be resized. The request may not be honored immediately.
   *
   * @param size new requested size. If size < 0 it is set to 0.
   */
  void setBufferSize(int size) {
    if (refiller == null) requestedBufferSize = size;
    else
      synchronized (getRefillerNotifier()) {
   ...
   *
   * @param buffering TclIO.BUFF_FULL, TclIO.BUFF_NONE or TclIO.BUFF_LINE
   */
  ...
  /**
   * @return true if the channel is in non-blocking mode, and the last read was terminated early
   ...
   *
   * @param eof End of file indicator is set to this state
   */
  void cancelEof() {
    this.eofSeen = false;
  }
   ...
   * @throws IOException if refill is not in progress, and an exception was detected
   */
   ...
   *
   * @param wait if true, this method does not return until the refill operation is complete
   * @throws IOException
   */
  ...
  /**
   * Reads one byte from buffer or underlying stream. Does not honor blockingMode; will block of no
  ...
  public int read() throws IOException {
    synchronized (getRefillerNotifier()) {
      ...
      if (remaining() == 0) requestRefill(true);
      else refiller.throwIOExceptionIfCaught();
      if (eofSeen) return -1;
      if (buffer.length == 0) {
        int c = super.read();
        if (c == -1) eofSeen = true;
   ...
   * @param off offset where bytes should go into buffer
   * @param len maximum number of bytes to read
   * @return number of bytes read, or -1 at end-of-file. Will return 0 if in non-blocking mode and
  ...
  private class Refiller extends Thread {
    ...
    /**
     * @throws IOException if there was an exception caught during refill
     */
    ...
    /**
     * Refill the buffer from the underlying input stream. Any data in the buffer is lost.
     *
     * @throws IOException
     */
