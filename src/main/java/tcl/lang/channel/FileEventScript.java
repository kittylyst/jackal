 ...
 * FileEventScript object indicates the existence of a particular 'fileevent' command instance. Once
 * the 'fileevent' command is called to delete the script, or if the interpreter is destroyed, the
 * associated FileEventScript is disposed which signals FileEvents associated with this script to
 * discontinue.
 */
public class FileEventScript implements AssocData {
   ...
   *
   * @param interp Interpreter for the fileevent
   * @param channel name of channel that the fileEvent is registered on
   * @param type either FileEvent.READABLE or FileEvent.WRITEABLE
  ...
  /**
   * Return a formatted name to use for the interpreter's assocData hash key to locate a
   * FileEventScript
   *
   * @param channelName name of the channel containing the fileevent
   * @param type either FileEvent.READABLE or FileEvent.WRITEABLE
  ...
  private static String getName(Channel channel, int type) {
    ...
    return channelName
        + (type == FileEvent.READABLE ? " readable " : " writeable ")
        + FileEventScript.class.getName();
  }

  /**
   * Find a FileEventScript if it exists
   *
   * @param interp Interpreter to search for the fileevent
   * @param channel name of channel that the fileEvent is registered on
   * @param type either FileEvent.READABLE or FileEvent.WRITEABLE
