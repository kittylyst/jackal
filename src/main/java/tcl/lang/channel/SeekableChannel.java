...
public abstract class SeekableChannel extends Channel {
   ...
   *
   * @param offset absolute offset into the file
   * @throws IOException
   */
  abstract void implSeek(long offset) throws IOException;

  /**
   * @return the current byte offset in the file, relative to the beginning of the file.
   */
  ...
  void seekReset() {
    if (inputBuffer != null) inputBuffer.seekReset();
    if (eolInputFilter != null) eolInputFilter.seekReset();
    if (eofInputFilter != null) eofInputFilter.seekReset();
    if (unicodeDecoder != null) unicodeDecoder.seekReset();
  ...
  /**
   * Called just prior to each write to the getOutputStream() stream, if the channel is in
   * TclIO.APPEND mode, to guarantee that write will append to the end of the output. There is no
   * need to flush any of the channel buffers.
   *
   * @throws IOException
   */
  ...
  void setEofSeenWithoutRead() {
    try {
      if (getNumBufferedInputBytes() == 0 && implTell() >= getMaxSeek()) eofSeen = true;
    } catch (IOException e) {
