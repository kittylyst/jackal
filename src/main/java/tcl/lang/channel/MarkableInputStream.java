...
/**
 * An InputStream that supports mark(), with an unlimited readlimit. Also supports unread() to push
 * bytes back into the sream.
 */
class MarkableInputStream extends FilterInputStream {
   ...
   *
   * @param in InputStream that provides bytes for MarkableStream
   */
  ...
  /**
   * Save bytes into the buffer, allocating space as necessary. Sets readPos and writePos to the end
   * of the new data.
   *
   * @param b byte array containing bytes to save
   * @param off offset of data in b
   * @param len length of data in b
   */
  ...
  /**
   * Sets the mark at the current stream position, or removes this stream's mark.
   *
   * @param readlimit if <= 0, removes the current mark. Otherwise, set the mark and ignore
   *     readlimit. The internal buffer grows as needed.
   ...
   *
   * @param c byte to put back into the stream
   */
  public void unread(int c) {
    if (c != -1) {
      byte[] b = new byte[1];
      b[0] = (byte) (c & 0xff);
      if (!marked) saveBytes(b, 0, 1);
      --readPos;
    }
  }
  ...
  public int read(byte[] b, int off, int len) throws IOException {
    ...
    if (len > 0) {
      ...
      if (cnt == -1 && nBytes == 0) nBytes = -1;
      else {
        if (writePos == readPos && marked) saveBytes(b, off, cnt);
        nBytes += cnt;
      }
    }
    return nBytes;
