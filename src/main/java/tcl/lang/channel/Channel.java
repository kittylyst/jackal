 ...
 * performs generic reads, writes, without specifying how a given channel is actually created. Each
...
public abstract sealed class Channel
  ...
  /**
   * This OutputStream writes the EOF character, and prevents this Chanel's getOutputStream() from
   * being closed by the chain.
   */
   ...
   * Tcl_ReadChars -> read
   *
   * <p>Read data from this Channel into the given TclObject.
   *
   * @param interp is used for TclExceptions.
   * @param tobj the object that data will be added to.
   * @param readType specifies if the read should read the entire input (TclIO.READ_ALL), the next
   *     line without the end-of-line character (TclIO.READ_LINE), or a specified number of bytes
   *     (TclIO.READ_N_BYTES).
   * @param numBytes the number of bytes/chars to read. Ignored unless readType is
   *     TclIO.READ_N_BYTES.
        ...
        case TclIO.READ_N_BYTES:
          {
            ...
            if (readChars) buf = new char[bufsize];
            else TclByteArray.setLength(interp, tobj, 0);
            while (total < numBytes) {

              if (readChars) cnt = finalReader.read(buf, 0, Math.min(buf.length, numBytes - total));
              else {
                /* resize array */
                 ...
                 */
                cnt =
                    finalInputStream.read(
                        TclByteArray.getBytes(interp, tobj),
                        total,
                        Math.min(bufsize, numBytes - total));
              }
              if (cnt == -1) {
                eofSeen = true;
                break;
              ...
              if (readChars) TclString.append(tobj, buf, 0, cnt);
              total += cnt;
            }
            if (!readChars) {
    ...
    try {
      ...
      if (outData.isByteArrayType()
        ...
        if (outData.isByteArrayType() && encoding != null) {
          ...
          cbuf =
              TclByteArray.decodeToString(interp, outData, EncodingCmd.systemTclEncoding)
                  .toCharArray();
          if (cbuf.length == 0 && TclByteArray.getLength(interp, outData) > 0) {
            ...
            throw new TclException(
                interp, "error writing \"" + getChanName() + "\": invalid argument");
          }
        } else {
   ...
   *
   * <p>Write string data to the Channel.
   *
   * @param interp is used for TclExceptions.
   ...
   *
   * @param takeOwnership if true, give Channel ownership to the current thread if no other thread
   *     owns it; otherwise, give up ownership if the current thread owns it
   * @param type Either READ_OWNERSHIP or WRITE_OWNERSHIP
   * @return If takeOwnership is true, returns true if the current thread successfully received
   *     ownership. If takeOwnership is false, returns true if the current thread was the owner of
   *     the channel.
   */
   ...
   *     owns it; otherwise, give up ownership if the current thread owns it
   * @param type Either READ_OWNERSHIP or WRITE_OWNERSHIP
   * @param threadId Thread.getId() of thread that should own the channel
   * @return If takeOwnership is true, returns true if the current thread successfully received
   *     ownership. If takeOwnership is false, returns true if the current thread was the owner of
   *     the channel.
   */
  ...
  /**
   * Channels subclasses should override this to perform any specific close() operations, including
   * closing of the getInputStream() and getOutputStream() streams if necessary. impClose() is
   * called by NonBlockingOutputStream.Transaction.perform() after the input and output chain of
   * readers, writers and streams are closed, or directly by Channel.close() if the channel is
   * read-only.
   *
   * @throws IOException
   */
  ...
  /**
   * Close the Channel. The channel is only closed, it is the responsibility of the "closer" to
   * remove the channel from the channel table. Channel subclass specific closing is done is
   * impClose(), which is called on behalf of this method by nonBlockingOutputStream for writable
   * channels. Note that any thread can close the channel, even if it is not the owner.
   */
   ...
   * Flush the Channel.
   *
   * @throws TclException when attempting to flush a read only channel.
   * @throws IOException for all other flush errors.
   */
  ...
  /**
   * Channel subclasses should override this to flush any operating system buffers for this Channel
   * out to the physical medium. The default implementation here does nothing. FileChannel and
   * similar classes should override this to provide sync() capability.
   */
   ...
   * @param mode where to begin incrementing the file pointer; beginning, current, end.
   * @throws TclException if channel does not support seek
   * @throws IOException
   */
  public void seek(Interp interp, long offset, int mode) throws IOException, TclException {
    throw new TclPosixException(
        interp, TclPosixException.EINVAL, true, "error during seek on \"" + getChanName() + "\"");
  }

  /**
   * @return the current file position. If tell is not supported on the given channel then -1 will
   *     be returned. Subclasses should use the SeekableChannel class to provide tell() capability.
   */
  ...
  /**
   * If we can detect EOF without another read, set eofSeen. Default implementation can't do much,
   * but SeekableChannel can.
   */
  void setEofSeenWithoutRead() {}

  /** Setup the input stream chain on the first read */
  protected void initInput() throws IOException {
    if (finalReader != null) return;
  ...
  /**
   * @return true if the channel is writable, according to the 'fileevent' definition
   */
  boolean isWritable() {
    if (outputBuffer == null && !closed) return true;
    if (closed) return false;
    if (outputBuffer.getBufferedByteCount() >= bufferSize) return false;
    return true;
  }

  /**
   * @return true if the channel is readable, according to the 'fileevent' definition
   */
  boolean isReadable() {
    if (inputBuffer == null) return false;
    if (encodingChangedSinceLastRead) return true;
    if (inputBuffer.eof()) return true;
    try {
      /* Line buffering may be waiting on a EOL.  test io-54.2 deadlocks
       * if line buffered channel doesn't become readable
       */
      if (buffering == TclIO.BUFF_LINE
          && !inputBuffer.lastReadWouldHaveBlocked()
          && inputBuffer.isRefillInProgress()) return true;
    ...
    try {
       ...
       */
      if (e.getMessage().toLowerCase().contains("closed")) return false;
      else return true;
    }
  }
  ...
  /**
   * This method should be overridden in the subclass to provide a channel specific InputStream
   * object.
   */
  protected abstract InputStream getInputStream() throws IOException;

  /**
   * This method should be overridden in the subclass to provide a channel specific OutputStream
   * object.
   */
   ...
   *     <p>This is the equivilent of the Tcl_ChannelType->typeName field.
   */
   ...
   *
   * @param chan the unique channel name for the chanTable hash table
   */
   ...
   * @throws TclException if this channel was not opened for reading
   */
   ...
   *
   * @param interp the current interpreter
   * @throws TclException if this Channel was not opened for writing
   */
   ...
   * Set blocking mode.
   *
   * @param inBlocking True for blocking mode, false for non-blocking mode.
   */
  ...
  /**
   * @return buffering mode - TclIO.BUFF_FULL, TclIO.BUFF_LINE or TclIO.BUFF_NONE.
   */
   ...
   * Set buffering mode
   *
   * @param inBuffering One of TclIO.BUFF_FULL, TclIO.BUFF_LINE, or TclIO.BUFF_NONE
   */
   ...
   *
   * <p>Sets the buffer size
   *
   * @param size new buffer size. Resize request is ignored if size < 1 or > 1 Mbyte
   */
  ...
  /**
   * @return Number of bytes stored in the chain of readers and input streams that have already been
   *     read from the Channel's underlying input stream
   */
  ...
  /**
   * @return Number of bytes stored in the chain of Writers and OutputStreams that have not yet been
   *     written to the Channel's underlying output strem
   */
  int getNumBufferedOutputBytes() {
    if (outputBuffer != null) return outputBuffer.getBufferedByteCount();
    else return 0;
  }
   ...
   * @return true if input if the last read on this channel was incomplete because input was blocked
   * @throws TclException if this channel is not opened for reading
   */
  public boolean isBlocked(Interp interp) throws TclException {
    checkRead(interp);
    if (inputBuffer != null) return inputBuffer.lastReadWouldHaveBlocked() && !eof();
    else return false;
  }
   ...
   *
   * @param inEncoding Java-style encoding string
   */
   ...
   *
   * @param inEof new EOF character, or 0 if none
   */
  public void setInputEofChar(char inEof) {
    ...
    if (inEof == inputEofChar) return;

    inputEofChar = (char) (inEof & 0xFF);
    if (eofInputFilter != null) {
      eofInputFilter.setEofChar((byte) inputEofChar);
    }
    if (inputBuffer != null) inputBuffer.cancelEof();
    if (unicodeDecoder != null) unicodeDecoder.cancelEof();
    eofSeen = false;
  }
   ...
   *
   * @param outEof new EOF character, or 0 if none
   */
