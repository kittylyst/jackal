...
public final class FileChannel extends SeekableChannel {
   ...
   * RandomAccessFile is the only java.io class that allows this behavior.
   */
  private RandomAccessFile file = null;

  /**
   * Open a file with the read/write permissions determined by modeFlags. This method must be called
   * before any other methods will function properly.
   *
   * @param interp currrent interpreter.
   * @param fileName the absolute path or name of file in the current directory to open
   * @param modeFlags modes used to open a file for reading, writing, etc
   * @return the channelId of the file.
   * @exception TclException is thrown when the modeFlags try to open a file it does not have
   *     permission for or if the file dosent exist and CREAT wasnt specified.
    ...
    if ((modeFlags & TclIO.RDWR) != 0) {
      ...
      if (fileObj.isDirectory()) {
        throw new TclException(
            interp, "couldn't open \"" + fileName + "\": illegal operation on a directory");
      }
      ...
      if (!appendModeCreate && !fileObj.canRead()) {
        throw new TclException(
            interp,
            "Java IO limitation: Cannot open a file " + "that has only write permissions set.");
      }
      file = new RandomAccessFile(fileObj, "rw");
  ...
  /**
   * Move the file pointer internal to the RandomAccessFile object. The file MUST be open or a
  ...
  /**
   * Return the current offset of the file pointer in number of bytes from the beginning of the
   * file. The file MUST be open or a TclRuntimeError is thrown.
   *
  ...
  private void checkFileExists(Interp interp, File fileObj) throws TclException {
    if (!fileObj.exists()) {
      throw new TclPosixException(
          interp, TclPosixException.ENOENT, true, "couldn't open \"" + fileObj.getName() + "\"");
    }
  }

  /**
   * Checks the read/write permissions on the File object. If inmode is less than 0 it checks for
   * read permissions, if mode greater than 0 it checks for write permissions, and if it equals 0
   * then it checks both.
   *
   * @param interp currrent interpreter.
   * @param fileObj a java.io.File object of the file for this channel.
  ...
  private void checkReadWritePerm(Interp interp, File fileObj, int inmode) throws TclException {
    ...
    if (error) {
      throw new TclPosixException(
          interp, TclPosixException.EACCES, true, "couldn't open \"" + fileObj.getName() + "\"");
    }
  }
  ...
  protected InputStream getInputStream() throws IOException {
    if (file == null) throw new IOException("file has not been opened, or has been closed");
