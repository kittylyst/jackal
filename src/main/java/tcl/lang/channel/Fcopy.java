...
public class Fcopy {
  ...
  /**
   * Callback Tcl command; bytesWritten and optionally an error message are appended as a list to
   * this string. A non-null callback implies background fcopy
   */
  String callback;

  /** Original value of source.getBlocking(), so it can be restored on fcopy completion */
  boolean sourceBlocking;

  /** Original value of destination.getBlockign(), so it can be restored on fcopy completion */
  boolean destinationBlocking;

  /** Original value of source.getBuffering(), so it can be restored on fcopy completion */
  int sourceBuffering;

  /** Original value of destination.getBuffering(), so it can be restored on fcopy completion */
  int destinationBuffering;

  /** Set to true if we can read/write bytes during fcopy; false if we must read/write chars */
  boolean transferBytes;

  /**
   * Set to true if input is encoded, but output is not; fcopy spec indicates output should be utf8
   * encoded in that case
   */
  boolean doUtf8OutputEncoding = false;

  /**
   * Set to true if output is encoded but input is not; fcopy spec indicates output should be
   * assumed utf8
   */
   ...
   * @param size number of bytes to write to the destination; if < 0 writing takes place to EOF on
   *     source
   * @param callback if null, return when copy is complete; if non-null, perform the copy in the
   *     background and call script in TclObject when copy is complete
   * @throws TclException
   */
  public Fcopy(Interp interp, Channel source, Channel destination, long size, String callback)
      throws TclException {
  ...
  /**
   * Start the copy process. If a callback was provided in the constructor, start() returns
   * immediately. Otherwise, it waits for the copy to cmoplete.
   *
   * @return number of bytes written to the destination, if a callback was not provided.
   * @throws TclException
   * @throws IOException
   */
    ...
    if (callback == null) {
      ...
      Runnable r =
          () -> {
            ...
            final String callbackWithArgs =
                callback + " " + bytesWritten + (errormsg == null ? "" : (" {" + errormsg + "}"));
      ...
      backgroundCopy.setName(
          "Fcopy ("
              + interp.toString()
              + "): "
              + source.getChanName()
              + " -> "
              + destination.getChanName());
   ...
   *
   * @param id Thread.getId() of thread that will own the channel
   * @throws TclException if one of the channels is busy
   */
  ...
  /**
   * Save state of Channels, change channel buffering and blocking for efficient data transfer, and
   * create Fcopy's internal buffer
   *
   * @throws IOException
   */
  private void setup() throws TclException, IOException {
     ...
     */
    if (sourceBuffering == TclIO.BUFF_FULL) source.setBuffering(TclIO.BUFF_NONE);
    if (destinationBuffering == TclIO.BUFF_FULL) destination.setBuffering(TclIO.BUFF_NONE);
    ...
    transferBytes =
        srcEncoding.equals(dstEncoding)
  ...
  /**
   * Actually copy data from the source channel to the destination channel, until EOF or size bytes
  ...
  private long doCopy() throws IOException {
    ...
    if (!destination.isClosed()) destination.firstWriter.flush();
    return bytesWritten;
  }
}
