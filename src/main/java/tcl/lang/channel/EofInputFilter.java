...
/**
 * This java.io.FilterInputStream subclass looks for a the Channel's specified EOF character to
 * signal end of file, or passes data through if there is no specified EOF character. It will not
 * propagate close() to the underlying stream - that is left to the Channel subclass to handle.
 *
 * @author Dan Bodoh
 */
class EofInputFilter extends FilterInputStream {
   ...
   *
   * @param in input stream to filter
   * @param eofchar end of file character, or 0 for none
   */
   ...
   *
   * @param eofChar new input end of file character, or 0 for no eof character
   */
  void setEofChar(byte eofChar) {
    if (eofChar != this.eofchar) cancelEof();
    this.eofchar = eofChar;
  }

  /**
   * @return true if we actually read the EOF character, indicating that we read read one too many
  ...
  public int read(byte[] b, int off, int len) throws IOException {
    ...
    if (eofchar == 0) return super.read(b, off, len);
  ...
  /**
   * Will not propagate close() to the lower level stream. This is left to the responsibility of the
   * Channel subclass.
   *
