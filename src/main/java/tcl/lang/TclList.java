...
public class TclList implements InternalRep {
  ...
  /**
   * Create a new empty Tcl List, with the list pre-allocated to the given size.
   *
   ...
   * copy (TclListObjCopy) --
   *
   * <p>Makes a "pure list" copy of a list value. This provides for the C level a counterpart of the
   * [lrange $list 0 end] command, while using internals details to be as efficient as possible.
   *
   * <p>Side effects: None.
   *
   * @param interp Used to report errors if not null.
   * @param list List object for which an element array is to be returned.
   * @return Normally returns a new TclObject, that contains the same list value as 'list' does. The
   *     returned TclObject has a refCount of zero. If 'list' does not hold a list, null is
   *     returned, and if 'interp' is non-null, an error message is recorded there.
   */
   ...
   *
   * <p>Appends a TclObject element to a list object.
   *
   * @param interp current interpreter.
   ...
   * TclList.append()
   *
   * <p>Appends multiple TclObject elements to a list object.
   *
   * @param interp current interpreter.
  ...
  /**
   * Queries the length of the list. If tobj is not a list object, an attempt will be made to
  ...
  /**
   * Returns a TclObject array of the elements in a list object. If tobj is not a list object, an
   * attempt will be made to convert it to a list.
   *
   * <p>The objects referenced by the returned array should be treated as readonly and their ref
   * counts are _not_ incremented; the caller must do that if it holds on to a reference.
   *
  ...
  /**
   * Returns an ArrayList of TclObject elements in a list object. If tobj is not a list object, an
   * attempt will be made to convert it to a list.
   *
   * <p>Note that the actual internal ArrayList of a TclList object is returned, so be careful when
   * changing the list.
   *
   * @param interp the current interpreter.
   * @param tobj the list to sort.
   ...
   *
   * <p>Set a single element of a list to a specified value
   *
   * <p>Side effects: Tcl_Panic if listPtr designates a shared object. Otherwise, attempts to
   * convert it to a list with a non-shared internal rep. Decrements the ref count of the object at
   * the specified index within the list, replaces with the object designated by valuePtr, and
   * increments the ref count of the replacement object.
   *
   * <p>It is the caller's responsibility to invalidate the string representation of the object.
   *
   * @param interp Tcl interpreter; used for error reporting if not null
   * @param list List object in which element should be stored
   * @param index Index of element to store
   * @param value Tcl object to store in the designated list element
   * @throws TclException
   */
  public static void setElement(Interp interp, TclObject list, int index, TclObject value)
      throws TclException {
    ...
    elems = (TclObject[]) listRep.alist.toArray(new TclObject[listRep.alist.size()]);
   ...
   * TclListObjSetElement --
   *
   * <p>Set a single element of a list to a specified value in place (LsetCmd)
   *
   * <p>Side effects: Tcl_Panic if listPtr designates a shared object. Otherwise, attempts to
   * convert it to a list with a non-shared internal rep. Decrements the ref count of the object at
   * the specified index within the list, replaces with the object designated by valuePtr, and
   ...
   * @throws TclException
   */
  public static void lsetElement(Interp interp, TclObject list, int index, TclObject value)
      throws TclException {
  ...
  /**
   * This procedure returns a pointer to the index'th object from the list referenced by tobj. The
   * first element has index 0. If index is negative or greater than or equal to the number of
   * elements in the list, a null is returned. If tobj is not a list object, an attempt will be made
  ...
  /**
   * This procedure inserts the elements in elements[] into the list at the given index. If tobj is
   * not a list object, an attempt will be made to convert it to a list.
   *
   * @param interp current interpreter.
   * @param tobj the TclObject to use as a list.
   * @param index the starting index of the insertion operation. <=0 means the beginning of the
   *     list. >= TclList.getLength(tobj) means the end of the list.
  ...
  /**
   * This procedure replaces zero or more elements of the list referenced by tobj with the objects
   * from an TclObject array. If tobj is not a list object, an attempt will be made to convert it to
   * a list.
   *
   * @param interp current interpreter.
   * @param tobj the TclObject to use as a list.
   * @param index the starting index of the replace operation. <=0 means the beginning of the list.
   *     >= TclList.getLength(tobj) means the end of the list.
   * @param count the number of elements to delete from the list. <=0 means no elements should be
   *     deleted and the operation is equivalent to an insertion operation.
   * @param elements the element(s) to insert.
   * @param from insert elements starting from elements[from] (inclusive)
   * @param to insert elements up to elements[to] (inclusive)
   * @exception TclException if tobj is not a valid list.
  ...
  /**
   * Sorts the list according to the sort mode and (optional) sort command. If tobj is not a list
   ...
   * @param unique true if only the last set of duplicate elements found in the list have to be
   *     retained
