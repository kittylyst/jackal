...
public class TclDouble implements InternalRep {
  ...
  private static void setDoubleFromAny(
      Interp interp, // Current interpreter.
      // May be null.
    ...
    if (validate) {
      if (tobj.getInternalRep() instanceof TclDouble) {
        throw new TclRuntimeError(
            "should not be TclDouble, was a " + tobj.getInternalRep().getClass().getName());
      }
    }
      ...
      if (Util.isJacl()) {
        ...
        if (validate) {
          ...
          if (!(tmp instanceof TclDouble)) {
            throw new TclRuntimeError("not a TclDouble, is a " + tmp.getClass().getName());
          }
   ...
   * internal rep to a TclDouble with the passed in double value. This method does not invalidate
   * the string rep since the object's value is not being changed.
   *
  ...
  static void exprSetInternalRep(TclObject tobj, double d) {
    if (validate) {
      ...
      if (rep instanceof TclDouble) {
        throw new TclRuntimeError(
            "exprSetInternalRep() called with object" + " that is already of type TclDouble");
      }
      ...
      if (d != d2) {
        throw new TclRuntimeError(
            "exprSetInternalRep() called with double value "
                + d
      ...
      try {
        ...
        throw new TclRuntimeError(
            "should not be able to parse string rep as int: " + tobj.toString());
