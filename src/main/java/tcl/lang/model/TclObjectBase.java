 ...
 * an object is its string value. However, an object can also have an internal representation, which
 * is a "cached" reprsentation of this object in another form. The type of the internal rep of Tcl
 * objects can mutate. This class provides the storage of the string rep and the internal rep, as
 * well as the facilities for mutating the internal rep. The Jacl or TclBlend specific
 * implementation of TclObject will extend this abstract base class.
 */
abstract class TclObjectBase {
  ...
  /**
   * The ivalue field is used for a TclObject that contains an integer value. This implementation
   * uses less memory than the previous approach that stored an int value in a internal rep of type
   * TclInteger. This implementation executes integer operations more quickly since instanceof and
   * upcast operations are no longer needed in the critical execution path. The ivalue field is
   * always set after a call to setInternalRep() in the TclInteger class.
   */
  public long ivalue;
  ...
  /**
   * Creates a TclObject with the given InternalRep and stringRep. This constructor is used by the
   * TclString class only. No other code should call this constructor.
   *
   * @param rep the initial InternalRep for this object.
  ...
  /**
   * Creates a TclObject with the given integer value. This constructor is used by the TclInteger
   * class only. No other code should call this constructor.
   *
  ...
  /**
   * Returns the handle to the current internal rep. This method should be called only by an
   * InternalRep implementation.
   *
  ...
  /**
   * Change the internal rep of the object. The old internal rep will be deallocated as a result.
   * This method should be called only by an InternalRep implementation.
   *
  ...
  /**
   * Sets the string representation of the object to null. Next time when toString() is called,
   * getInternalRep().toString() will be called. This method should be called ONLY when an
   * InternalRep is about to modify the value of a TclObject.
   *
    ...
    if (refCount > 1) {
      throw new TclRuntimeError(
          "string representation of object \""
              + toString()
              + "\" cannot be invalidated: refCount = "
              + refCount);
    }
    stringRep = null;
  }
   ...
   *
   * <p>Duplicate a TclObject, this method provides the preferred means to deal with modification of
   * a shared TclObject. It should be invoked in conjunction with isShared instead of using the
   * deprecated takeExclusive method.
   *
   * <p>Example:
   *
   * <p>if (tobj.isShared()) { tobj = tobj.duplicate(); } TclString.append(tobj, "hello");
   *
  ...
  /**
   * @deprecated The takeExclusive method has been deprecated in favor of the new duplicate()
   *     method. The takeExclusive method would modify the ref count of the original object and
   *     return an object with a ref count of 1 instead of 0. These two behaviors lead to lots of
  ...
  public final TclObject takeExclusive() throws TclRuntimeError {
    ...
    if (refCount == 1) {
      ...
      throw new TclRuntimeError(
          "takeExclusive() called on object \"" + toString() + "\" with: refCount = 0");
    }
  }
  ...
  /**
   * Raise a TclRuntimeError in the case where a TclObject was already disposed of because the last
   * ref was released.
   */
  ...
  /**
   * Return a String that describes TclObject and internal rep type allocations and conversions. The
   * string is in lines separated by newlines. The saveObjRecords needs to be set to true and Jacl
   * recompiled for this method to return a useful value.
   */
