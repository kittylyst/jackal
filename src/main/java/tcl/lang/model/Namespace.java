 ...
 * namespace's name in some other namespace. It is the internal representation for a nsName object.
...
public final class Namespace {
  ...
  /**
   * Number of "activations" or active call frames for this namespace that are on the Tcl call
   * stack. The namespace won't be freed until activationCount becomes zero.
   */
  public int activationCount;

  /**
   * Count of references by nsName objects. The namespace can't be freed until refCount becomes
   * zero.
   */
  public int refCount;
   ...
   * include "string match" style wildcard characters to specify multiple commands; however, no
   * namespace // qualifiers are allowed. null if no export patterns are registered.
   */
   ...
   * This procedure is invoked within findCommand and findNamespaceVar to esolve all command and
  ...
  public static class ResolvedNsName {
    ...
    /**
     * sPtr's unique namespace id. Used to verify that ns is still valid (e.g., it's possible that
     * the namespace was deleted and a new one created at the same address).
     */
    public long nsId;

    /**
     * reference to the namespace containing the reference (not the namespace that contains the
     * referenced namespace).
     */
    public Namespace refNs;

    /**
     * Reference count: 1 for each nsName object that has a pointer to this ResolvedNsName structure
     * as its internal rep. This structure can be freed when refCount becomes zero.
     */
    public int refCount;
  }
  ...
  /**
   * NS_DYING - 1 means deleteNamespace has been called to delete the namespace but there are still
   * active call frames on the Tcl stack that refer to the namespace. When the last call frame
   * referring to it has been popped, it's variables and command will be destroyed and it will be
   * marked "dead" (NS_DEAD). The namespace can no longer be looked up by name.
   */
  static final int NS_DYING = 0x01;

  /**
   * NS_DEAD - 1 means deleteNamespace has been called to delete the namespace and no call frames
   * still refer to it. Its variables and command have already been destroyed. This bit allows the
   * namespace resolution code to recognize that the namespace is "deleted". When the last
   * namespaceName object in any byte code code unit that refers to the namespace has been freed
   * (i.e., when the namespace's refCount is 0), the namespace's storage will be freed.
   */
  public static final int NS_DEAD = 0x02;

  /**
   * Flag passed to getNamespaceForQualName to have it create all namespace components of a
   * namespace-qualified name that cannot be found. The new namespaces are created within their
   * specified parent. Note that this flag's value must not conflict with the values of the flags
  ...
  public static void pushCallFrame(
      Interp interp, // Interpreter in which the
      // new call frame
      // is to be pushed.
  ...
  /**
   * Creates a new namespace with the given name. If there is no active namespace (i.e., the
   * interpreter is being initialized), the global :: namespace is created and returned.
   *
   * @param interp interpreter in which to create new namespace
   * @param name name for the new namespace; may be a qualified name with the ancestors separated
   *     with ::'s
   * @param deleteProc procedure called when namespace is deleted, or null if none should be called
   * @return a reference to the new namespace if successful. If the namespace already exists or if
   *     another error occurs, this routine returns null, along with an error message in the
   *     interpreter's result object.
   *     <p>Side effects: If the name contains "::" qualifiers and a parent namespace does not
   ...
   * <p>Results: None.
   *
   * <p>Side effects: When a namespace is deleted, it is automatically removed as a child of its
   * parent namespace. Also, all its commands, variables and child namespaces are deleted.
   *
  ...
  /**
   * Used internally to dismantle and unlink a namespace when it is deleted. Divorces the namespace
   * from its parent, and deletes all commands, variables, and child namespaces.
   *
   * <p>This is kept separate from Tcl_DeleteNamespace so that the global namespace can be handled
   * specially. Global variables like "errorInfo" and "errorCode" need to remain intact while other
  ...
  /**
   * Makes all the commands matching a pattern available to later be imported from the namespace
   * specified by namespace (or the current namespace if namespace is null). The specified pattern
   * is appended onto the namespace's export pattern list, which is optionally cleared beforehand.
   *
   * <p>Results: Returns if successful, raises TclException if something goes wrong.
   *
   * <p>Side effects: Appends the export pattern onto the namespace's export list. Optionally reset
   * the namespace's export pattern list.
   *
   * @param interp current interpreter
   * @param namespace export commands from this namespace, or null for the current namespace
   * @param pattern String pattern indicating which commands to export. This pattern may not include
   *     any namespace qualifiers; only commands // in the specified namespace may be exported.
   * @param resetListFirst If true, resets the namespace's export list before appending. If false,
   *     return an error if an imported cmd conflicts
   */
  public static void exportList(
      Interp interp, Namespace namespace, String pattern, boolean resetListFirst)
      throws TclException {
    ...
    if ((exportNs != ns) || (pattern.compareTo(simplePattern) != 0)) {
      throw new TclException(
          interp, "invalid export pattern \"" + pattern + "\": pattern can't specify a namespace");
    }
   ...
   *
   * <p>Results: The method will return when successful; in this case the object referenced by obj
   * has each export pattern appended to it. If an error occurs, an exception and the interpreter's
   * result holds an error message.
   *
   * <p>Side effects: If necessary, the object referenced by obj is converted into a list object.
   *
   * @param interp the interpreter used for error reporting
   * @param namespace points to the namespace whose export pattern list is appended onto obj; null
   *     for current namespace
   * @param obj Tcl object onto which current export pattern list is appended
   */
  public static void appendExportList(Interp interp, Namespace namespace, TclObject obj)
      throws TclException {
    Namespace ns;
    int i;
  ...
  /**
   * Imports all of the commands matching a pattern into the namespace specified by namespace (or
   * the current namespace if namespace is null). This is done by creating a new command (the
   * "imported command") that points to the real command in its original namespace.
   *
   * <p>If matching commands are on the autoload path but haven't been loaded yet, this command
   * forces them to be loaded, then creates the links to them. Side effects: Creates new commands in
   * the importing namespace. These indirect calls back to the real command and are deleted if the
   * real commands are deleted.
   *
   * @param interp current interpreter
   * @param namespace reference to the namespace into which the commands are to be imported. null
   *     for the current namespace.
   * @param pattern String pattern indicating which commands to import. This pattern should be
   *     qualified by the name of the namespace from which to import the command(s).
   * @param allowOverwrite If true, allow existing commands to be overwritten by imported commands.
   *     If 0, return an error if an imported cmd conflicts with an existing one.
   */
  public static void importList(
      Interp interp, Namespace namespace, String pattern, boolean allowOverwrite)
      throws TclException {
    ...
    if (importNs == ns) {
      if (pattern.equals(simplePattern)) {
        throw new TclException(
            interp, "no namespace specified in import pattern \"" + pattern + "\"");
      } else {
        throw new TclException(
            interp,
            "import pattern \""
    ...
    for (Map.Entry<String, WrappedCommand> entry : importNs.getCmdTable().entrySet()) {
      ...
      if (Util.stringMatch(cmdName, simplePattern)) {
        ...
        if (oldCommand == null || allowOverwrite) {
          ...
          if (throwException)
            throw new TclException(
                interp, "can't import command \"" + cmdName + "\": already exists");
        }
      }
    }
    return;
   ...
   *
   * @param interp current interpreter
   * @param namespace Points to the namespace from which previously imported commands should be
   *     removed. null for current namespace.
   * @param pattern String pattern indicating which importedcommands to remove. This pattern may be
   *     qualified by the name of the namespace from which the command(s) were imported.
   */
  public static void forgetImport(Interp interp, Namespace namespace, String pattern)
      throws TclException {
    Namespace ns, importNs;
    String simplePattern, cmdName;
    WrappedCommand cmd;
    ...
    if (importNs == null) {
      throw new TclException(
          interp, "unknown namespace in namespace forget pattern \"" + pattern + "\"");
    }
    ...
    if (ns == importNs) {
      /* If the imported namespace was not specified, just delete all matching commands and return  */
      for (WrappedCommand c : matchingCommands) {
        interp.deleteCommandFromToken(c);
      }

    } else {
      /* importNs was specified as part of the pattern, so only delete commands that are imported from it */
  ...
  /**
   * An imported command is created in a namespace when a "real" command is imported from another
   * namespace. If the specified command is an imported command, this procedure returns the original
   * command it refers to.
   *
   * @param command the imported command for which the original command should be returned
   * @return If the command was imported into a sequence of namespaces a, b,...,n where each
   *     successive namespace just imports the command from the previous namespace, this procedure
   *     returns the Tcl_Command token in the first namespace, a. Otherwise, if the specified
   *     command is not an imported command, the procedure returns null.
   */
   ...
   * Finds the "real" command (in another namespace), and passes control to it.
   *
   * <p>Results: Returns if successful, raises TclException if something goes wrong.
   *
   * <p>Side effects: Returns a result in the interpreter's result object. If anything goes wrong,
   * the result object is set to an error message.
   *
   * @param interp
  ...
  /**
   * Invoked by Tcl whenever an imported command is deleted. The "real" command keeps a list of all
   * the imported commands that refer to it, so those imported commands can be deleted when the real
   * command is deleted. This procedure removes the imported command reference from the real
   * command's list, and frees up the memory associated with the imported command.
   *
   * <p>Results: None.
   *
   * <p>Side effects: Removes the imported command from the real command's import list.
   *
  ...
  public static void getNamespaceForQualName(
      Interp interp, // Interpreter in
      // which to find
      // the
   ...
   *
   * @param interp the interpreter in which to find the namespace
   * @param name Namespace name. If it starts with "::", will be looked up in global namespace.
   *     Else, looked up first in contextNs (current namespace if contextNs is null), then in global
   *     namespace.
   * @param contextNs Ignored if TCL.GLOBAL_ONLY flag is set or if the name starts with "::".
   *     Otherwise, points to namespace in which to resolve name; if null, look up name in the
   *     current namespace.
   * @param flags Flags controlling namespace lookup: an OR'd combination of TCL.GLOBAL_ONLY and
   *     TCL.LEAVE_ERR_MSG flags.
   * @return Returns a reference to the namespace if it is found. Otherwise, returns null and leaves
   *     an error message in the interpreter's result object if "flags" contains TCL.LEAVE_ERR_MSG.
   */
  public static Namespace findNamespace(
      Interp interp, String name, Namespace contextNs, int flags) {
    Namespace ns;
    ...
    if (ns != null) {
      ...
      if (name.startsWith("::")) {
        ...
        interp.setResult("namespace \"" + name + "\" not found in \"" + current.fullName + "\"");
      }
    }
    return null;
  }
   ...
   *
   * <p>Searches for a command.
   *
   * @param interp the interpreter in which to find the command
   * @param name the commands name; if it starts with "::" it will be looked up in the global
   *     namespace, otherwise in contextNs, or the current namespace if contextNs is null, and then
   *     the global namespace
   * @param contextNs Ignored if TCL.GLOBAL_ONLY flag set. Otherwise, points to namespace in which
   *     to resolve name. If null, look up name in the current namespace.
   * @param flags An OR'd combination of flags: TCL.GLOBAL_ONLY (look up name only in global
   *     namespace), TCL.NAMESPACE_ONLY (look up only in contextNs, or the current namespace if
   *     contextNs is null), and TCL.LEAVE_ERR_MSG. If both TCL.GLOBAL_ONLY and TCL.NAMESPACE_ONLY
   *     are given, TCL.GLOBAL_ONLY is ignored.
   * @return a token for the command if it is found. Otherwise, if it can't be found or there is an
   *     error, returns null and leaves an error message in the interpreter's result object if
   *     "flags" contains TCL.LEAVE_ERR_MSG.
   */
  public static WrappedCommand findCommand(
      Interp interp, String name, Namespace contextNs, int flags //
      ) throws TclException {
  ...
  public static Var findNamespaceVar(
      Interp interp, // The interpreter in
      // which to find the
      // variable.
   ...
   * ----------------------------------------------------------------------
   *
   * <p>Tcl_SetNamespaceResolvers -> setNamespaceResolver
   *
   * <p>Sets the command/variable resolution object for a namespace, thereby changing the way that
   * command/variable names are interpreted. This allows extension writers to support different name
   * resolution schemes, such as those for object-oriented packages.
   *
   * <p>Command resolution is handled by the following method:
   *
   * <p>resolveCmd (Interp interp, String name, Namespace context, int flags) throws TclException;
   *
   * <p>Whenever a command is executed or Namespace.findCommand is invoked within the namespace,
   * this method is called to resolve the command name. If this method is able to resolve the name,
   * it should return the corresponding WrappedCommand. Otherwise, the procedure can return null,
   * and the command will be treated under the usual name resolution rules. Or, it can throw a
   * TclException, and the command will be considered invalid.
   *
   * <p>Variable resolution is handled by the following method:
   *
   * <p>resolveVar (Interp interp, String name, Namespace context, int flags) throws TclException;
   *
   * <p>If this method is able to resolve the name, it should return the variable as Var object. The
   * method may also return null, and the variable will be treated under the usual name resolution
   * rules. Or, it can throw a TclException, and the variable will be considered invalid.
   *
   * <p>Results: See above.
   *
   * <p>Side effects: None.
   ...
   * <p>----------------------------------------------------------------------
   */
  public static void setNamespaceResolver(
   ...
   * ----------------------------------------------------------------------
   *
   * <p>Tcl_GetNamespaceResolvers -> getNamespaceResolver
   *
   * <p>Returns the current command/variable resolution object for a namespace. By default, these
   * objects are null. New objects can be installed by calling setNamespaceResolver, to provide new
   * name resolution rules.
   *
   * <p>Results: Returns the esolver object assigned to this namespace. Returns null otherwise.
   *
   * <p>Side effects: None.
   *
   * <p>----------------------------------------------------------------------
  ...
  /**
   * Return the first Object value contained in the given table. This method is used only when
   * taking apart a table where entries in the table could be removed elsewhere. An Iterator is no
   * longer valid once entries have been removed so it is not possible to take a table apart safely
   * with a single iterator. This method returns null when there are no more elements in the table,
   * so it should not be used with a table that contains null values. This method is not efficient,
   * but it is required when dealing with a Java Iterator when the table being iterated could have
