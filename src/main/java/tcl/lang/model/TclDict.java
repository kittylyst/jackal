...
public final class TclDict implements InternalRep {
   ...
   * the value map. (key -> key)
   */
  private final Map<TclObject, TclObject> keymap;

  /**
   * Create a new empty Tcl dict, with the hashmap pre-allocated to the given size.
   *
  ...
  public static TclObject newInstance() {
    return new TclObject(new TclDict(16));
  }
   ...
   * @param tobj the TclObject to convert to use the Dict internal rep.
   ...
   *
   * @param interp current interpreter.
   * @param map store the dict elements into this Map.
  ...
  public static void remove(Interp interp, TclObject dict, TclObject key) throws TclException {
    ...
    TclDict ir = (TclDict) dict.getInternalRep();
    TclObject val = ir.map.remove(key);
    if (val != null) {
      val.release();
      TclObject oldKey = ir.keymap.remove(key);
      oldKey.release();
    }
  }
  ...
  /**
   * An interface to traverse (visit) each key-value pair in a Tcl dictionary object. This is an
  ...
  public interface Visitor {
    ...
    Object visit(Interp interp, Object accum, TclObject key, TclObject value) throws TclException;
  }

  /**
   * An internal iterator methods for traversing the elements of a Tcl dictionary object. The
   * TclDict.Visitor object is invoked for each key/value pair in the dictionary. The <code>accum
   * </code> argument can be used to thread some arbitrary state through the traversal, and in this
   * way forms a <em>fold</em> operation over the dictionary, familiar from functional programming
   * languages. The Visitor object can throw a <code>TclException</code> to terminate the loop early
   * either with an error (which will propagate) or with a <code>TCL.BREAK</code> exception which
   * will cause termination of the loop with the current accumulator value. This method will take
   * care of cleaning up resources associated with the loop in the event of any exception &mdash;
   * which is the reason this is structured as an internal iterator rather than a more usual (in
   * Java) external iterator.
   *
   * <p>Equivalent in functionality to the <code>Tcl_DictObjFirst/Next/Done</code> C functions.
   *
   * @param interp A Tcl interpreter for reporting errors.
   * @param accum Arbitrary state to thread through the traversal.
   * @param dict The Tcl dictionary object.
   * @param body The Visitor object to invoke for each key-value pair.
  ...
  public static Object foreach(Interp interp, Object accum, TclObject dict, TclDict.Visitor body)
    ...
    for (var entry : ir.map.entrySet()) {
      entry.getKey().preserve();
      entry.getValue().preserve();
    }
  ...
  public static void removeKeyList(
    ...
    for (int i = start; i < end; ++i) {
      ...
      TclDict ir = (TclDict) current.getInternalRep();
      current.invalidateStringRep();
      TclObject next = ir.map.get(keys[i]);
      if (next == null) {
        throw new TclException(
            interp, "key \"" + keys[i].toString() + "\" not known in dictionary");
      } else if (next.isShared()) {
        // If the nested dict is shared then we need to take a copy
        // of it and store the fresh copy back in the containing
        // dict. This ensures that we have a chain of unshared
    ...
    TclDict ir = (TclDict) current.getInternalRep();
    TclObject oldValue = ir.map.remove(keys[end]);
    if (oldValue != null) {
      oldValue.release();
      TclObject oldKey = (TclObject) ir.keymap.remove(keys[end]);
      oldKey.release();
