...
public class TclPosixException extends TclException {
  ...
  /**
   * Creates a TclException with the appropiate Tcl error message for posix error, and sets the
   * interp's error code to a 3 element list (eg. {"posix" "eexist" {file already exists}}). Does
   * not append posix message to intepreter result.
   *
  ...
  /**
   * Creates a TclException with the appropiate Tcl error message for posix error, and sets the
   * interp's error code to a 3 element list (eg. {"posix" "eexist" {file already exists}}).
   *
   * @param interp Current interpretor, or null
   * @param errno code of posix error
   ...
   * @param appendPosixMsg true if the Posix message should be appended to the errorMessage in the
   *     interpreter result
   * @param errorMessage error message to return in interpreter result
   * @throws TclException
   */
  public TclPosixException(
      Interp interp, IOException ioException, boolean appendPosixMsg, String errorMessage)
      throws TclException {
    super(TCL.ERROR);
  ...
  private void init(Interp interp, int errno, boolean appendPosixMsg, String errorMsg)
      throws TclException {
