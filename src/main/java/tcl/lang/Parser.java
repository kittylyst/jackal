 ...
 * can be used for many different purposes, including compilation, direct execution, code analysis,
 * etc. This class also includes a few additional procedures such as evalObjv, eval, and eval2,
 * which allow scripts to be evaluated directly, without compiling.
 */
public class Parser {

  /**
   * Given a script, parse the first Tcl command in the string and returns information about the
   ...
   * @param numChars Total number of characters in string. If < 0, the script consists of all
   *     characters up to the first null character
   * @param fileName Name of file from which script was loaded. Null means there is no known file
   *     for the script. Used for error messages.
   * @param lineNum Line number of first byte in string. Used for error messages.
   * @param nested True means that this is a nested command: close bracket should be considered a
   *     command terminator. If false, then close bracket has no special meaning.
   * @return a TclParse object that contains information about the parsed command. If the command
   *     was parsed successfully, the TclParse object's result variable is set to TCL_OK and
   *     TCL_ERROR otherwise. If an error occurs and interp isn't null then an error message is left
   *     in its result. Side effects: None.
   */
  static TclParse parseCommand(
      Interp interp,
      char[] script_array,
      int script_index,
      int numChars,
  ...
  static int eolCharCount(char[] script_array, int script_index, int end_index) {
    ...
    if (script_index < end_index + 1
        && script_array[script_index] == '\r'
  ...
  /**
   * This procedure evaluates a Tcl command that has already been parsed into words, with one
   * TclObject holding each word.
   *
   * <p>Results: A result or error message is left in interp's result. If an error occurs, this
   * procedure does NOT add any information to the errorInfo variable.
   *
   * @param interp Interpreter in which to evaluate the ommand. Also used for errorreporting.
   * @param objv An array of pointers to objects that are the words that make up the command.
   * @param length Number of characters in command; if -1, all characters up to the first null
   *     character are used. Currently unused.
  ...
  /**
   * This procedure is invoked after an error occurs in an interpreter. It adds information to the
   * "errorInfo" variable to describe the command that was being executed when the error occurred.
   * Side effects: Information about the command is added to errorInfo and the line number stored
   * internally in the interpreter is set. If this is the first call to this procedure or
   * interp.addErrorInfo since an error occurred, then old information in errorInfo is deleted.
   *
   * @param interp current interpreter
   * @param script_array script to be logged
   * @param script_index first character in script containing command; must be <= cmdIndex
   * @param cmdIndex first character in command that generated the error
   * @param length number of bytes in command, or -1 to use all bytes up to first null byte
   * @param e exception caused by the script evaluation
   */
  ...
  /**
   * This procedure evaluates a Tcl script without using the compiler or byte-code interpreter. It
   * just parses the script, creates values for each word of each command, then calls evalObjv to
   * execute each command.
   *
   * <p>Results: A result or error message is left in interp's result.
   *
   * @param interp interpreter in which to evaluate the script and report errors
   * @param script_array array of characters containing script
   * @param script_index starting index into script array
   * @param numChars number of characters in script; if < 0, the script consists of all characters
   *     up to the end
  ...
  static TclParse parseVarName(
      Interp interp, // Interpreter to use for error
      // reporting;
  ...
  public static ParseResult parseVar(
      Interp interp, // Context for looking up
      // variable.
  ...
  /**
   * Figure out how to handle a backslash sequence. The script_index value must be the index of the
   * first
   *
   ...
   *     in place of the backslash sequence that starts at script_index, and an index to the next
   *     character after the backslash sequence.
   */
  static BackSlashResult backslash(
  ...
  static TclParse ParseQuotedString(
