 ...
 * optionally replace the matched parts with new strings. It serves as a gasket between TCL regular
 * expression style and java.util.regex.* style.
 *
 * <p>Known problems: FIXME
 *
 * <p>- Most important: TCL always attempts to match the longest string starting from the outermost
 * levels to the inner levels of parens. With alternation (|) TCL chooses the longest match of all
 * the branches. Java, on the other hand, evaluates the RE from left to right, and returns the first
 * successful match, even if it's not the longest. This class follows the Java rules, because there
 * doesn't appear to be a way to influence Matcher's behavior to choose the longest. Probably the
 * real solution is to write a custom regex engine that performs according to TCL rules.
 *
 * <p>- BRE's are not supported (embedded option 'b' causes PatternSyntaxException)
 *
 * <p>- ERE's are not supported, unless they are 'ARE'-compatible and are not explicitly requested
 * with the 'e' embedded option ('e' causes PatternSyntaxException)
 *
 * <p>- getInfo(), used by 'regexp -about', doesn't provide flag information. But the test cases in
 * reg.test that use 'regexp -about', and the behavior during 'regexp -about' compile errors is
 * adjusted.
 *
 * <p>- Some syntax errors that would occur in C TCL don't occur here because Java is more forgiving
 * of bad RE syntax
 *
...
public class Regex {
  ...
  /**
   * Stores params in object and compiles given regexp. Additional param 'flags' is a bitwise-or of
   * Regex.TCL_REG_* flags Note that 'flags = flags | TCL_REG_ADVANCED' internally prior to any
   * processing of embedded options.
   *
   * @param regexp TCL-style regular expression
   * @param string input string
   * @param offset offset of the input string where matching starts
   * @param flags Regex.TCL_REG_* flags of pattern object that compiles regexp
   * @throws PatternSyntaxException when there is an error during regexp compilation
   */
  ...
  /**
   * Stores params in object and compiles given regexp. Flags are set to TCL_REG_ADVANCED prior to
   * any processing of embedded options.
   *
   * @param regexp TCL-style regular expression
   * @param string input string
   * @param offset offset of the input string where matching starts
  ...
  /**
   * Stores params in object and compiles given regexp. This constructor is used to support
   * testregexp, which has direct access to flags.
   *
   * @param regexp TCL-style regular expression
   * @param string input string
   * @param offset offset of the input string where matching starts
   * @param flags Regex.TCL_REG_* flags of pattern object that compiles regexp
   * @param xflags Flag string from reg.test (for testregexp)
   * @throws PatternSyntaxException when there is an error during regexp compilation
   */
  ...
  /**
   * Attempts to match the input string against the regular expression. On the first call, it
   * attempts matching starting at the offset specified in the constructor. On subsequent calls, it
   * attempts matching where the previous match succeeded.
   *
  ...
  public boolean match() {
    ...
    if (matchCalled) {
      ...
      matchCalled = true;
      int off = this.offset;
      if (off > string.length()) off = string.length();
  ...
  /**
   * Replaces the subsequence(s) of the input sequence that match the pattern with the given
   * TCL-style replacement string.
   *
   * @param tclSubSpec TCL-regsub-style replacement string
   * @param all If true, all matches are replaced. If false, only the first match is replaced
   * @return The string constructed by replacing the matching subsequence(s) by the replacement
  ...
  public String replace(String tclSubSpec, boolean all) {
    ...
    if (found) javaSubSpec = Regex.parseSubSpec(tclSubSpec);
  ...
  /**
   * Returns a list containing information about the regular expression. The first element of the
   * list is a subexpression count. The second element is a should be a list of property names that
   * describe various attributes of the regular expression. Currently, that property name list is
   * empty.
   *
   * <p>Primarily intended for debugging purposes.
   *
   * @param interp current Jacl interpreter object
   ...
   * <p>parseSubSpec --
   *
   * <p>Parses the replacement string (subSpec param) which is in Tcl's form. This method replaces
   * Tcl's '&' and '\N' where 'N' is a number 0-9. to Java's reference characters. This method also
   * quotes any characters that have special meaning to Java's regular expression APIs.
   *
   * <p>The replacement string (subSpec param) may contain references to subsequences captured
   * during the previous match: Each occurrence of $g will be replaced by the result of evaluating
   * group(g). The first number after the $ is always treated as part of the group reference.
   * Subsequent numbers are incorporated into g if they would form a legal group reference. Only the
   * numerals '0' through '9' are considered as potential components of the group reference. If the
   * second group matched the string "foo", for example, then passing the replacement string "$2bar"
   * would cause "foobar" to be appended to the string buffer. A dollar sign ($) may be included as
   * a literal in the replacement string by preceding it with a backslash (\$).
   *
   * <p>Results: None.
   *
   * <p>Side effects: None.
   *
   * @param subSpec The replacement string
   ...
   * @return the start index of the subsequence captured by the given group during the previous
   *     match operation.
  ...
  /**
   * @param group The index of a capturing group in this matcher's pattern
   * @return The index of the last character captured by the group, plus one, or -1 if the match was
  ...
  /**
   * Return a regexp pattern syntax error message in a format expected by Tcl, primarily to make TCL
   * tests to pass.
   *
   ...
   * @param isEmbed if true, string will be ignored if it does not start with (?, and anything after
   *     ')' will be ignored.
   * @return index+1 of ')' or last character of string
   * @throws PatternSyntaxException if flag string contains unknown or unsupported flags
   */
  protected int parseFlagString(String optionString, boolean isEmbed)
    ...
    if (isEmbed) {
      ...
      if ((this.flags & (TCL_REG_NLSTOP | TCL_REG_NLANCH)) == (TCL_REG_NLSTOP | TCL_REG_NLANCH)) {
        has_eaxn = true;
      }
    }
    for (int i = 0; i < len; i++) {
      ...
      switch (c) {
        ...
        case 'q':
          ...
          break;
        case 's':
          if (isEmbed) this.flags &= ~(TCL_REG_NLSTOP | TCL_REG_NLANCH);
          // else match BOS only FIXME
          break;
        case 't':
          if (isEmbed) this.flags &= ~TCL_REG_EXPANDED;
          else this.flags |= TCL_REG_CANMATCH;
          break;
        case 'w':
          this.flags &= ~TCL_REG_NLSTOP;
          this.flags |= TCL_REG_NLANCH;
  ...
  /**
   * Rewrite TCL regex into a Java regex, and compiles it to a Java Pattern. There are some know
   * problems, see Javadoc at top of file
   *
  ...
  protected Pattern compile(String tclRegex) throws PatternSyntaxException {
    ...
    /** Make sure all the flags that have been specified are actually supported */
    ...
    while (index < regexsb.length() && ((this.flags & TCL_REG_QUOTE) != TCL_REG_QUOTE)) {
      ...
      if (!inBrackets
          ...
          && (index + 3) < regexsb.length()
          && regexsb.charAt(index + 1) == '?'
       ...
       **************************************************************/
      if (bracketExprIndex == 1
          && (this.flags & TCL_REG_NLSTOP) != 0
          && regexsb.charAt(index) == '^') {
       ...
       *****************************************************************/
      if (!inBrackets && ((this.flags & TCL_REG_NLANCH) == TCL_REG_NLANCH)) {
        String replace = null;
        if ('^' == regexsb.charAt(index)) {
          // match at beginning of string or when prev
          // character is a newline
          if (this.offset == 0) replace = "(?:^|(?<=\\n))";
          else
      ...
      /**
       * TCL doesn't allow '^' to match at any offset > 0. Java doesn't either, except when the
       * string is zero-length. If offset > 0, we'll just replace '^' with a zero-length lookahead
       * sequence that can't match. Note that the TCL_REG_NLANCH case is taken care of above.
       */
      if (this.offset > 0
          && !inBrackets
          && regexsb.charAt(index) == '^'
      ...
      if ('[' == regexsb.charAt(index)) {
        char c2;
        if (index + 1 < regexsb.length()) c2 = regexsb.charAt(index + 1);
        ...
        if (c2 == ':' || c2 == '.' || c2 == '=') {
          ...
          if (replacement != null) {
            ...
            if (c2 == ':'
                && (regexsb.charAt(index + 2) == '>' || regexsb.charAt(index + 2) == '<')) {
              --index;
            ...
            throw new PatternSyntaxException(
                charClass.toString() + " not supported", regexsb.toString(), index);
          }
      ...
      if ('{' == regexsb.charAt(index)) {
        ...
        if (index >= regexsb.length() || !Character.isDigit(regexsb.charAt(index))) {
      ...
      if ('\\' == regexsb.charAt(index)) {
        ...
        switch (regexsb.charAt(index)) {
          ...
          case '7':
            ...
            if (value > validSubExpression.size()) {
              if (octalLen >= 2) {
                ...
                throw new PatternSyntaxException(
                    "invalid backreference number", regexsb.toString(), index);
              }
            } else {
          ...
          case 'M':
            ...
            if (inBrackets)
              throw new PatternSyntaxException(
                  "\\M illegal within a bracket expression", regexsb.toString(), index);
          ...
          case 'U':
            ...
            if ("0000".equals(regexsb.substring(index + 1, index + 5))) {
              ...
              throw new PatternSyntaxException(
                  "32-bit unicode value not supported", regexsb.toString(), index);
            }
            break;
          ...
          case 'Y':
            ...
            if (inBrackets)
              throw new PatternSyntaxException(
                  "\\Y illegal within a bracket expression", regexsb.toString(), index);
            escapeReplacement = "B";
            break;
