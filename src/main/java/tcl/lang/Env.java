...
class Env {
  ...
  static void initialize(Interp interp) {
    ...
    try {
      interp.setVar(
          "env", "CLASSPATH", Util.tryGetSystemProperty("java.class.path", ""), TCL.GLOBAL_ONLY);
    } catch (TclException e) {
      // Ignore errors.
    }

    try {
      interp.setVar("env", "HOME", Util.tryGetSystemProperty("user.home", ""), TCL.GLOBAL_ONLY);
    } catch (TclException e) {
      // Ignore errors.
    }

    try {
      interp.setVar("env", "USER", Util.tryGetSystemProperty("user.name", ""), TCL.GLOBAL_ONLY);
    } catch (TclException e) {
      // Ignore errors.
    }
    ...
    try {
      ...
      while (list.hasMoreElements()) {
        ...
        try {
          interp.setVar("env", key, props.getProperty(key), TCL.GLOBAL_ONLY);
