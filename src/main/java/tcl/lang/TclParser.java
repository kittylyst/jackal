...
public final class TclParser implements Command {
  ...
  static void ParseList(
    ...
    charListOffset = (charIndex - script.getIndex());
    ...
    for (; ; ) {
      ...
      try {
        ...
        int byteRange = script.getByteRange(script.getIndex(), charListOffset + listIndex);
      ...
      listIndex = fer.getElemEnd();
      // charLength -= (listIndex - prevListIndex);
      elementIndex = fer.getElemStart();
      size = fer.getSize();
      ...
      TclList.append(
          interp, resultPtr, ParseMakeByteRange(script, charListOffset + elementIndex, size));
    }
  ...
  static ParseMakeTokenListResult ParseMakeTokenList(
    ...
    TclList.append(null, resultList, ParseMakeByteRange(script, token.script_index, token.size));
  ...
  static TclObject ParseMakeRange(
      UTF8CharPointer script, // Pointer to the
      // start of whole
      // script.
      int start, // Index of start of the range, in bytes.
      int length) // The length of the range, in bytes.
      throws TclException {
    int scriptByteIndex = script.getByteIndex(script.getIndex());
  ...
  static TclObject ParseMakeByteRange(
      UTF8CharPointer script, // Pointer to
      // the start of
      // whole script.
  ...
  static void ParseCharIndex(
    ...
    int charIndex = script.getCharIndex(index);
    interp.setResult(charIndex - script.getIndex());
  }
