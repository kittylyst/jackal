...
public class TclIO {
  ...
  /**
   * Table of channels currently registered for all interps. The interpChanTable has "virtual"
   * references into this table that stores the registered channels for the individual interp.
   */
   ...
   * @param pattern Return only channel names that match this patter, as in 'string match', can be
   *     null to match all channels
   * @throws TclException
   */
  ...
  /**
   * Register a channel in this interpreter's channel table. Any channel opening or manipulation
   * must be done by the caller. If one of stdin, stdout or stderr is not defined in the channel
   * table, the channel is registered as the first non-defined standard channel.
   *
   * @param interp This interpreter
   * @param chan channel to register
   */
  ...
  /**
   * Call flush() for each currently registered open channel, ignoring exceptions. Non-blocking
   * channels are guaranteed to be flushed after this call.
   *
  ...
  public static void flushAllOpenChannels(Interp interp) {
    ...
    for (Channel channel : chanTable.values()) {
      if (channel.isWriteOnly() || channel.isReadWrite()) {
        ...
        if (!blockingMode) channel.setBlocking(false);
      }
    }
  }
   ...
   *
   * @param master Interpreter giving the channel
   * @param slave Interpreter receiving the channel
   * @param chanName Name of channel to transfer
   * @param removeFromMaster If true, channel is unregistered from the master after being
   *     transferred
   * @throws TclException if channel cannot be found
   */
  public static void giveChannel(
    ...
    if (channel == null) {
      ...
      channel.refCount++;
      if (removeFromMaster) unregisterChannel(master, channel);
    }
  }
  ...
  /**
   * Initialize this interpreter's channel table if it has not already been initialized.
   *
   * @param interp this interpreter
  ...
  /**
   * @param type one of StdChannel.STDIN, StdChannel.STDOUT, StdChannel.STDERR
  ...
  /**
   * Given a prefix for a channel name (such as "file") returns the next available channel name
   * (such as "file5").
   *
   * @param interp current interpreter.
   * @param prefix string portion of the channel name
