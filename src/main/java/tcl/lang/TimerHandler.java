...
public abstract sealed class TimerHandler
  ...
  /**
   * Create a timer handler to be fired after the given time lapse. Side effects: The timer is
   * registered in the list of timers in the given notifier. After milliseconds have elapsed, the
   * processTimerEvent() method will be invoked exactly once inside the primary thread of the
   * notifier.
   *
  ...
  /**
   * Mark this timer handler as cancelled so that it won't be invoked. Side effects: The timer
   * handler is marked as cancelled so that its processTimerEvent() method will not be called. If
   * the timer has already fired, then nothing this call has no effect.
   */
  ...
  /**
   * Execute the timer handler if it has not been cancelled. This method should be called by the
   * notifier only.
   *
   * <p>Because the timer handler may be being cancelled by another thread, both this method and
  ...
  /**
   * This method is called when the timer is expired. Override This method to implement your own
   * timer handlers.
   */
