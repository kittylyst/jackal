 ...
 *
 * <p>Copyright (c) 1997 Cornell University. Copyright (c) 1997-1998 Sun Microsystems, Inc.
 *
...
public class Interp extends EventuallyFreed {
   ...
   * destroy the structure as soon as all nested invocations of eval() are done.
   */
  ...
  /**
   * Name of the shell class name, used to create [info nameofexecutable]. The constructor sets this
   * values as the name of the main class.
   */
  ...
  public Interp() {
    ...
    try {
      ...
      setVar(
          "tcl_platform",
          "user",
          Util.tryGetSystemProperty("user.name", "unknown"),
          TCL.GLOBAL_ONLY);
      ...
      setVar(
          "tcl_platform",
          "osVersion",
          Util.tryGetSystemProperty("os.version", "?"),
          TCL.GLOBAL_ONLY);
  ...
  /**
   * Invoked to indicate that the interp should be disposed of. If there are no Tcl_Preserve calls
   * in effect for this interpreter, it is deleted immediately, otherwise the interpreter is deleted
   * when the last Tcl_Preserve is matched by a call to Tcl_Release.
   *
   * <p>Results: None.
   *
   * <p>Side effects: Cleans up the interpreter.
   */
  public void dispose() {
   ...
   * This routine needs to break any circular references that might keep the interpreter alive
   * indefinitely.
   *
   * <p>This proc should never be called directly. Instead it is called via the EventuallyFreed
   * superclass. This method will only ever be invoked once.
   *
   * <p>Results: None.
   *
   * <p>Side effects: Cleans up the interpreter.
  ...
  /**
   * Check if an interpreter is ready to eval commands or scripts, i.e., if it was not deleted and
   * if the nesting level is not too high.
   *
   * <p>Results: Raises a TclExcetpion is the interp is not ready.
   *
   * <p>Side effects: The interpreters result is cleared.
   */
  public void ready() throws TclException {
    ...
    if (deleted) {
      ...
      setErrorCode(
          TclString.newInstance("CORE IDELETE {attempt to call eval in deleted interpreter}"));
  ...
  /**
   * Create the build-in commands. These commands are loaded on demand -- the class file of a
   * Command class are loaded into the JVM the first time the given command is executed.
   */
  protected void createCommands() {
    ...
    try {
      eval(
          "package ifneeded tcltest 1.0 {source "
  ...
  /**
   * Creates a named association between user-specified data and this interpreter. If the
   * association already exists the data is overwritten with the new data. The
   * data.deleteAssocData() method will be invoked when the interpreter is deleted.
   *
   * <p>NOTE: deleteAssocData() is not called when an old data is replaced by a new data. Caller of
   * setAssocData() is responsible with deleting the old data.
   *
   * <p>Results: None.
   *
   * <p>Side effects: Sets the associated data, creates the association if needed.
   *
   ...
   * <p>Results: None.
   *
   * <p>Side effects: Deletes the association.
   *
   ...
   *
   * <p>Results: The AssocData instance in the AssocData record denoted by the named association, or
   * null.
   *
   * <p>Side effects: None.
   *
  ...
  /**
   * This procedure is invoked to handle errors that occur in Tcl commands that are invoked in
   * "background" (e.g. from event or timer bindings).
   *
   * <p>Results: None.
   *
   * <p>Side effects: The command "bgerror" is invoked later as an idle handler to process the
   * error, passing it the error message. If that fails, then an error message is output on stderr.
   */
   ...
   *
   * @param nameObj name of variable, array or array element
   * @param value new value for variable
   * @param flags any of TCL.GLOBAL_ONLY, TCL.NAMESPACE_ONLY, TCL.APPEND_VALUE, or TCL.LIST_ELEMENT.
   ...
   * Set the value of a variable
   *
   * @param name name of variable, array or array element
   * @param value new value for variable
   * @param flags any of TCL.GLOBAL_ONLY, TCL.NAMESPACE_ONLY, TCL.APPEND_VALUE, or TCL.LIST_ELEMENT.
   * @return new value for variable
   * @throws TclException
   ...
   * Set the value of a variable
   *
   * @param name1 name of variable if name2 is null, or an array
   * @param name2 name of an array element, or null
   * @param value new value for variable
   * @param flags any of TCL.GLOBAL_ONLY, TCL.NAMESPACE_ONLY, TCL.APPEND_VALUE, or TCL.LIST_ELEMENT.
   * @return new value for variable
   * @throws TclException
   */
  public final TclObject setVar(String name1, String name2, TclObject value, int flags)
      throws TclException {
   ...
   *
   * @param name name of variable, array or array element
   * @param strValue new value for variable
   * @param flags any of TCL.GLOBAL_ONLY, TCL.NAMESPACE_ONLY, TCL.APPEND_VALUE, or TCL.LIST_ELEMENT.
   * @return new value for variable
   * @throws TclException
   */
   ...
   *
   * @param name1 If name2 is null, this is the name of a scalar variable; otherwise it is the name
   *     of an array
   * @param name2 name of element within an array, or null
   * @param strValue new value for variable
   * @param flags any of TCL.GLOBAL_ONLY, TCL.NAMESPACE_ONLY, TCL.APPEND_VALUE, or TCL.LIST_ELEMENT.
   * @return the new value of the variable
   * @throws TclException
   */
  public final TclObject setVar(String name1, String name2, String strValue, int flags)
      throws TclException {
   ...
   * @param flags Various flags that tell how to set valu any of TCL.GLOBAL_ONLY,
   *     TCL.NAMESPACE_ONLY, TCL.APPEND_VALUE, or TCL.LIST_ELEMENT.
  ...
  public final TclObject setVar(String name1, String name2, long intValue, int flags)
      throws TclException {
    return Var.setVar(
        this, name1, name2, checkCommonInteger(intValue), (flags | TCL.LEAVE_ERR_MSG));
  }
   ...
   *     of an array
   * @param name2 name of element within an array, or null
   * @param dValue new value for variable
   * @param flags any of TCL.GLOBAL_ONLY, TCL.NAMESPACE_ONLY, TCL.APPEND_VALUE, or TCL.LIST_ELEMENT.
   * @return the new value of the variable
   * @throws TclException
   */
  public final TclObject setVar(String name1, String name2, double dValue, int flags)
      throws TclException {
   ...
   *     of an array
   * @param name2 name of element within an array, or null
   * @param bValue new value for variable
   * @param flags any of TCL.GLOBAL_ONLY, TCL.NAMESPACE_ONLY, TCL.APPEND_VALUE, or TCL.LIST_ELEMENT.
   * @return the new value of the variable
   * @throws TclException
   */
  public final TclObject setVar(String name1, String name2, boolean bValue, int flags)
      throws TclException {
   ...
   *
   * @param nameObj the name of a variable, array or array element
   * @param flags TCL.GLOBAL_ONLY or TCL.NAMESPACE_ONLY.
   * @return the value of the variable.
   * @throws TclException If the variable is not
   */
  public final TclObject getVar(
      TclObject nameObj, // The name of a variable,
      // array, or array
      // element.
   ...
   * Get the value of a variable. Side effects: May trigger traces.
   *
   * @param name the name of a variable, array or array element
   * @param flags TCL.GLOBAL_ONLY or TCL.NAMESPACE_ONLY.
   * @return the value of the variable.
   * @throws TclException If the variable is not
   */
   ...
   * Get the value of a variable. Side effects: May trigger traces.
   *
   * @param name1 if name2 is null, this is the name of a scalar. Otherwise, it is the name of an
   *     array
   * @param name2 name of an element within an array, or null
   * @param flags TCL.GLOBAL_ONLY or TCL.NAMESPACE_ONLY.
   * @return the value of the variable.
   * @throws TclException If the variable is not
   */
  ...
  /**
   * Replacement for System.getenv(). Returns JTcl's view of the environment, as stored in the
   * ::env() array.
   *
   * <p>JTcl modifications to values in the env array are not visible to System.getenv() due to Java
   * API restrictions.
   *
   ...
   *
   * @param nameObj name of a variable, array or array element
   * @param flags any of TCL.GLOBAL_ONLY or TCL.NAMESPACE_ONLY.
   * @throws TclException
   */
   ...
   * Unset a variable. May trigget traces.
   *
   * @param name name of a variable, array or array element
   * @param flags any of TCL.GLOBAL_ONLY or TCL.NAMESPACE_ONLY.
   * @throws TclException
   */
   ...
   * <p>Side effects: None.
   *
   * @param nameObj name of variable, may contain a parenthesized index value
   * @param trace object to notify when specified ops are invoked upon the variable
   * @param flags OR-ed collection of bits, including any of TCL.TRACE_READS, TCL.TRACE_WRITES,
   *     TCL.TRACE_UNSETS, TCL.GLOBAL_ONLY, TCL.NAMESPACE_ONLY.
   */
   ...
   * Add a trace to a variable.
   *
   * @param name name of variable, may contain a parenthesized index value
   * @param trace object to notify when specified ops are invoked upon the variable
   * @param flags OR-ed collection of bits, including any of TCL.TRACE_READS, TCL.TRACE_WRITES,
   *     TCL.TRACE_UNSETS, TCL.GLOBAL_ONLY, TCL.NAMESPACE_ONLY.
   */
  public void traceVar(
      String name, // Name of variable; may end with
      // "(index)"
   ...
   * @param part2 name of element in within an array, null means part1 is a scalar or entire array
   * @param trace object to notify when specified ops are invoked upon the variable
   * @param flags OR-ed collection of bits, including any of TCL.TRACE_READS, TCL.TRACE_WRITES,
   *     TCL.TRACE_UNSETS, TCL.GLOBAL_ONLY, TCL.NAMESPACE_ONLY.
   */
   ...
   * <p>Side effects: None.
   *
   * @param nameObj Name of variable; may end with "(index)" to signify an array reference.
   * @param trace Object associated with trace.
   * @param flags OR-ed collection of bits describing current trace, including any of
   *     TCL.TRACE_READS, TCL.TRACE_WRITES, TCL.TRACE_UNSETS, TCL.GLOBAL_ONLY and
   *     TCL.NAMESPACE_ONLY.
   */
   ...
   * <p>untraceVar --
   *
   * <p>Remove a trace from a variable.
   *
   * <p>Results: None.
   *
   * <p>Side effects: None.
   *
   * @param name Name of variable; may end with "(index)" to signify an array reference.
   * @param trace Object associated with trace.
   * @param flags OR-ed collection of bits describing current trace, including any of
   *     TCL.TRACE_READS, TCL.TRACE_WRITES, TCL.TRACE_UNSETS, TCL.GLOBAL_ONLY and
   *     TCL.NAMESPACE_ONLY.
   ...
   * <p>Side effects: None.
   *
   * @param part1 Name of scalar variable or array.
   * @param part2 Name of element within array; null means trace applies to scalar variable or array
   *     as-a-whole.
   * @param trace Object associated with trace.
   * @param flags OR-ed collection of bits describing current trace, including any of
   *     TCL.TRACE_READS, TCL.TRACE_WRITES, TCL.TRACE_UNSETS, TCL.GLOBAL_ONLY and
  ...
  /**
   * Define a new command in the interpreter. Side effects: If a command named cmdName already
   * exists for interp, it is deleted. In the future, when cmdName is seen as the name of a command
   * by eval(), cmd will be called. When the command is deleted from the table, cmd.disposeCmd() be
   * called if cmd implements the CommandWithDispose interface.
   *
   * @param cmdName name of the command
   * @param cmdImpl Command instance that implements the command
   */
  ...
  /**
   * Given a token returned by, e.g., Tcl_CreateCommand or Tcl_FindCommand, this procedure returns
   * the command's full name, qualified by a sequence of parent namespace names. The command's
   * fully-qualified name may have changed due to renaming.
   *
   * @param cmd command of which to get the full name
  ...
  /**
   * Given a token returned by, e.g., Tcl_CreateCommand or Tcl_FindCommand, this procedure returns
   * the command's name. The command's fully-qualified name may have changed due to renaming.
   *
   * @param cmd command of which to get the name of
   ...
   *
   * <p>Side effects: CmdName will no longer be recognized as a valid command for the interpreter.
   *
   * @param cmdName name of command to remove in this interpreter
   * @return 0 is returned if the command was deleted successfully. -1 is returned if there didn't
   *     exist a command by that name.
   */
   ...
   * <p>Side effects: cmdName will no longer be recognized as a valid command for the interpreter.
   *
   * @param cmd wrapper token for the command to delete; may be null
   * @return 0 is returned if the command was deleted successfully. -1 is returned if there didn't
   *     exist a command by that name (cmd is null)
   */
  ...
  /**
   * Called to give an existing Tcl command a different name. Both the old command name and the new
   * command name can have "::" namespace qualifiers. If the new command has a different namespace
   * context, the command will be moved to that namespace and will execute in the context of that
   * new namespace.
   *
   * <p>If the new command name is null or the empty string, the command is deleted.
   *
   ...
   *
   * <p>When defining an alias or renaming a command, prevent an alias loop from being formed.
   *
   * <p>Results: A standard Tcl object result.
   *
   * <p>Side effects: If TCL_ERROR is returned, the function also stores an error message in the
   * interpreter's result object.
   *
   * <p>NOTE: This function is public internal (instead of being static to this file) because it is
   * also used from TclRenameCommand.
   *
   * @param cmdInterp Interp in which the command is being defined.
   * @param cmd Tcl command we are attempting to define.
   * @throws TclException
   */
   ...
   * @return The command procedure of the given command, or null if the command doesn't exist. Do
   *     not directly call getCommand().cmdProc(); see WrappedCommand.invoke() and
   *     WrappedCommand.mustCallInvoke()
   */
   ...
   *
   * @param command fully qualified command name
   * @param type ExecutionTrace.ENTER, ExecutionTrace.LEAVE, ExecutionTrace.ENTERSTEP,
   *     ExecutionTrace.LEAVESTEP
   * @param callbackCmd callback command to remove
   * @throws TclException
   */
  public void untraceExecution(String command, int type, TclObject callbackCmd)
      throws TclException {
    WrappedCommand cmd = Namespace.findCommand(this, command, null, TCL.LEAVE_ERR_MSG);
  ...
  /**
   * @return a copy of the active execution step traces. A zero-length array is returned if there
   *     are no active traces
   */
   ...
   * Convenience functions are also available to create a Tcl Object out of the most common Java
   ...
   * <p>Results: None.
   *
   * <p>Side effects: It resets the result object to an unshared empty object. It also clears any
  ...
  /**
   * Convert a string to a valid Tcl list element and append it to the result (which is ostensibly a
   * list).
   *
   * <p>Results: None.
   *
   * <p>Side effects: The result in the interpreter given by the first argument is extended with a
   * list element converted from string. A separator space is added before the converted list
   * element unless the current result is empty, contains the single character "{", or ends in " {".
   *
   * <p>If the string result is empty, the object result is moved to the string result, then the
   * object result is reset.
   *
   ...
   *
   * <p>A standard Tcl Exception may be generated. The interpreter's result object will contain the
   * value of the evaluation but will persist only until the next call to one of the eval functions.
   *
  ...
  /**
   * This procedure adds its command argument to the current list of recorded events and then
   * executes the command by calling eval.
   *
   * <p>Results: The return value is void. However, a standard Tcl Exception may be generated. The
   * interpreter's result object will contain the value of the evaluation but will persist only
   * until the next call to one of the eval functions.
   *
   * <p>Side effects: The side effects will be determined by the exact Tcl code to be evaluated.
   *
   * @param script a script to evaluate
   * @param flags TCL.NO_EVAL records only; TCL.EVAL_GLOBAL evalutes the script in a global context
   * @throws TclException
   */
  ...
  /**
   * Loads a Tcl script from a file using the system encoding and evaluates it in the current
   * interpreter.
   *
   ...
   *
   * @param s Name of the file to evaluate
   * @param encoding Java charset name of file's encoding
   * @throws TclException on any TCL error or on a file read error
   */
  ...
  /**
   * Loads a Tcl script (encoded in system encoding) from a URL and evaluate it in the current
   * interpreter.
   *
   ...
   * @param context URL context under which s is to be interpreted
   * @param s the URL
   * @param javaEncoding Java charset name in which script is encoded
   * @throws TclException on any TCL or read error
   */
   ...
   * @param encoding Java charset encoder name
   ...
   *
   * <p>Read a script from a Java InputStream into a string.
   *
   * @param s Java InputStream containing script
   * @param javaEncoding Java charset name of stream
   ...
   * <p>Side effects: The side effects will be determined by the exact Tcl code to be evaluated.
   *
   * @param resName the location of the Java resource. See the Java documentation of
   *     Class.getResourceAsStream() for details on resource naming.
   */
   ...
   * @param resName the location of the Java resource. See the Java documentation of
   *     Class.getResourceAsStream() for details on resource naming.
   * @param javaEncoding Java charset name for Unicode encoding of resource
   */
   ...
   * <p>Results: None.
   *
   * <p>Side effects: The errorCode global variable is modified to be the new value. A flag is set
   * internally to remember that errorCode has been set, so the variable doesn't get set
   * automatically when the error is returned.
   *
   * <p>If the errorCode variable have write traces, any arbitrary side effects may happen in those
   * traces. TclException's caused by the traces, however, are ignored and not passed back to the
  ...
  /**
   * Add information to the "errorInfo" variable that describes the current error. Side effects: The
   * contents of message are added to the "errorInfo" variable. If eval() has been called since the
   * current value of errorInfo was set, errorInfo is cleared before adding the new message. If we
   * are just starting to log an error, errorInfo is initialized from the error message in the
   * interpreter's result. If the errorInfo variable have write traces, any arbitrary side effects
   * may happen in those traces. TclException's caused by the traces, however, are ignored and not
   * passed back to the caller of this function.
   *
  ...
  /**
   * Procedure called by Tcl_EvalObj to set the interpreter's result value to an appropriate error
   * message when the code it evaluates returns an unexpected result code (not TCL_OK and not
   * TCL_ERROR) to the topmost evaluation level.
   *
   * <p>Results: None.
   *
   * <p>Side effects: The interpreter result is set to an error message appropriate to the result
   * code.
   *
  ...
  /**
   * This method is used by various parts of the Jacl and external packages. interpreter when a
   * TclException of TCL.RETURN is received. The most common case is when the "return" command is
   * executed inside a Tcl procedure. This method examines fields such as interp.returnCode and
   * interp.errorCode and determines the real return status of the Tcl procedure accordingly. Side
   * effects: The errorInfo and errorCode variables may get modified.
   *
   * @return The return value is the true completion code to use for the Tcl procedure, instead of
   *     TCL.RETURN. It's the same value that was given to the "return -code" option.
   *     <p>If TCL.OK is returned, it means than the caller of this method should ignore any
   *     TclException that it has received.
   */
   ...
   * @throws TclException incorrect arguments passed.
   */
  ...
  /**
   * Retrieve the current working directory for this interpreter. Side effects: If the working dir
   * is null, set it to env(HOME)
   *
  ...
  public void setWorkingDir(String dirName) throws TclException {
    ...
    if (dirObj.isDirectory() && dirName.length() > 0) {
      ...
      throw new TclException(
          this,
          "couldn't change working directory to \"" + dname + "\": no such file or directory");
    }
  }

  /**
   * Retrieve the Notifier associated with this Interp. This method can safely be invoked from a
   * thread other than the thread the Interp was created in. If this method is invoked after the
   * Interp object has been disposed of then null will be returned.
   *
  ...
  /**
   * This procedure is invoked to declare that a particular version of a particular package is now
   * present in an interpreter. There must not be any other version of this package already provided
   * in the interpreter.
   *
   * <p>Results: Normally does nothing; if there is already another version of the package loaded
   * then an error is raised.
   *
   * <p>Side effects: The interpreter remembers that this package is available, so that no other
   * version of the package may be provided for the interpreter.
   *
   ...
   * If the package is not already provided in the interpreter, this procedure invokes a Tcl script
   * to provide it. If the package is already provided, this procedure makes sure that the caller's
   * needs don't conflict with the version that is present. Side effects: The script from some
   * previous "package ifneeded" command may be invoked to provide the package.
   *
   ...
   * @return If successful, returns the version string for the currently provided version of the
   *     package, which may be different from the "version" argument.
   * @throws TclException If the caller's requirements cannot be met (e.g. the version requested
   *     conflicts with a currently provided version, or the required version cannot be found, or
   *     the script to provide the required version generates an error), a TclException is raised.
   */
   ...
   *
   * <p>The following section defines two debugging API functions for logging information about the
   * point of execution of Tcl scripts:
   *
   * <p>- pushDebugStack() is called when a procedure body is executed, or when a file is source'd.
   * - popDebugStack() is called when the flow of control is about to return from a procedure body,
   * or from a source'd file.
   *
   * <p>Two other API functions are used to determine the current point of execution:
   *
   * <p>- getScriptFile() returns the script file current being executed. - getArgLineNumber(i)
   * returns the line number of the i-th argument of the current command.
   *
   * <p>Note: The point of execution is automatically maintained for control structures such as
   * while, if, for and foreach, as long as they use Interp.eval(argv[?]) to evaluate control
   * blocks.
   *
   * <p>The case and switch commands need to set dbg.cmdLine explicitly because they may evaluate
   * control blocks that are not elements inside the argv[] array. ** This feature not yet
   * implemented. **
   *
   * <p>The proc command needs to call getScriptFile() and getArgLineNumber(3) to find out the
   * location of the proc body.
   *
   * <p>The debugging API functions in the Interp class are just dummy stub functions. These
   * functions are usually implemented in a subclass of Interp (e.g. DbgInterp) that has real
   * debugging support.
   *
   ...
   * @param lineNumber the line number at which the execution of the new stack level begins.
   */
  ...
  /**
   * Returns the line number where the given command argument begins. E.g, if the following command
   * is at line 10:
   *
   * <p>foo {a b } c
   *
   * <p>getArgLine(0) = 10 getArgLine(1) = 10 getArgLine(2) = 11
   *
  ...
  /**
   * Copy the result (and error information) from one interp to another. Used when one interp has
   * caused another interp to evaluate a script and then wants to transfer the results back to
   * itself.
   *
   * <p>This routine copies the string reps of the result and error information. It does not simply
   * increment the refcounts of the result and error information objects themselves. It is not legal
   * to exchange objects between interps, because an object may be kept alive by one interp, but
   * have an internal rep that is only valid while some other interp is alive.
   *
   * <p>Results: The target interp's result is set to a copy of the source interp's result. The
   * source's error information "$errorInfo" may be appended to the target's error information and
   * the source's error code "$errorCode" may be stored in the target's error code.
   *
   * <p>Side effects: None.
   *
   * @param sourceInterp interp whose result and error information should be moved to the target
   *     interp. AFter moving the results, the interp's result is reset
   * @param result TCL.OK if just the result should be copied, TCL.ERROR if both teh result and the
   *     error information should be copied
   * @throws TclException
   */
  ...
  /**
   * Makes a command hidden so that it cannot be invoked from within an interpreter, only from
   * within an ancestor.
   *
   * <p>Results: A standard Tcl result; also leaves a message in the interp's result if an error
   * occurs.
   *
   * <p>Side effects: Removes a command from the command table and create an entry into the hidden
   * command table under the specified token name.
   *
   * @param cmdName Name of command to hide.
   * @param hiddenCmdToken Token name of the to-be-hidden command.
   * @throws TclException
   */
  public void hideCommand(String cmdName, String hiddenCmdToken) throws TclException {
    ...
    if (hiddenCmdToken.indexOf("::") >= 0) {
      throw new TclException(
          this, "cannot use namespace qualifiers in " + "hidden command token (rename)");
    }
    ...
    cmd =
        Namespace.findCommand(this, cmdName, null, /* flags */ TCL.LEAVE_ERR_MSG | TCL.GLOBAL_ONLY);
  ...
  /**
   * Makes a previously hidden command callable from inside the interpreter instead of only by its
   * ancestors.
   *
   * <p>Results: A standard Tcl result. If an error occurs, a message is left in the interp's
   * result.
   *
   * <p>Side effects: Moves commands from one hash table to another.
   *
   * @param hiddenCmdToken Token name of the to-be-hidden command.
   * @param cmdName Name of command to hide.
   * @throws TclException
  ...
  public void exposeCommand(String hiddenCmdToken, String cmdName) throws TclException {
    ...
    if (cmdName.indexOf("::") >= 0) {
      throw new TclException(
          this, "can not expose to a namespace " + "(use expose to toplevel, then rename)");
    }
   ...
   * ----------------------------------------------------------------------
   *
   * <p>TclHideUnsafeCommands -> hideUnsafeCommands
   *
   * <p>Hides base commands that are not marked as safe from this interpreter.
   *
   * <p>Results: None
   *
   * <p>Side effects: Hides functionality in an interpreter.
   *
   * <p>----------------------------------------------------------------------
   */
  ...
  /**
   * Invokes a Tcl command, given an objv/objc, from either the exposed or hidden set of commands in
   * the given interpreter. NOTE: The command is invoked in the global stack frame of the
   * interpreter, thus it cannot see any current state on the stack of that interpreter.
   *
   * <p>Results: A standard Tcl result.
   *
   * <p>Side effects: Whatever the command does.
   *
   * @param objv Argument objects; objv[0] points to the name of the command to invoke.
   * @param flags Combination of flags controlling the call: INVOKE_HIDDEN,_INVOKE_NO_UNKNOWN, or
   *     INVOKE_NO_TRACEBACK.
   * @return result
   * @throws TclException
   */
  ...
  /**
   * Invokes a Tcl command, given an objv/objc, from either the exposed or the hidden sets of
   * commands in the given interpreter.
   *
   * @param objv argument objects, objv[0] is the name of the command to invoke
   * @param flags Combination of flags controlling the call: INVOKE_HIDDEN,_INVOKE_NO_UNKNOWN, or
   *     INVOKE_NO_TRACEBACK.
  ...
  /**
   * Sets a flag in an interpreter so that exceptions can occur in the next call to Tcl_Eval without
   * them being turned into errors.
   *
   * <p>Results: None.
   *
   * <p>Side effects: The TCL_ALLOW_EXCEPTIONS flag gets set in the interpreter's evalFlags
   ...
   * ----------------------------------------------------------------------
   *
   * <p>Tcl_AddInterpResolvers -> addInterpResolver
   *
   * <p>Adds a set of command/variable resolution procedures to an interpreter. These procedures are
   * consulted when commands are resolved in Namespace.findCommand, and when variables are resolved
   * in Namespace.findNamespaceVar and thus Var.lookupVar. Each namespace may also have its own
   * resolution object which take precedence over those for the interpreter.
   *
   * <p>When a name is resolved, it is handled as follows. First, the name is passed to the
   * resolution objects for the namespace. If not resolved, the name is passed to each of the
   * resolution procedures added to the interpreter. Finally, if still not resolved, the name is
   * handled using the default Tcl rules for name resolution.
   *
   * <p>Results: None.
   *
   * <p>Side effects: The list of resolvers of the given interpreter is modified.
   *
   * @param name Name of this resolution scheme
   * @param resolver Object to resolve commands/variables.
   */
  ...
  /**
   * Looks for a set of command/variable resolution procedures with the given name in an
   * interpreter. These procedures are registered by calling addInterpResolver.
   *
   * <p>Results: If the name is recognized, this procedure returns the object implementing the name
   * resolution procedures. If the name is not recognized, this procedure returns null.
   *
   * <p>Side effects: None.
   *
   * @param name Look for a scheme with scheme.
   ...
   * The next time a command/variable name is resolved, these procedures won't be consulted.
   *
   * <p>Results: Returns true if the name was recognized and the resolution scheme was deleted.
   * Returns false otherwise.
   *
   * <p>Side effects: The list of resolvers of the given interpreter may be modified.
   *
   * @param name Name of the scheme to be removed.
   * @return true if the name was recognized and the resolution scheme was deleted. Returns false
   *     otherwise.
   */
   ...
   * checkCommonInteger()
   *
   * <p>If a given integer value is in the common value pool then return a shared object for that
   * integer. If the integer value is not in the common pool then use to use the recycled int value
   * or a new TclObject.
   *
  ...
  public final TclObject checkCommonInteger(long value) {
    if (VALIDATE_SHARED_RESULTS) {
      TclObject[] objv = {
        m_minusoneIntegerResult, m_zeroIntegerResult, m_oneIntegerResult, m_twoIntegerResult
      };
      for (int i = 0; i < objv.length; i++) {
        TclObject obj = objv[i];
        if (!obj.isShared()) {
  ...
  /**
   * If a given double value is in the common value pool the return a shared object for that double.
   * If the double value is not in the common pool then a new TclDouble wrapped in a TclObject will
   * be created.
   */
  final TclObject checkCommonDouble(double value) {
    if (VALIDATE_SHARED_RESULTS) {
      TclObject[] objv = {
        m_zeroDoubleResult, m_onehalfDoubleResult, m_oneDoubleResult, m_twoDoubleResult
      };
      for (int i = 0; i < objv.length; i++) {
        TclObject obj = objv[i];
        if (!obj.isShared()) {
   ...
   * If the String value is not in the common pool then a new TclString wrapped in a TclObject will
   * be created.
   */
  ...
  /**
   * It is very common to create a TclObject that contains a single character. It can be costly to
   * allocate a TclObject, a TclString internal rep, and a String to represent a character. This
   * method avoids that overhead for the most common characters. This method will return null if a
   * character does not have a cached value.
   */
  public final TclObject checkCommonCharacter(int c) {
    ...
    if (VALIDATE_SHARED_RESULTS) {
      if ((m_charCommon[c] != null) && !m_charCommon[c].isShared()) {
        throw new TclRuntimeError(
            "ref count error: " + "common character for '" + c + "' is not shared");
      }
    }
  ...
  /**
   * Get the TclClassLoader used for the interp. This class loader delagates to the context class
   * loader which delagates to the system class loader. The TclClassLoader will read classes and
   * resources from the env(TCL_CLASSPATH).
   *
   * <p>Results: This method will return the classloader in use, it will never return null.
   *
   * <p>Side effects: None.
   */
  ...
  /**
   * Resolve a resource name into an InputStream. This method will search for a resource using the
   * TclClassLoader. This method will return null if a resource can't be found.
   *
   * <p>Results: None.
   *
   * <p>Side effects: None.
   *
   * @param resName resource to open as InputStream
   * @return InputStream
   */
  ...
  /**
   * Invoke this method to indicate that an executing interp should be interrupted at the next safe
   * moment. Interrupting a running interpreter will unwind the stack by throwing an exception. This
   * method can safely be called from a thread other than the one processsing events. No explicit
   * synchronization is needed. Once a thread has been interrupted or disposed of, setInterrupted()
   * calls will do nothing.
   *
   * <p>Results: Stops execution of the Interp via an Exception.
   *
   * <p>Side effects: None.
   */
  ...
  /**
   * This method is invoked after an eval operation to check if a running interp has been marked as
   * interrupted. This method is not public since it should only be used by the Jacl internal
   * implementation.
   *
   * <p>Results: This method will raise a TclInterruptedException if the Interp.setInterrupted()
   * method was invoked for this interp. This method will only raise a TclInterruptedException once.
   *
   * <p>Side effects: None.
   */
  ...
  /**
   * This method is invoked to cleanup an Interp object that has been interrupted and had its stack
   * unwound. This method will remove any pending events from the Tcl event queue and then invoke
   * the dispose() method for this interp. The interp object should not be used after this method
  ...
  final void disposeInterrupted() {
    ...
    if (interruptedEvent == null) {
      final String msg =
          "Interp.disposeInterrupted() invoked for "
  ...
  /**
   * Get the name of the executable file to invoke JTcl. Typically set on the first call to [info
   * nameofexecutable]
   *
  ...
  /**
   * Set the name of the executable file to invoke JTcl. Typically set on the first call to [info
   * nameofexecutable]
   */
  ...
  /**
   * Get the name of the shell class name to invoke JTcl. Set by default as the constructer of
   * Interp. If shellClassName was set as null, return the name of "tcl.lang.NoInteractiveShell"
   *
