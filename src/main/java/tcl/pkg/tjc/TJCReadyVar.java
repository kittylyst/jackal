        ...
        new TclEvent() {
          public int processEvent(int flags) {
            try {
              ...
              // GENINFO
              TclList.append(interp, tlist, TclString.newInstance(geninfo));

              // FILENAME
              TclList.append(interp, tlist, TclString.newInstance(jfilename));

              // SRCCODE
              TclList.append(interp, tlist, TclString.newInstance(jsrcode));
              ...
              if (cnames != null) {
                for (int i = 0; i < cnames.size(); i++) {
                  TclList.append(
                      interp, cnames_list, TclString.newInstance((String) cnames.get(i)));
                }
              }
              ...
              if (cdata != null) {
                for (int i = 0; i < cdata.size(); i++) {
                  ...
                  if ((bytes.length == 4)
                    ...
                    tobj = ReflectObject.newInstance(interp, byte[].class, bytes);
                  }
              ...
              // MSG
              if (cdata != null) {
                TclList.append(interp, tlist, TclString.newInstance(msg));
              } else {
                TclList.append(interp, tlist, TclString.newInstance(""));
              }
