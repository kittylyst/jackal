...
public class TJCThread implements Runnable {
  ...
  public static interface CompiledClassReady {
    public void compiled(
        final String geninfo, // Name that identifies the
        // source
  ...
  public static synchronized void compileJavaSource(
      String filename, String source, CompiledClassReady callback) {
    if (callback == null) {
  ...
  public static synchronized void compileTclSource(
      String filename, String proc_source, CompiledClassReady callback) {
    if (eventLog != null) {
  ...
  private void processEvent(Vector event) {
    ...
    try {
      ...
      if (interp == null) {
        ...
        interp.eval("source resource:/tcl/pkg/tjc/library/tjcthread.tcl");
      }
  ...
  private void processJavaSource(String filename, String source, CompiledClassReady callback)
      throws TclException {
    if (eventLog != null) {
    ...
    for (int i = 0; i < len; i += 2) {
      ...
      if (!(obj instanceof byte[])) {
        throw new TclException(interp, "obj \"" + obj + "\" is not a byte[] instance");
      }
  ...
  private void processTclSource(String filename, String source, CompiledClassReady callback)
      throws TclException {
    ...
    String proc_name = TclList.index(interp, processTclSourceResult, 0).toString();
    String java_source = TclList.index(interp, processTclSourceResult, 1).toString();
