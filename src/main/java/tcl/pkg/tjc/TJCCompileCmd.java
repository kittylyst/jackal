...
public class TJCCompileCmd implements Command {
    ...
    if (cmd.equals("-java")) {
      ...
      if (TclList.getLength(interp, obj) != 2) {
        throw new TclException(
            interp, "-java JINFO argument must be a 2 element list of {JCLASSNAME JSRCCODE}");
      }
  ...
  static void JavaCompile(Interp interp, String javaInfo, String readyCmd, String readyVar)
      throws TclException {
  ...
  static void TclCompile(Interp interp, String cmd, String readyCmd, String readyVar)
      throws TclException {
    int i, len;
  ...
  static boolean isClassDefined(Interp interp, String javaClassName) {
    ...
    try {
      if (debug) {
        System.out.println("checking for duplicate class name : " + javaClassName);
      }
...
class TJCCompileTclCmd extends TclEvent implements TJCThread.CompiledClassReady {
  ...
  public void compiled(
      final String geninfo, // Name of Tcl command that was
      // compiled.
    ...
    if (debug) {
      System.out.println("TJCCompileTclCmd CompiledClassReady.compiled()");
  ...
  public int processEvent(int flags) {
    ...
    try {
      ...
      instErr = "instance of class " + className + " could not be created";
    } catch (IllegalAccessException iae) {
      instErr = "instance of class " + className + " could not be created";
    }
    if (!(o instanceof TJC.CompiledCommand)) {
      instErr = "instance of class " + className + " must extend TJC.CompiledCommand";
    }
    if (instErr != null) {
      status = false;
      errorMsg = instErr;
  ...
  void readyReport() {
    try {
      ...
      if (readyVar != null) {
        ...
        if (debug) {
          System.out.println("now to set readyVar: " + readyVar + " " + tlist);
        }
  ...
  public int processEvent(int flags) {
    ...
    for (int i = 0; i < cdata.size(); i++) {
      ...
      if (cl == null) {
        ...
        status = false;
        if (cname != null) {
          errorMsg = "class \"" + cname + "\" not loaded by TclClassLoader";
        } else {
          errorMsg = "class not loaded by TclClassLoader";
        }
        readyReport();
      ...
      if (readyVar != null) {
        ...
        if (cnames != null) {
          for (int i = 0; i < cnames.size(); i++) {
            ...
            TclList.append(interp, cnames_list, TclString.newInstance(cname));
          }
        }
