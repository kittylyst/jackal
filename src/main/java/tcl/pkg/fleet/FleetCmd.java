...
public final class FleetCmd implements Command {
  ...
  Namespace ns;
  String fleetName;
  private static final HashMap<String, FleetMember> fleetMembers =
      new HashMap<String, FleetMember>();

  private enum SubCmds {
    create() {

      void eval(final Interp interp, final TclObject argv[], final FleetCmd mCmd)
          throws TclException {
      ...
      void eval(final Interp interp, final TclObject argv[], final FleetCmd mCmd)
        ...
        if (argv.length < 4) {
          throw new TclNumArgsException(
              interp, 2, argv, "memberName msg ?-reply reply? ?-var varName?");
        }
        ...
        if (memberName.equals("*")) {
          ...
          if (member == null) {
            throw new TclException(
                interp,
                "Can't find member \"" + memberName + "\" in fleet \"" + mCmd.fleetName + "\"");
          }
          sendCommand(interp, member, messageList, replyCmd, doneVar);
        }
      }
        ...
        if (!memberName.equals("")) {
          ...
          if (member == null) {
            ...
            TclDict.put(
                interp,
                tDict,
                TclString.newInstance("processing"),
                TclDouble.newInstance(processingTime));
            TclDict.put(
                interp,
                tDict,
                TclString.newInstance("waiting"),
                TclDouble.newInstance(waitingTime));
    ...
    abstract void eval(final Interp interp, final TclObject argv[], final FleetCmd mCmd)
        throws TclException;
  }
      ...
      if (subCmd == null) {
        ...
        if (count == 1) {
          ...
          if (count == 0) {
            ...
            throw new TclException(
                interp, "ambiguous option \"" + name + "\": must be " + valueString);
          }
        }
      }
    }
