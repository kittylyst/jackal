    ...
    try {
      ...
      if (readyVar != null) {
        ...
        if (status) {
          ...
          TclDict.put(
              interp, tDict, TclString.newInstance("status"), TclString.newInstance("FAIL"));
        }

        TclDict.put(
            interp, tDict, TclString.newInstance("fleet"), TclString.newInstance(fleet.fleetName));
        TclDict.put(
            interp, tDict, TclString.newInstance("member"), TclString.newInstance(memberName));
        ...
        if (debug) {
          System.out.println("now to set readyVar: " + readyVar + " " + tDict);
        }
