...
public class BiCmds {
  ...
  private static BiMethod[] BiMethodList = {
    ...
    new BiMethod(
        ...
        new BiConfigureCmd()),
    new BiMethod("isa", "className", "@itcl-builtin-isa", new BiIsaCmd())
  };
  ...
  public static void BiInit(Interp interp) // current interpreter
    ...
    Ensemble.AddEnsemblePart(
        interp, "::itcl::builtin::info", "inherit", "", new BiInfoInheritCmd());

    Ensemble.AddEnsemblePart(
        interp, "::itcl::builtin::info", "heritage", "", new BiInfoHeritageCmd());
    ...
    Ensemble.AddEnsemblePart(
        interp, "::itcl::builtin::info", "args", "procname", new BiInfoArgsCmd());

    Ensemble.AddEnsemblePart(
        interp, "::itcl::builtin::info", "body", "procname", new BiInfoBodyCmd());
    ...
    itclBiNs = Namespace.findNamespace(interp, "::itcl::builtin", null, TCL.LEAVE_ERR_MSG);
    if (itclBiNs == null) {
  ...
  public static void InstallBiMethods(
    ...
    public void cmdProc(
      ...
      if (objv.length != 2) {
        ...
        throw new TclException(
            interp, "wrong # args: should be \"object " + token + " className\"");
      }
    ...
    public void cmdProc(
      ...
      try {
        ...
        for (int i = 1; i < objv.length; i += 2) {
          ...
          if (vlookup == null || vlookup.vdefn.member.protection != Itcl.PUBLIC) {
            throw new TclException(interp, "unknown option \"" + token + "\"");
          }
          if (i == objv.length - 1) {
  ...
  public static final class BiCgetCmd implements Command {
    public void cmdProc(
      ...
      val =
          Objects.GetInstanceVar(
              interp, vlookup.vdefn.member.fullname, contextObj, contextObj.classDefn);
  ...
  private static TclObject ReportPublicOpt(
    ...
    val = Objects.GetInstanceVar(interp, vdefn.member.fullname, contextObj, contextObj.classDefn);
    ...
    public void cmdProc(
      ...
      while ((cdefn = Class.AdvanceHierIter(hier)) != null) {
        ...
        if (mfunc != null) {
          ...
          cmdline = Util.CreateArgs(interp, mfunc.member.fullname, objv, 1);
  ...
  public static final class BiInfoFunctionCmd implements Command {
    ...
    static int[] DefInfoFunction = {BIfProtectIdx, BIfTypeIdx, BIfNameIdx, BIfArgsIdx, BIfBodyIdx};
      ...
      if (cmdName != null) {
        ...
        for (int i = 0; i < objc; i++) {
          switch (iflist[i]) {
            ...
            case BIfTypeIdx:
              val = ((mfunc.member.flags & ItclInt.COMMON) != 0) ? "proc" : "method";
              obj = TclString.newInstance(val);
              break;
          }
  ...
  public static final class BiInfoVariableCmd implements Command {
    ...
    static int[] DefInfoVariable = {BIvProtectIdx, BIvTypeIdx, BIvNameIdx, BIvInitIdx, BIvValueIdx};

    static int[] DefInfoPubVariable = {
      BIvProtectIdx, BIvTypeIdx, BIvNameIdx, BIvInitIdx, BIvConfigIdx, BIvValueIdx
    };
    ...
    public void cmdProc(
      ...
      if (varName != null) {
        ...
        if (objc == 0) {
          if (member.protection == Itcl.PUBLIC && ((member.flags & ItclInt.COMMON) == 0)) {
        ...
        for (int i = 0; i < objc; i++) {
          switch (ivlist[i]) {
            ...
            case BIvInitIdx:
              ...
              if ((member.flags & ItclInt.THIS_VAR) != 0) {
                if (contextObj != null && contextObj.accessCmd != null) {
                  name = contextObj.classDefn.interp.getCommandFullName(contextObj.w_accessCmd);
            ...
            case BIvTypeIdx:
              val = ((member.flags & ItclInt.COMMON) != 0) ? "common" : "variable";
              obj = TclString.newInstance(val);
              break;
