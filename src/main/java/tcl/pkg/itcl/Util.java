...
class Util {
  ...
  static void Assert(
      boolean tExpr, // test expression
      String tExprStr) // string representing test expression
      {
    if (!tExpr) throw new TclRuntimeError("Itcl Assertion failed: \"" + tExprStr + "\"");
  }
  ...
  static void PushStack(
      Object cdata, // data to be pushed onto stack
  ...
  static void DeleteList(Itcl_List list) // list to be deleted
    ...
    Assert(list.validate == Util.VALID_LIST, "list.validate == Util.VALID_LIST");
  ...
  static Itcl_ListElem InsertList(
      Itcl_List list, // list being modified
  ...
  static Itcl_ListElem InsertListElem(
      Itcl_ListElem pos, // insert just before
      // this element
      Object val) // value associated with new element
      {
    Itcl_List list;
  ...
  static Itcl_ListElem AppendList(
      Itcl_List list, // list being modified
      Object val) // value associated with new element
      {
    Itcl_ListElem elem;
  ...
  static Itcl_ListElem AppendListElem(
      Itcl_ListElem pos, // insert just after
      // this element
      Object val) // value associated with new element
      {
    Itcl_List list;
  ...
  static void SetListValue(
      Itcl_ListElem elem, // list element being modified
  ...
  static Itcl_InterpState SaveInterpState(
      Interp interp, // interpreter being
      // modified
  ...
  static int RestoreInterpState(
    ...
    Assert(info.validate == Util.STATE_VALID, "info.validate == Util.STATE_VALID");
  ...
  static int Protection(
    ...
    if (newLevel != 0) {
      Assert(
          newLevel == Itcl.PUBLIC
              || newLevel == Itcl.PROTECTED
              || newLevel == Itcl.PRIVATE
              || newLevel == Itcl.DEFAULT_PROTECT,
          "newLevel Protection");
  ...
  static boolean CanAccess(
    ...
    Assert(member.protection == Itcl.PROTECTED, "member.protection == Itcl.PROTECTED");
  ...
  static boolean CanAccessFunc(
      ItclMemberFunc mfunc, // member function being
      // tested
      Namespace fromNs) // namespace requesting access
      {
    ItclClass cd, fromCd;
  ...
  static Namespace GetTrueNamespace(
      Interp interp, // interpreter being
      // queried
  ...
  static DecodeScopedCommandResult DecodeScopedCommand(
    ...
    if ((len > 17) && name.startsWith("namespace ")) {
      ...
      if (((pos + 8) <= len)
        ...
        try {
          listv = TclList.getElements(interp, TclString.newInstance(name));
        ...
        if (ex == null) {
          if (listv.length != 4) {
            ...
            ns = Namespace.findNamespace(interp, findNS, null, TCL.LEAVE_ERR_MSG);
