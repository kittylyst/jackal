...
public class Methods {
    ...
    public void cmdProc(
      ...
      if (head == null || head.length() == 0) {
        throw new TclException(
            interp, "missing class specifier for body declaration \"" + token + "\"");
      }
      ...
      if (mfunc == null) {
        throw new TclException(
            interp, "function \"" + tail + "\" is not defined in class \"" + cdefn.fullname + "\"");
      }
  ...
  public static final class ConfigBodyCmd implements Command {
    public void cmdProc(
      ...
      if (vlookup == null) {
        throw new TclException(
            interp, "option \"" + tail + "\" is not defined in class \"" + cdefn.fullname + "\"");
      }
      member = vlookup.vdefn.member;

      if (member.protection != Itcl.PUBLIC) {
        throw new TclException(
            interp, "option \"" + member.fullname + "\" is not a public configuration option");
      }
  ...
  static void CreateMethod(
      Interp interp, // interpreter managing this action
  ...
  static ItclMemberFunc CreateMemberFunc(
    ...
    if (!newEntry) {
      throw new TclException(
          interp, "\"" + name + "\" already defined in class \"" + cdefn.fullname + "\"");
    }
  ...
  static void ChangeMemberFunc(
      Interp interp, // interpreter managing this
      // action
    ...
    if ((mfunc.member.flags & ItclInt.ARG_SPEC) != 0
        && !Methods.EquivArgLists(mfunc.arglist, mfunc.argcount, mcode.arglist, mcode.argcount)) {
  ...
  static ItclMemberCode CreateMemberCode(
      Interp interp, // interpreter
      // managing this
      // action
  ...
  static void GetMemberCode(
    ...
    if (!IsMemberCodeImplemented(mcode)) {
      throw new TclException(
          interp,
          "member function \"" + member.fullname + "\" is not defined and cannot be autoloaded");
    }
  ...
  static void EvalMemberCode(
    ...
    for (int i = Util.GetStackSize(info.transparentFrames) - 1; i >= 0; i--) {
      transFrame = (CallFrame) Util.GetStackValue(info.transparentFrames, i);
    ...
    try { // start try block that releases context
      ...
      if ((member.flags & ItclInt.CONSTRUCTOR) != 0
          && contextObj != null
      ...
      if ((member.flags & ItclInt.DESTRUCTOR) != 0
          && contextObj != null
  ...
  static CreateArgListResult CreateArgList(
      Interp interp, // interpreter
      // managing this
      // function
  ...
  static CompiledLocal CreateArg(
  ...
  static void GetMemberFuncUsage(
      ItclMemberFunc mfunc, // command member being
      // examined
    ...
    if ((mfunc.member.flags & ItclInt.COMMON) == 0) {
      if ((mfunc.member.flags & ItclInt.CONSTRUCTOR) != 0 && contextObj.constructed != null) {
        ...
        if (mf == mfunc) {
          String fname =
              contextObj.classDefn.interp.getCommandFullName(contextObj.classDefn.w_accessCmd);
          buffer.append(fname);
          buffer.append(" ");
  ...
  public static final class ExecMethod implements CommandWithDispose {
    ...
    public void cmdProc(
      ...
      if (contextObj == null) {
        throw new TclException(
            interp, "cannot access object-specific info without an object context");
      }
      ...
      if (mfunc.member.protection != Itcl.PUBLIC) {
        Namespace contextNs = Util.GetTrueNamespace(interp, contextClass.info);
      ...
      if (token.indexOf("::") == -1) {
        ItclMemberFunc tmp = (ItclMemberFunc) contextObj.classDefn.resolveCmds.get(member.name);
        if (tmp != null) {
          mfunc = tmp;
          member = mfunc.member;
        }
  ...
  public static final class ExecProc implements CommandWithDispose {
    ...
    public void cmdProc(
      ...
      if (mfunc.member.protection != Itcl.PUBLIC) {
        Namespace contextNs = Util.GetTrueNamespace(interp, mfunc.member.classDefn.info);
  ...
  static void PushContext(
      Interp interp, // interpreter managing this body of
      // code
      ItclMember member, // member containing code body
  ...
  static ParseConfigResult ParseConfig(
      Interp interp, // interpreter
    ...
    while (objc-- > 0) {
      ...
      if (varName.length() < 2 || varName.charAt(0) != '-') {
        throw new TclException(
            interp,
            "syntax error in config assignment \"" + varName + "\": should be \"-variable value\"");
      } else if (objc-- <= 0) {
        throw new TclException(
            interp,
      ...
      vlookup = (ItclVarLookup) contextObj.classDefn.resolveVars.get(varName.substring(1));
  ...
  static void HandleConfig(
      Interp interp, // interpreter currently in control
  ...
  static void InvokeMethodIfExists(
      Interp interp, // interpreter
  ...
  static void ReportFuncErrors(
      Interp interp, // interpreter being modified
