...
public class Objects {
  ...
  static ItclObject CreateObject(
      Interp interp, // interpreter mananging new
      // object
    ...
    WrappedCommand wcmd = Namespace.findCommand(interp, name, null, TCL.NAMESPACE_ONLY);
    // cmd = wcmd.cmd;
      ...
      for (Object o : cd.variables.entrySet()) {
        ...
        if ((vdefn.member.flags & ItclInt.THIS_VAR) != 0) {
          if (cd == cdefn) {
            ...
            interp.traceVar("this", newObj, TCL.TRACE_READS | TCL.TRACE_WRITES);
          }
  ...
  static void DeleteObject(
      Interp interp, // interpreter mananging object
  ...
  static ItclObject FindObject(
      Interp interp, // interpreter containing this
      // object
  ...
  public static final class HandleInstanceCmd implements CommandWithDispose {
    ...
    public void cmdProc(
      ...
      if (objv.length < 2) {
        throw new TclException(
            interp, "wrong # args: should be one of..." + ReportObjectUsage(interp, contextObj));
      }
      ...
      mfunc = (ItclMemberFunc) contextObj.classDefn.resolveCmds.get(token);
      if (mfunc != null) {
  ...
  static String ReportObjectUsage(
      Interp interp, // current interpreter
      ItclObject contextObj) // current object
      {
    ItclClass cdefn = contextObj.classDefn;
  ...
  static void TraceThisVar(
      ItclObject contextObj, // object instance data
  ...
  static void CreateObjVar(
      Interp interp, // interpreter managing this object
    ...
    vlookup = (ItclVarLookup) contextObj.classDefn.resolveVars.get(vdefn.member.fullname);
  ...
  static Var ScopedVarResolver(
    ...
    if (elems.length != 3) {
      throw new TclException(
          interp,
          "scoped variable \"" + name + "\" is malformed: " + "should be: @itcl object variable");
    }
    ...
    if (Objects.IsObject(wcmd)) {
      ...
      throw new TclException(
          interp,
          "can't resolve scoped variable \"" + name + "\": " + "can't find object " + elems[1]);
    }

    // Resolve the variable with respect to the most-specific
    // class definition.

    vlookup = (ItclVarLookup) contextObj.classDefn.resolveVars.get(elems[2].toString());
    if (vlookup == null) {
      throw new TclException(
          interp,
          "can't resolve scoped variable \"" + name + "\": " + "no such data member " + elems[2]);
    }
  ...
  static class ScopedVarResolverImpl implements Resolver {
    public WrappedCommand resolveCmd(
