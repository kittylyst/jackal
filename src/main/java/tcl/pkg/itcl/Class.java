...
public class Class {
  ...
  static ItclClass CreateClass(
      Interp interp, // interpreter that will contain
      // new class
    ...
    if (wcmd != null && !Cmds.IsStub(wcmd)) {
      ...
      if (path.indexOf("::") == -1) {
        buffer.append(" in namespace \"" + Namespace.getCurrentNamespace(interp).fullName + "\"");
      }
    ...
    if (classNs == null) {
      classNs = Namespace.createNamespace(interp, path, new DestroyClassNamespImpl(cd));
    } else {
  ...
  static void DeleteClass(
      Interp interp, // interpreter managing this class
  ...
  static void DeleteClassFailed(Interp interp, String fullName, TclException ex)
      throws TclException {
    StringBuffer buffer = new StringBuffer(64);
  ...
  static Namespace FindClassNamespace(
    ...
    if (classNs == null && contextNs.parent != null && (!path.startsWith("::"))) {
      ...
      if (contextNs.name.equals(path)) {
        ...
        classNs = Namespace.findNamespace(interp, buffer.toString(), null, 0);
      }
    }
    return classNs;
  }
    ...
    public void cmdProc(
      ...
      if (start.indexOf("#auto") != -1) {
        ...
        while (true) {
          ...
          if (interp.getCommand(objName) == null) break; // No command with this name, use it
        }
      }
  ...
  static WrappedCommand ClassCmdResolver(
  ...
  static Var ClassVarResolver(
    ...
    if (varFrame != null && ItclAccess.isProcCallFrame(varFrame) && name.indexOf("::") == -1) {
    ...
    if (contextObj.classDefn != vlookup.vdefn.member.classDefn) {
      ItclVarLookup tmp =
          (ItclVarLookup) contextObj.classDefn.resolveVars.get(vlookup.vdefn.member.fullname);
      if (tmp != null) {
        vlookup = tmp;
      }
    }
  ...
  static void BuildVirtualTables(ItclClass cdefn) // class definition being
    ...
    while (cd != null) {
      for (Object o : cd.variables.entrySet()) {
        ...
        vlookup.accessible =
            (vdefn.member.protection != Itcl.PRIVATE || vdefn.member.classDefn == cdefn);
  ...
  static ItclVarDefn CreateVarDefn(
    ...
    if (!newEntry) {
      throw new TclException(
          interp,
          "variable name \"" + name + "\" already defined in class \"" + cdefn.fullname + "\"");
    }
  ...
  static ItclMember CreateMember(
      Interp interp, // interpreter managing this
      // action
