...
class Parse {
  ...
  public static class ClassCmd implements CommandWithDispose {
    ...
    public void cmdProc(
      ...
      parserNs = Namespace.findNamespace(interp, "::itcl::parser", null, TCL.LEAVE_ERR_MSG);
      ...
      try {
        Namespace.importList(interp, cdefn.namesp, "::itcl::builtin::*", true);
  ...
  public static class ClassInheritCmd implements Command {
    public void cmdProc(
      ...
      for (int i = 1; i < objv.length; i++) {
        ...
        if (baseCdefn == cdefn) {
          // goto inheritError;
          ClassInheritCmdInheritError(
              interp, cdefn, "class \"" + cdefn.name + "\" cannot inherit from itself");
        }
  ...
  static void ClassInheritCmdInheritError(Interp interp, ItclClass cdefn, String exmsg)
      throws TclException {
    Itcl_ListElem elem;
  ...
  public static class ClassCommonCmd implements Command {
    public void cmdProc(
      ...
      if (init != null) {
        TclObject val =
            interp.setVar(
                vdefn.member.name.toString(), TclString.newInstance(init), TCL.NAMESPACE_ONLY);
        if (val == null) {
          throw new TclException(
              interp, "cannot initialize common variable \"" + vdefn.member.name + "\"");
        }
      }
    }
        ...
        if (!vlookup.accessible) {
          throw new TclException(
              ...
              "can't access \""
                  + name
                  + "\": "
                  + Util.ProtectionStr(vlookup.vdefn.member.protection)
                  + " variable");
        }
