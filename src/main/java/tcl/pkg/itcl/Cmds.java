...
public class Cmds {
  ...
  static String initScript =
      "namespace eval ::itcl { source resource:/tcl/pkg/itcl/library/itcl.tcl }";

  // The following script is used to initialize Itcl in a safe interpreter.

  static String safeInitScript =
  ...
  static void Initialize(Interp interp) // interpreter to be updated
    ...
    Ensemble.AddEnsemblePart(
        interp, "::itcl::find", "classes", "?pattern?", new FindClassesCmd(info));
    Util.PreserveData(info);
    ...
    Ensemble.AddEnsemblePart(
        interp, "::itcl::delete", "class", "name ?name...?", new DelClassCmd(info));
    Util.PreserveData(info);

    Ensemble.AddEnsemblePart(
        interp, "::itcl::delete", "object", "name ?name...?", new DelObjectCmd(info));
    Util.PreserveData(info);
    ...
    Ensemble.AddEnsemblePart(interp, "::itcl::is", "class", "name", new IsClassCmd(info));
    Util.PreserveData(info);

    Ensemble.AddEnsemblePart(
        interp, "::itcl::is", "object", "?-class classname? name", new IsObjectCmd(info));
    Util.PreserveData(info);
    ...
    Ensemble.AddEnsemblePart(interp, "::itcl::import::stub", "create", "name", new StubCreateCmd());
    Ensemble.AddEnsemblePart(interp, "::itcl::import::stub", "exists", "name", new StubExistsCmd());
    ...
    itclNs = Namespace.findNamespace(interp, "::itcl", null, TCL.LEAVE_ERR_MSG);
    ...
    interp.setVar(
        "::itcl::patchLevel", TclString.newInstance(Itcl.PATCH_LEVEL), TCL.NAMESPACE_ONLY);

    interp.setVar("::itcl::version", TclString.newInstance(Itcl.VERSION), TCL.NAMESPACE_ONLY);
      ...
      while (Util.GetStackSize(search) > 0) {
        ...
        for (Object o : ns.getCmdTable().entrySet()) {
          ...
          if (Class.IsClass(cmd)) {
            ...
            if (newEntry && (pattern == null || tcl.lang.Util.stringMatch(cmdName, pattern))) {
    ...
    public void cmdProc(
      ...
      if (pos < objv.length) {
        throw new TclNumArgsException(
            interp, 1, objv, "?-class className? ?-isa className? ?pattern?");
      }
      ...
      while (Util.GetStackSize(search) > 0) {
        ...
        for (Object o : ns.getCmdTable().entrySet()) {
          ...
          if (Objects.IsObject(wcmd)) {
            ...
            match = false;
            if (newEntry && (pattern == null || tcl.lang.Util.stringMatch(cmdName, pattern))) {
              if (classDefn == null || (contextObj.classDefn == classDefn)) {
                if (isaDefn == null) {
                  match = true;
                } else {
                  if (contextObj.classDefn.heritage.get(isaDefn) != null) {
                    match = true;
                  }
                }
              }
  ...
  public static final class ScopeCmd implements Command {
    public void cmdProc(
      ...
      if (Class.IsClassNamespace(contextNs)) {
        ...
        if (vlookup == null) {
          throw new TclException(
              interp,
              "variable \"" + token + "\" not found in class \"" + contextClass.fullname + "\"");
        }
        ...
        TclList.append(
            interp, list, TclString.newInstance(interp.getCommandFullName(contextObj.w_accessCmd)));
      ...
      // onward) as well.
        ...
        var = Namespace.findNamespaceVar(interp, token, contextNs, TCL.NAMESPACE_ONLY);

        if (var == null) {
          throw new TclException(
              interp,
              "variable \"" + token + "\" not found in namespace \"" + contextNs.fullName + "\"");
        }
    ...
    public void cmdProc(
      ...
      for (pos = 1; pos < objv.length; pos++) {
        ...
        if (token.equals("-namespace")) {
          if (objv.length == 2) {
            throw new TclNumArgsException(
                interp, 1, objv, "?-namespace name? command ?arg arg...?");
          } else {
            token = objv[pos + 1].toString();
            contextNs = Namespace.findNamespace(interp, token, null, TCL.LEAVE_ERR_MSG);
          ...
          throw new TclException(
              interp, "bad option \"" + token + "\": should be -namespace or --");
        }
      }
