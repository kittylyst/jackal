...
public final class JavaBindCmd implements Command {
  ...
  public void cmdProc(
    ...
    if (argv.length == 2) {
      ...
      Object arr[] = getEventMethod(interp, robj.javaObj, robj.javaClass, argv[2].toString());
      ...
      if (!eventDesc.getListenerType().isInterface()) {
        throw new TclException(
            interp,
            "Cannot handle event listener: "
                + "listererType \""
                + eventDesc.getListenerType()
                + "\" is not an interface");
      }
      ...
      if (argv.length == 3) {
        ...
        TclObject script = eventMgr.getBinding(interp, robj, eventDesc, method);
