...
public class BlendExtension extends Extension {
  ...
  public void init(Interp interp) // Interpreter to intialize.
    ...
    interp.eval(
        "namespace eval ::java {namespace export bind call cast defineclass event field getinterp import info instanceof isnull load new null prop throw try}");
    ...
    TclObject plat = interp.getVar("tcl_platform", "platform", TCL.GLOBAL_ONLY);

    if (plat.toString().equals("java")) {
      interp.setVar("tcljava", "tcljava", TclString.newInstance("jacl"), TCL.GLOBAL_ONLY);
    } else {
      interp.setVar("tcljava", "tcljava", TclString.newInstance("tclblend"), TCL.GLOBAL_ONLY);
    }
