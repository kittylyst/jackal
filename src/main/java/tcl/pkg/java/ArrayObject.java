...
final class ArrayObject extends ReflectObject {
  ...
  private static Class dereferenceClassDims(
      Interp interp, // Current
      // interpreter.
    ...
    if (numDims < numDerefDims) {
      throw new TclException(
          interp, "bad class dereference class only has " + numDims + " dimension(s)");
    }
  ...
  static void setArrayElts(
      ...
      {
    for (int i = 0; i < count; i++, index++) {
      setArrayElt(interp, arrayObj, arrayClass, index, TclList.index(interp, tclValueListObj, i));
    }
  }
