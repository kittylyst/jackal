...
/**
 * This class implements the internal representation of a Java method or constructor signature.
...
class FuncSig implements InternalRep {
  ...
  static FuncSig get(
    ...
    if (isConstructor && Modifier.isAbstract(cls.getModifiers())) {
      throw new TclException(
          interp, "Class \"" + JavaInfoCmd.getNameFromClass(cls) + "\" is abstract");
    }
    ...
    if ((sigLength > 1) || (sigLength == 1 && count == 0)) {
      ...
      match =
          matchSignature(
              interp, cls, signature, methodName, isConstructor, argv, startIdx, count, isStatic);
    }
  ...
  static Method lookupMethod(
    ...
    if (isStatic) methods = getAccessibleStaticMethods(cls);
    else {
  ...
  static Object matchSignature(
    ...
    if (isConstructor) {
      ...
      if (isStatic) funcs = getAccessibleStaticMethods(cls);
      else {
      ...
      if (match_list.size() == 1) {
        ...
        for (i = 0; i < match_list.size(); i++) {
          ...
          for (j = 0; j < argv_count; j++) {
            Class c = match_classes[j];
            TclList.append(
                interp, cur_siglist, TclString.newInstance(JavaInfoCmd.getNameFromClass(c)));
  ...
  static Constructor getAccessibleConstructor(
  ...
  static Method[] getAccessibleInstanceMethods(Class cls) // The class to
    ...
    for (int i = 0; i < methods.length; i++) {
      ...
      if (lst == null) map.put(name, lst = new ArrayList<Method>());
      lst.add(methods[i]);
    }
  ...
  private static int getMethodRank(
      Class declaringCls, // The class that
      // declares the
      // method.
