...
public final class JavaTryCmd implements Command {
   ...
   * <p>cmdProc --
   *
   * <p>This procedure is invoked as part of the Command interface to process the "java::try" Tcl
  ...
  public void cmdProc(Interp interp, TclObject[] argv) throws TclException {
    ...
    if (argv.length < 4 || (!try_catch && !try_catch_finally)) {
      throw new TclNumArgsException(
          interp, 1, argv, "script ?catch exception_pair script? ?finally script?");
    }
    ...
    if (exrec.exception_thrown) {
      if (debug) {
        ...
        if (exrec.reflect_exception != null) System.out.println("ReflectException");
        else if (exrec.tcl_exception != null) System.out.println("TclException");
        else if (exrec.runtime_exception != null) System.out.println("RuntimeException");
        else throw new TclRuntimeError("Java exception not found");
      }
      ...
      for (int i = 2; i < end_loop; i += 3) {
        ...
        if (!catch_clause.toString().equals("catch")) {
          throw new TclException(
              interp, "invalid catch clause \"" + catch_clause.toString() + "\"");
        }
        ...
        if (exception_pair_problem) {
          throw new TclException(
              interp, "invalid exception_pair \"" + exception_pair.toString() + "\"");
        }
        ...
        if (type_name.equals("TclInterruptedException")
          ...
          if (type_name.equals("TclException") && (exrec.tcl_exception != null)) {
            ...
            if (debug) {
              System.out.println(
                  "match for \"" + matched_class.getName() + "\" == \"" + type_name + "\"");
            }
          } else {
        ...
        if (matched_class != null) {
          ...
          if (debug) {
            if (exrec.exception_thrown) System.out.println("exception thrown by exception handler");
            else System.out.println("no exception thrown by exception handler");
          }
    ...
    if (try_catch_finally) {
      ...
      if (!finally_clause.toString().equals("finally")) {
        throw new TclException(
            interp, "invalid finally clause \"" + finally_clause.toString() + "\"");
      }
