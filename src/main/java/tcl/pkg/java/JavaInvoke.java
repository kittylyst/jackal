...
/**
 * This class implements the common routines used by the java::* commands to create Java objects,
 * call Java methods and access fields and properties. It also has auxiliary routines for converting
 * between TclObject's and Java Object's.
 */
public class JavaInvoke {
  ...
  static TclObject newInstance(
    ...
    if (count == 1) {
      ...
      } else if (sig.equals("Character") || sig.equals("java.lang.Character")) {
    ...
    Object javaObj = call(interp, sig.pkgInvoker, signature, sig.func, null, argv, startIdx, count);
  ...
  static TclObject callMethod(
    ...
    if (!PkgInvoker.isAccessible(rtype)) {
      throw new TclException(
          interp, "Return type \"" + JavaInfoCmd.getNameFromClass(rtype) + "\" is not accessible");
    }

    Object result = call(interp, sig.pkgInvoker, signature, method, javaObj, argv, startIdx, count);
   ...
   * @throws TclException
   */
  static TclObject callStaticMethod(
      Interp interp, // Current interpreter.
    ...
    Object result = call(interp, sig.pkgInvoker, signature, method, null, argv, startIdx, count);
  ...
  /**
   * Call the constructor, instance method, or static method with the given parameters. Check the
   * parameter types and perform TclObject to JavaObject conversion
   *
   * @param interp Current interpreter
   * @param func Constructor or method to call
   * @param obj The object associated with an instance method call, should be null for static method
   *     calls
   * @param argv Arguments
   * @param startIdx Index of the first argument in argv[] to pass to the method.
   * @param count Number of arguments to pass to the method.
  ...
  static Object call(
      Interp interp,
      PkgInvoker invoker, // The PkgInvoked used
      // to invoke the
    ...
    if (count == 0) {
      ...
      for (i = 0; i < count; i++) {
        args[i] = convertTclObject(interp, paramTypes[i], argv[i + startIdx]);
      }
    }
    ...
    try {
      ...
      if (debug) {
        System.out.println("result object from invocation is \"" + result + "\"");
      }
   ...
   * @return an array: Object result[2]. result[0] is the value of the field; result[1] is the type
   *     of the field.
   * @throws TclException
   */
  static final TclObject getField(
      Interp interp, // Current interpreter.
  ...
  static TclObject getsetField(
    ...
    if (!PkgInvoker.isAccessible(field.getType())) {
      throw new TclException(
          interp, "Field type \"" + JavaInfoCmd.getNameFromClass(ftype) + "\" is not accessible");
    }
    ...
    try {
      if (isget) {
        return wrap(interp, ftype, sig.pkgInvoker.getField(field, obj), convert);
      } else {
  ...
  static TclObject getProperty(
    ...
    try {
      return wrap(
          ...
          sig.pkgInvoker.invokeMethod(readMethod, javaObj, EMPTY_ARGS),
          convert);
    } catch (IllegalAccessException e) {
  ...
  /**
   * Returns Class object identified by the string name. We allow abbreviation of the java.lang.*
   * class if there is no ambiguity: e.g., if there is no class whose fully qualified name is
   * "String", then "String" means java.lang.String. Inner classes are supported both with fully
   * qualified names and imported class names.
   *
   * @param interp current interpreter
  ...
  /**
   * Converts the java.lang.Object into a Tcl object and return TclObject that holds the result.
   * Primitive data types are converted into primitive Tcl data types. Otherwise, a ReflectObject
   * wrapper is created for the object so that it can be later accessed with the Reflection API.
   *
   * @param interp current interpreter
   ...
   * @throws TclException
   */
  static TclObject convertJavaObject(
      Interp interp, // Current interpreter.
   ...
   */
  static final Object convertTclObject(
      Interp interp, // Current interpreter.
    ...
    if (!isReflectObj) {
      ...
      throw new TclException(
          ...
          "expected object of type "
              ...
              + tclObj
              + "\" ("
              + ((javaClass == null) ? "null" : JavaInfoCmd.getNameFromClass(javaClass))
              + ")");
    }
  }

  /**
   * Wraps a Java Object into a TclObject according to whether the convert flag is set.
   *
   * @param interp current interpreter
   ...
   * @throws TclException
   */
  private static final TclObject wrap(
      Interp interp, // Current interpreter.
      Class cls, // The class of the Java Object
   ...
   */
  static final boolean isAssignable(
      Class to_cls, // The class we want to
      // assign to
  ...
  static void notAccessibleError(Interp interp, Class cls) throws TclException {
    throw new TclException(
        interp, "Class \"" + JavaInfoCmd.getNameFromClass(cls) + "\" is not accessible");
  }

  /**
   * @param cls class to test
   * @return true if is a class is either an inner class or an inner interface. This is true only
   *     for classes defined inside other classes.
   * @throws TclException
   */
