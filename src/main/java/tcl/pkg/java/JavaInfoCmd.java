...
public class JavaInfoCmd implements Command {
      ...
      case EVENTS:
        ...
        lookup:
        {
          ...
          for (int i = 0; i < events.length; i++) {
            TclList.append(
                interp, list, TclString.newInstance(getNameFromClass(events[i].getListenerType())));
          }
  ...
  private static Class getClassFromObj(
  ...
  private static TclObject getPropInfoList(
  ...
  private static TclObject getFieldInfoList(
      Interp interp, // Current
      // interpreter
      // for info
      // query.
  ...
  private static TclObject getMethodInfoList(
      Interp interp, // Current
      // interpreter
      // for info
      // query.
      ...
      if (true) { // FIXME: left in to keep diff simple
        ...
        if (typeOpt) {
          ...
          TclList.append(
              interp,
              sublist,
              TclString.newInstance(getNameFromClass(methodArray[m].getReturnType())));
  ...
  private static TclObject getConstructorInfoList(
      Interp interp, // Current
      // interpreter
      // for info
      // query.
