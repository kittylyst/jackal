...
public sealed class ReflectObject implements InternalRep, CommandWithDispose permits ArrayObject {
  ...
  private static void removeFromReflectTable(ReflectObject roRep) {
    ...
    if (found == null) {
      ...
      throw new TclRuntimeError(
          "reflect table returned null for " + id + " with hash \"" + hash + "\"");
    } else {
      ...
      if (found == roRep) {
        ...
        if (conflicts == null) {
          throw new TclRuntimeError(
              "conflict table mapped to null for " + id + " with hash \"" + hash + "\"");
        }

        if (debug) {
          System.out.println(
              "removing conflict table entry for " + id + " with hash \"" + hash + "\"");
        }
        ...
        if (index == -1) {
          throw new TclRuntimeError(
              "no entry in conflict table for " + id + " with hash \"" + hash + "\"");
        }
  ...
  private static ReflectObject findInConflictTable(Interp interp, Object obj, String hash) {
    ...
    for (ListIterator iter = conflicts.listIterator(); iter.hasNext(); ) {
      ...
      if (found.javaObj == obj) {
        if (debug) {
          System.out.println("found conflict table entry for hash " + hash);
        }
  ...
  private static ReflectObject findInReflectTable(Interp interp, Class cl, Object obj) {
    ...
    if (found == null) {
      if (debug) {
        System.out.println("could not find reflect object for hash \"" + hash + "\"");
      }
      ...
      if (found.javaObj == obj) {
        if (debug) {
          System.out.println(
              "found reflect table match for id " + found.refID + " with hash \"" + hash + "\"");
        }
      ...
      for (var entry : interp.getReflectObjTable().entrySet()) {
        ...
        if (!hash.equals(hash2)) {
          throw new RuntimeException(
              "hash \"" + hash + "\" is not equal to calculated" + " hash \"" + hash2);
        }
  ...
  private static ReflectObject makeReflectObject(Interp interp, Class cl, Object obj)
      throws TclException // if a null class with a non null
        // object is passed in
      {
    ...
    if (debug) {
      System.out.println("object will be reflected as " + JavaInfoCmd.getNameFromClass(cl));
    }
    ...
    if (roRep != null && roRep.isValid) {
      ...
      if (debug) {
        System.out.println(
            "incr useCount of found object " + roRep.refID + " to " + roRep.useCount);
      }
  ...
  public void dispose() {
    ...
    if ((useCount == 0) && (refID != NULL_REP)) {
      ...
      if (debug) {
        System.out.println("reflect object " + refID + " is no longer being used");
      }
      ...
      if (isValid) ownerInterp.deleteCommand(refID);
  ...
  private static void setReflectObjectFromAny(
    ...
    if (s.startsWith(CMD_PREFIX)) {
      if (s.equals(NULL_REP)) {
        ...
        if ((cmd != null) && (cmd instanceof ReflectObject) && ((ReflectObject) cmd).isValid) {
          ...
          if (debug) {
            System.out.println(
                "setReflectObjectFromAny(): incr useCount of "
                    + roRep.refID
                    + " to "
                    + roRep.useCount);
          }
  ...
  public static Object get(
      Interp interp, // Current interpreter. Must be
      // non-null.
  ...
  static ReflectObject getReflectObject(
      Interp interp, // Current interpreter.
      // Must be non-null
      TclObject tobj) // The TclObject to query.
      throws TclException // If the internal rep of tobj cannot
  ...
  public void cmdProc(
    ...
    TclObject result =
        JavaInvoke.callMethod(interp, argv[0], argv[sigIdx], argv, startIdx, count, convert);

    if (result == null) interp.resetResult();
    else interp.setResult(result);
  }
