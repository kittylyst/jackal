 ...
 * If you used java::call directly the entire application would hang because the AWT blocks the
 * calling thread in an AWT modal dialos show() method. You would use this command in place of the
 * java::call method or a java instance method. It will invoke a static or instance Java method
...
public final class UnsupportedJDetachCallCmd implements Command, Runnable {
  ...
  public void cmdProc(Interp interp, TclObject[] argv) throws TclException {
    ...
    if (argv.length < 3) {
      throw new TclNumArgsException(interp, 1, argv, "classorObj signature ?arg arg ...?");
    }
    ...
    try {
      ...
      try {
        ...
        throw new TclException(interp, "unknown java class or object \"" + reflectObj + "\"");
      }
    }
    ...
    if (count != paramTypes.length) {
      throw new TclException(interp, "wrong # args for calling " + "method \"" + signature + "\"");
    }
    ...
    if (count == 0) {
      ...
      for (int i = 0; i < count; i++) {
        call_args[i] = JavaInvoke.convertTclObject(interp, paramTypes[i], argv[i + startIdx]);
      }
    }
