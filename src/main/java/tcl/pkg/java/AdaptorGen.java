 ...
 * implements any given JavaBean event interface. The generated class is used to provide a callback
 * mechanism for JavaBeans to invoke Tcl scripts.
 *
 * <p>The program in src/test/AdaptorGenTest.java can be used to test the operation of the
 * AdaptorGen class -- it saves the data generated by the AdaptorGen class into a .class file, which
 * can then be examined using tools such as javap.
 */
class AdaptorGen {
  ...
  private void generateConstantPool() throws IOException // This exception may
    ...
    for (Enumeration e = returnTypes.keys(); e.hasMoreElements(); ) {
      ...
      if (retType.isPrimitive()) {
        ref =
            cp_putMethodRef(
                cp_super_class, "_return_" + retType.getName(), "()" + getTypeDesc(retType));
      } else {
        cp_putString(retType.getName());
        ref =
            cp_putMethodRef(
                cp_super_class, "_return_Object", "(Ljava/lang/String;)" + getTypeDesc(retType));
      }
    ...
    for (int i = 0; i < methods.length; i++) {
      cp_methodDesc[i] =
          cp_putMethodDesc(methods[i].getName(), getMethodDescriptor(methods[i]), true);
    }
  ...
  private static final String internalClassName(String className) // "Normal"
        // name of
        // the
        // class.
      {
  ...
  private static final Class getWrapperClass(Class primType) // The Class
        // object that
        // represents
        // the
        // primitive type.
  ...
  private static final String getMethodDescriptor(Method method) // Returns
        // the desc
        // of this
        // method.
      {
  ...
  void writeLoadStore(
      DataOutputStream code, // The DataOutputStream to write
      // the byte-code
      // into.
