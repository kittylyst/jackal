...
public final class JavaImportCmd implements Command {
  ...
  public void cmdProc(Interp interp, TclObject[] objv) throws TclException {
    ...
    if (objv.length == 1) {
      ...
      if (wildcardList != null) {
        for (Object wildcardPkgObj : wildcardList) {
          String wildcardPkg = (String) wildcardPkgObj;
      ...
      if (forget) {
        ...
        if (!found) {
          ...
          throw new TclException(
              interp, "cannot forget package \"" + pkg + "\", no classes were imported from it");
        }
    ...
    for (i = startIdx; i < objv.length; i++) {
      ...
      if (forget) {
        if (!wildcardClass && classTable.get(class_name) == null) {
          throw new TclException(
              interp, "cannot forget class \"" + fullyqualified + "\", it was never imported");
        }
      }
    ...
    if (forgetClasses.size() != 0) {
      ...
      for (Object o : forgetClasses) {
        String fullyqualified = (String) o;
        int ind = fullyqualified.lastIndexOf('.');
        if (ind == -1) {
        ...
        if (class_list.remove(cindex) == null) {
          throw new TclRuntimeError(
              "could not remove element at index "
                  + cindex
                  + " from {"
                  + class_list
                  + "}, class_name is "
  ...
  /**
   * Attempt to load a class, successful completion indicates that the class was loaded.
   *
   * @param interp
   ...
   * @throws TclException
   */
  private static void attemptLoadClass(Interp interp, String fullyqualified, String class_name)
      throws TclException {
    ...
    if (c == null) {
      ...
      if (inner != null && JavaInvoke.isInnerClass(inner)) {
        ...
        throw new TclException(
            interp, "cannot import class \"" + fullyqualified + "\", it does not exist");
      }
    }
  }

  /**
   * This method is invoked can be invoked to query the import system to find the fully qualified
   * name of a class. See the user documentation for details on what it does.
   *
   * @param interp Current interp
   * @param name Class name to qualify
   * @return Returns the fully qualified name if it was imported. If the class was not imported then
  ...
  public static String getImport(Interp interp, String name) {
    ...
    ArrayList wildcardList = (ArrayList) wildcardTable.get("*");
    if (wildcardList != null) {
      for (Object classPkgObj : wildcardList) {
        String class_package = (String) classPkgObj;
